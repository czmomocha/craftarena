...
| 内容签名 | **M4b 第 1 章已交**：sidecar 信封，不改 SimulationBundle 字段。ContentHash = StateHasher 规范编码 `to_dictionary()` 的 SHA-256；签名 = HMAC-SHA256(`content_id` + LF + `version` + LF + hash)。对局开局锁定哈希；官方课不要求信封。无 `latest` |
