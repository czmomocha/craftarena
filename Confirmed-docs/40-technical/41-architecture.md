...
| `CreatorTools` | 共用编辑器外壳、玩法专用工具面板、EditCommand、Undo/Redo、AuthoringDocument、独立持续 Preview 窗口、3D 占位映射、传送连线与检查点顺序 gizmos |
