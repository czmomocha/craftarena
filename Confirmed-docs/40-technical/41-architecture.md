...
上面是目标形态，不是当前完成度。截至本刀，`game/src/shared/` 已有稳定 ID、命令信封、领域事件、状态哈希、定点运算与 [CD-42 §1.2](42-contracts-and-rulevm.md#12-字段标识符v1) 组件袋；`backend/contracts/schemas/` 已有 L0 信封与 `component_record` JSON Schema，由 `tools/content-validator/` 做正反例。`game/src/creator/` 已有 AuthoringWorld（put / remove / replace、吸附格、楼层查询、`portal_link` 分类）、AuthoringSession（EditCommand 应用、`expected_revision` 门禁、Undo/Redo 反向 payload）、发布前通路/循环（`evaluate_reachability`）与独立 Preview 会话（安全点 P0–P2 Patch、失败回滚、无结算写）。M1 仿真与灰盒完成度见 [CD-61](../60-plan/61-milestones.md) M1 退出记录。仍待后续：OpenAPI、Rule VM 图 JSON Schema、Preview 窗口/多人试玩、障碍/道具/推击冲量数值，以及 `bot-runner/` / `replay-inspector/`。红线扫描、worktree、shell-guard、`.cursor/agents/` 与 `.cursor/BUGBOT.md` 的状态不在此复述，见 [CD-53](../50-engineering/53-testing-and-ci.md) 与 [CD-52](../50-engineering/52-ai-workflow.md)。
