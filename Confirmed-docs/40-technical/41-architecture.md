...
| `CreatorTools` | 共用编辑器外壳、玩法专用工具面板、EditCommand、Undo/Redo、AuthoringDocument、独立持续 Preview 窗口与 3D 占位映射 |
