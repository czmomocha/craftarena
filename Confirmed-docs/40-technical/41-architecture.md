...
上面是目标形态，不是当前完成度。截至 M2 第一刀（Component Schema v1），`game/src/shared/` 已有稳定 ID、命令信封、领域事件、状态哈希、定点运算与 [CD-42 §1.2](42-contracts-and-rulevm.md#12-字段标识符v1) 组件袋；`backend/contracts/schemas/` 已有 L0 信封与 `component_record` JSON Schema，由 `tools/content-validator/` 做正反例。`game/src/creator/` 已有 AuthoringWorld 骨架（put / remove / revision；不应用 EditCommand）。M1 仿真与灰盒完成度见 [CD-61](../60-plan/61-milestones.md) M1 退出记录。仍待后续：OpenAPI、Rule VM 图 JSON Schema、EditCommand 应用与网格 / Undo / Preview、障碍/道具/推击冲量数值，以及 `bot-runner/` / `replay-inspector/`。红线扫描、worktree、shell-guard、`.cursor/agents/` 与 `.cursor/BUGBOT.md` 的状态不在此复述，见 [CD-53](../50-engineering/53-testing-and-ci.md) 与 [CD-52](../50-engineering/52-ai-workflow.md)。
