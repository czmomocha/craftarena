...
上面是目标形态，不是当前完成度。截至 M1 阶段 B 两域第八轮合入，`game/src/shared/` 已有稳定 ID、命令信封、领域事件、状态哈希与定点运算；`game/src/simulation/` 已有 Tick 计数的 SimulationWorld、确定性 RNG、直立胶囊重叠、XZ/Y 目的地阻挡、静态 AABB、沿半径的离散扫掠、占用感知 try_set_pose、关闭静态盒阻挡、静态盒重叠查询与状态哈希（尚无命令回放带）；`game/src/games/traprush/` 已有有序检查点、传送链（跳数由调用方传入）、MoveIntent / JumpIntent / ShoveIntent 解码、检查点复活落点、推击冷却门闩、IntentStepper、尺度 1 的 Destructible 耐久、ShoveApply 用调用方位移推开目标、PortalLanding 在出口空闲时落地、占用则等待，以及 GrayboxCourse 单人灰盒夹具（尚无占用检查点垫）；`backend/contracts/schemas/` 已有对应的 L0 JSON Schema，由 `tools/content-validator/` 做正反例；`tools/redline-scanner/` 扫描宪法第五、七、十一条的可机械化红线；`.cursor/worktrees.json` 负责隔离工作区 setup；`.cursor/hooks.json` + `tools/shell-guard/` 拦向 `main` 的 git 写操作；`.cursor/agents/` 有六份角色定义（无审查 Agent）；`.cursor/BUGBOT.md` 给 PR 侧 Bugbot 提供门禁 2 红线与 CD-00 链接，**不是合并门禁**。Component Schema v1 的代码与 JSON Schema、OpenAPI、障碍/道具/推击冲量，以及 `bot-runner/` / `replay-inspector/` 仍待后续任务。
