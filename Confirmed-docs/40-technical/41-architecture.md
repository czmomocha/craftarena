...
上面是目标形态，不是当前完成度。截至 M1 阶段 A，`game/src/shared/` 已有稳定 ID、命令信封、领域事件、状态哈希与定点运算；`backend/contracts/schemas/` 已有对应的 L0 JSON Schema，由 `tools/content-validator/` 做正反例；`tools/redline-scanner/` 扫描宪法第五、七、十一条的可机械化红线；`.cursor/worktrees.json` 负责隔离工作区 setup；`.cursor/hooks.json` + `tools/shell-guard/` 拦向 `main` 的 git 写操作。Component Schema v1 的代码与 JSON Schema、OpenAPI、以及 `bot-runner/` / `replay-inspector/` 仍待后续任务。
