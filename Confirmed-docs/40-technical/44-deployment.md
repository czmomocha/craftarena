...
| SQLite | 仅控制面直连 |
| UGC 开局 | **已拍未接线（M5 C4）**：控制面 launch 带 content 引用；MatchHost 用控制面 HTTP 取该 version 信封，**仍不查库**。开局验签并重算 hash。字段见 [CD-42 §3.5](42-contracts-and-rulevm.md#35-匹配与玩家发布-http已拍未接线) |
