...
实现落点（2026-08-24）：对局进程在本场端口监听 WebSocket（`match_server.gd`），连接经 `MatchRealtime` 映射到槽位（按需占用最小空槽、断开释放、满员拒绝）；二进制命令帧（§1 布局）解码后 FIFO 排队，**每占用槽位每个 commit_tick 至多一条**（先到先得，后到拒绝），在服务端自己的 commit_tick 边界按到达顺序应用——命令帧里的 tick 只解码、不信任，服务端 tick 权威；断开丢弃该槽已排队命令。快照帧不能当命令。每 2 个引擎 tick 广播一帧二进制快照（占位节奏，非产品快照频率，见 §4）。快照含全部已配置槽位（含未占用）与可破坏箱耐久。墙钟发送速率仍待（§4）。
