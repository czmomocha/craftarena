...
| 匹配 JSON | **已拍未接线（M5 C4）**：可选 `content` 对象；二进制帧不变。字段形状只在 [CD-42 §3.5](42-contracts-and-rulevm.md#35-匹配与玩家发布-httpc3-已接线c4-已拍未接线) |
...
- 所有客户端一期**产品形态**统一通过 TLS WebSocket 网关接入（宪法第二十二条）。测试开发阶段允许例外，见下方 2026-08-27 落点；
