...
- 对局进程动作数值占位桩：boot 后 `jump_dy`/`support_dy`/`use_item_reach_dz` = `Fixed.SCALE`、伤害 1、reach dx/dy = 0（与 Preview 对齐，不是产品数值）；官方 `course_01` 出生点 UseItem 打碎 +Z 箱且快照耐久 0；Solo 出生点 UseItem 撤橙盒；官方赛道 Jump 仍为空操作；人类真机步骤见 [章节真机清单](../../docs/runbooks/chapter-device-check.md) 本刀（人工检查，非 CI 门禁）；
