...
- 权威 Move 位移门禁：`MOVE_STEP_MAX` 等于 `Fixed.SCALE`；一格正/负/对角 Move 应用；`SCALE+1` 拒绝且位姿不变；线上超限帧可入队但 apply 失败、不续租；人类真机步骤见 [章节真机清单](../../docs/runbooks/chapter-device-check.md) 本刀（人工检查，非 CI 门禁）；
