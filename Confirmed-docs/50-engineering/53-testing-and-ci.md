...
- 编辑器 Place finish：Place finish 写 `zone.tags` 含 `finish` 且编进 `finish`；Editor/Preview 金色占位（`FINISH_ALBEDO`，与大厅 pending 金同色、与 solid 石色不同）；第二份终点仍写入、编译整份拒绝；楼层切换作用于 Place finish；已连接 Preview 跟随；Undo 撤盒；不新增 EDIT `op`；不改官方赛道；GUT 933/933、`npm test` 323；人类真机步骤见 [章节真机清单](../../docs/runbooks/chapter-device-check.md) 本刀（人工检查，非 CI 门禁）；
