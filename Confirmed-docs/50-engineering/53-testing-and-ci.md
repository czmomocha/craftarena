...
**当前（2026-08-21）：人类声明已启用。** 项目负责人当场确认 `main` 已开分支保护。本开发环境未安装 `gh`，未能从 GitHub API 复核「禁止直推 / 要求 CI / 要求一次批准」是否逐项勾上；不得把未核验的勾选写成已证实。`CODEOWNERS` 文件仍未入库，该目标项尚未闭合。此后 Agent 可在隔离分支上创建提交，仍不得向 `main` 提交或推送（[CD-52 §1.1](52-ai-workflow.md)）。
