...
| 字体覆盖（UI 缺字） | 已启用（2026-09-13，只验产物） | GUT `tests/unit/test_font_packaging.gd`：本地化表、常用 3500 字表、项目补集**零缺字**；入包字体改名不含保留字体名；没挂 theme 的 `Control` 运行时拿到的默认字体就是入包那份；单文件体积 < 2 MB。包内自检另加 `ui_font_loadable`。**重新生成子集的工具不进 CI**：`tools/font-subset/` 需要 Python 3 + fontTools，往 CI 加依赖属宪法第十八条。所以门禁保护的是"已生成的那份文件没被改坏"，**不**保证"字表过期后有人重跑"——改了文案要自己跑 `collect_project_chars.py` |
| 字体子集工具 | **不进 CI** | `tools/font-subset/`（Python + fontTools）：下载源字体 → 子集 → 改名 → 落盘，源按 SHA-256 锁定。只服务开发机，见该目录 README |
