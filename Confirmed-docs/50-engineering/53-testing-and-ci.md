...
- 对局大厅本席分色：`follow_slot` 把本席盒涂成 `OWN_ALBEDO`（青）、远端仍 `REMOTE_ALBEDO`（海军蓝）；名次标本席前缀 `*`；`follow_slot < 0` 全员远端色；Solo / 线上 seat 0 与 seat 1 对调；Cancel 清 `follow_slot`；不是产品皮肤；人类真机步骤见 [章节真机清单](../../docs/runbooks/chapter-device-check.md) 本刀（人工检查，非 CI 门禁）；
