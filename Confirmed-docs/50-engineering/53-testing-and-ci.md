...
- 机关狂奔离线单人试玩：官方 course_01 开玩即 1 人快照且 HUD 持续「离线试玩，成绩不上传」；Web / 缺课拒绝；二次开玩须先停；Move 改本地位姿且 `try_advance` 推进 tick；Shove/Interact 不编码；course_01 五步冲线后本地 `finish_tick=0` 且 `allows_online_writes=false`；大厅 Solo 开玩不发 HTTP，在线进行中不能开离线，离线进行中不能开匹配，Cancel 停离线并清玩家盒；不插值、不预测、不结算、不锁幽灵或课程选择 API；
