...
- 对局命令门禁与双人 Headless 冲线：同槽同 tick 第二条命令拒绝且位姿只 +1 格；断开丢弃排队，重入后 commit 不继承旧 Move；快照帧不能当命令；两槽各一条 FIFO；官方 course_01 两槽经 MatchRealtime 各 5 步后 `finish_tick=4` 且 MVP 为 slot 0；同磁带同快照字节与状态哈希；`allows_settlement` / `allows_online_writes` 为 false；不插值、不预测、不结算、不锁墙钟速率；
