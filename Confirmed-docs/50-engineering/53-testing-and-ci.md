...
- 对局基础推击：命令 id 5=`ShoveIntent` 编解码且保留字段须为零；Interact 仍拒绝；两席出生点 Shove 把较低邻座沿 -Z 推开 `SCALE/4`；无目标 / 超限 step / 邻域外拒绝且位姿不变；三席选最近席；同 tick 冷却不位移、commit 后可再推；线上帧可入队并续租；大厅可见窗口 F 上升沿编码、隐藏不采样；Solo 无目标；人类真机步骤见 [章节真机清单](../../docs/runbooks/chapter-device-check.md) 本刀（人工检查，非 CI 门禁）；
