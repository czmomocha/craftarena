...
- 真匹配与房间码：建房发码并签发首票正例；按码加入（大小写不敏感）；满员 409；未知码 404、非法码 400；快速游戏加入最旧未满房，无空位则拉起；运维登记不进快速游戏；邻房不受影响；注销后余票 401、再加入 404；MatchHost 容量满 503、拉起失败 502；席位上限拒绝超额签发；不查库、不锁 FIFO 等待队列/账号绑定/重连补票；
