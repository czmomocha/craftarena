...
- 机关狂奔离线单人试玩：官方 course_01 开玩即 1 人快照且 HUD 持续「离线试玩，成绩不上传」；缺课拒绝；**测试期允许 Web Solo**（已接线；`web_platform` 不再拒绝）；二次开玩须先停；Move 改本地位姿且 `try_advance` 推进 tick；Solo 仅一枚胶囊故 Shove 无目标不编码；Interact 不编码；course_01 五步冲线后本地 `finish_tick=0` 且 `allows_online_writes=false`；大厅 Solo 开玩不发 HTTP，在线进行中不能开离线，离线进行中不能开匹配，Cancel 停离线并清玩家盒；Solo 复用官方赛道选择器；不插值、不预测、不结算；Solo 本机幽灵（R3）是并行会话，直播 `player_count` 仍为 1，不上名次；
...
- 官方赛道选择：HTTP JSON 仍只用 `course_01` / `course_02` / `course_03` / `course_04` / `course_05` / `course_06`；空 body 默认 `course_01`；快速游戏只进同一赛道未满房；队列记住 `course` 且不占邻课余席；按码加入拒绝 body 并回该房课程；未知课 / `course_f_playable` / `res://` 路径 / 多余字段 400；MatchHost 按 id 映射 `--course=` 并登记；大厅跟从响应编译同一赛道，Solo 复用选择器。已签名 UGC 走互斥 `content: { id, version }`（M5 C4）：未知 / 未签名 / 官方课 id / `course_f_playable` / `res://` 400；建房钉死 version；官方响应仍只 `course`；UGC 回 `course: null` + `content` + `content_hash`；MatchHost 经控制面 HTTP 取信封不查库；字段见 [CD-42 §3.5](../40-technical/42-contracts-and-rulevm.md#35-匹配与玩家发布-httpc3-已接线c4-已接线)；
...
- 对局大厅周期机关表现映射：官方三张赛道 0 个盒；编译袋画出洋红 1 米盒；tick 固体半周期显示、开放半周期撤盒且 `live_solid_boxes` 变空；缺课 / 坏袋保留上一份；大厅 HUD `hazards=n/m`；本席 overlay 撞固体机关停在权威、开放后可过；Preview `hazard` 占位同一色，开玩后非固体隐藏；不改协议、不发明 `period`、不读 damage/knockback；GUT 906、`npm test` 319；
- 对局进程动作数值占位桩：boot 后 `use_item_reach_dz` = `Fixed.SCALE`、`jump_dy` = `Fixed.SCALE / 4`、`support_dy` = `-Fixed.SCALE`、伤害 1、reach dx/dy = 0（与 Preview 对齐，不是产品数值）；官方 `course_01` 出生点 UseItem 打碎 +Z 箱且快照耐久 0；Solo 出生点 UseItem 撤橙盒；官方赛道出生点 Jump 接地 hop（该句由立足固体章覆盖；历史「仍为空操作」见当时口径）；
...
- 创作者可测：XYZ SpinBox + 点 Editor 3D 空地设 X/Z（Y 保持当前楼层）；Place bomb / Place dash 写已有 `inventory.item_state` 并编进 `pickups`；未知 kind 拒绝；无 dash 时 Preview Sprint 拒绝；Editor / Preview 按主视口对半锁死 `MODE_WINDOWED` 并排（不按 1600 override 写死），大厅仍最大化；Editor 地板网格；切楼层时已放实体仍可见（引导平面 alpha、不写深度）；点选占位盒拖格点走已有 `set_component`（Shift+拖改 Y）；不新增 EDIT `op`；不改官方赛道 JSON；人类真机步骤见 [开发机窗口验收](../../docs/runbooks/dev-window-check.md) 本刀（人工检查，非 CI 门禁）；
- F 线 FA 时间闭环：大厅 / Solo / Preview 大字局时 `clock=`（`tick/60`，冲线冻在 `finish_tick`）；过垫分段客户端记忆，不进 `hash_state`；结算表画 `rows[]` / `mvp_slot`（Solo 本地 `try_build`，在线 GET 后显示，客户端仍不 POST）；`join=` 状态行仍保留 `settled=` token；不改跳跃 / 协议 / Schema；人类真机步骤见 [开发机窗口验收](../../docs/runbooks/dev-window-check.md) 本刀（人工检查，非 CI 门禁）；
- F 线 FB–FH：Master/Sfx 总线 + 8 个内部临时 WAV（Headless 静默）；机关预警 `HAZARD_WARN_TICKS=15` 表现闪烁；视觉目录按 `asset_id` 回退袋类型并补画 pickup / 传送门 / 出生点；`JUMP_DY=SCALE*5/4`、`FALL_DY=-JUMP_DY`（可玩性深化覆盖为 `JUMP_DY=SCALE/4`、`FALL_DY=-SCALE/32`）；选中实体参数面板与框选填充只走已有 EDIT op；SimulationBundle 可选 `movers` + `try_set_static_box_pose` + 载客出界复位；示范课 `course_f_playable`（不计入 M5 官方课）`--bot-run` 可完赛；FH 后可读性：主路拉开、HUD/Label3D 字号从 spec 读、滚轮调距保持 45°；人类真机步骤见 [开发机窗口验收](../../docs/runbooks/dev-window-check.md) 本刀（人工检查，非 CI 门禁）；
- 可玩性深化第四批：踩区开关门。可选 `switches` / `gates` 袋（省略 ≡ 空，旧编译体照常解码）；编译拒绝与 conveyor / launch / mover 同实体、缺 `interactable`、switch+gate 同体；一组 `link_group` 在被开关支撑或与门盒相交时打开，走开同一拍关上，关在身上 crush 复位；不改命令帧 / 快照帧；Place switch / Place gate；示范课 `course_f_playable` revision 5 主路第一格开关门；人类真机步骤见 [开发机窗口验收](../../docs/runbooks/dev-window-check.md) 本刀（人工检查，非 CI 门禁）；
- 可玩性深化第五批：能量墙 + 机关程序化占位。可选 `energy_walls` 袋（省略 ≡ 空，旧编译体照常解码）；几何和耐久在 `destructibles`，本袋只带 `entity_id`；编译拒绝无 destructible、与 solid / conveyor / launch / switch / gate / lift 同体；打碎走已有 UseItem；`OccupancyGadget` 给传送带 / 电梯 / 弹射垫 / 开关 / 门 / 能量墙挂程序化网格，不铺地块或箱子 glb；Place energy wall；示范课 `course_f_playable` revision 6 主路 x=5、+Z 一格能量墙；不改命令帧 / 快照帧 / 爆破半径；人类真机步骤见 [开发机窗口验收](../../docs/runbooks/dev-window-check.md) 本刀（人工检查，非 CI 门禁）；
- 可玩性深化第六批：开关传送。可选 `portal_switches` 袋（省略 ≡ 空，旧编译体照常解码）；几何在 `portals`，本袋只带 `link_group`；编译拒绝无 portal / 无 interactable / 悬空带 tag、与 solid / conveyor / launch / switch / gate / energy_wall / lift 同体；关上不落地；传送门自己的占用不开组；Place gated portal；示范课 `course_f_playable` revision 7 主路 x=4、+Z 一格开关传送（`link_group=2`）；不改命令帧 / 快照帧；人类真机步骤见 [开发机窗口验收](../../docs/runbooks/dev-window-check.md) 本刀（人工检查，非 CI 门禁）；
- 可玩性深化第七批：地刺 / 喷火 / 压板。可选 `spikes` / `flames` / `crushers` 袋（省略 ≡ 空，旧编译体照常解码）；地刺几何在 `solids`、被支撑才 hazard；喷火几何在 `hazards`、永远非固体、半周期开时重叠才烫；压板是竖直 `mover` + `crusher` 标签，重叠且非乘客 crush；编译拒绝无 solid / 无 hazard / 无竖直 mover、与 conveyor / launch / switch / gate / lift 等同体；Place spike / flame / crusher；示范课 `course_f_playable` revision 8 主路 +Z 一侧三机关；不改命令帧 / 快照帧；人类真机步骤见 [开发机窗口验收](../../docs/runbooks/dev-window-check.md) 本刀（人工检查，非 CI 门禁）；
- 可玩性深化第八批：滚柱 / 碎石 / 障碍核心。可选 `rollers` / `rubbles` / `obstacle_cores` 袋（省略 ≡ 空）；滚柱几何在 `hazards`、半周期固体挡路；碎石与核心几何在 `destructibles`，打碎走已有 UseItem；Place roller / rubble / core；示范课 `course_f_playable` revision 9；不改命令帧 / 快照帧；人类真机步骤见 [开发机窗口验收](../../docs/runbooks/dev-window-check.md) 本刀（人工检查，非 CI 门禁）；
- 可玩性深化第九批：道具栏 HUD。Solo / Preview 显示爆破球 / 冲刺持有数；线上快照无该字段，不画；不改协议；人类真机步骤见 [开发机窗口验收](../../docs/runbooks/dev-window-check.md) 本刀（人工检查，非 CI 门禁）；
- 可玩性深化第十批：摆锤。可选 `pendulums` 袋（省略 ≡ 空）；水平 `mover` + crush；Place pendulum；示范课 `course_f_playable` revision 10；不改命令帧 / 快照帧；人类真机步骤见 [开发机窗口验收](../../docs/runbooks/dev-window-check.md) 本刀（人工检查，非 CI 门禁）；
- 可玩性深化第十一批：冰面。可选 `ices` 袋（省略 ≡ 空）；支撑时按走路步长滑；Place ice；示范课 revision 10；不改协议；人类真机步骤见 [开发机窗口验收](../../docs/runbooks/dev-window-check.md) 本刀（人工检查，非 CI 门禁）；
- 可玩性深化第十二批：失败次数 HUD。Solo / Preview 显示本局环境失败次数；线上快照无该字段，不画；不改协议；人类真机步骤见 [开发机窗口验收](../../docs/runbooks/dev-window-check.md) 本刀（人工检查，非 CI 门禁）；
- 可玩性深化第十三批：打碎碎裂反馈。可破坏占用离开 live set 时喷占位碎片；换课不喷；不进权威；人类真机步骤见 [开发机窗口验收](../../docs/runbooks/dev-window-check.md) 本刀（人工检查，非 CI 门禁）；
...
- 天空盒 A3 多层官方课：`course_06` 七层（站立 `y ∈ {-6,-4,-2,0,2,4,6}`，层距 2 格，底层 13×13 满铺）；AuthoringDocument 走 EditCommand 生成、实体 350–400、发布可达零问题码；匹配 HTTP 接受 `course_06`，仍拒 `course_f_playable` 与 `res://` 路径；`bot_run_cli` 缺省覆盖白名单六张并重放手写三连传送脚本（A* 两次中转不够三段，不搜索）；`--route=safe` 仍只对 `course_01`；顶层两层无弹射垫；人类真机步骤见 [开发机窗口验收](../../docs/runbooks/dev-window-check.md) 本刀（人工检查，非 CI 门禁）；
- M6 E2 第一张官方 BASTION 蓝图：`blueprint_01` 对称双线；AuthoringDocument 编译零问题码、发布前可达通过；两侧对称预算相等；5 波基础表在 5 分钟局时内能跑完；id 白名单拒 `res://` 路径；`blueprint_01` 塞进 TRAPRUSH `course` 仍失败（匹配 HTTP 属 E3）；人类真机步骤见 [开发机窗口验收](../../docs/runbooks/dev-window-check.md) 本刀（人工检查，非 CI 门禁）；
...
- TRAPRUSH 拓扑编译：空世界编成空 bundle；两张官方赛道编出检查点垫、`two_way` 传送、一份终点占用与一份侧向可破坏箱且布局不同；传送袋含源点 `x/y/z`；`finish` 为 0 或 1 个袋；`destructibles` 为 0 或多份袋（含 `durability`）；`hazards` 为 0 或多份袋（含 `cooldown_ticks`）；`solids` 为 0 或多份袋；官方三张赛道 `hazards` 各 1、`solids` 各 2；`dangling` 省略、`one_way` 保留；检查点或缺源点/`finish`/可破坏/机关 `transform` 整份拒绝；两份终点拒绝；可破坏或机关与检查点同实体拒绝；缺 `destructibles` / `hazards` / `solids` 键拒绝；多余键拒绝；加载后垫盒、传送源点盒与终点盒非固体且可占用查询；可破坏箱固体（`durability` 0 已打开）；机关盒固体（tick 0 为固体半周期）；固定固体盒始终固体；`one_way` 可落地；`two_way` 走 `try_land_exit` 而非 `follow`；不 tick 进加载器、不结算；
...
- 内容广场：发布后 `GET /content/plaza` 列出；四 tab；词库名与占用袋标签；未验证不进 verified；`POST /content/:id/plays` 后 verified；自由文本标签 400；Godot `ContentPlaza` 与契约同名；大厅广场窗四 tab + 空列表可关回大厅；`live_io` 时拉控制面列表，Solo 缺 bundle 再 `GET /content/:id/latest`；匹配 HTTP 仍只官方课；
- 玩家代签发布：验证器全绿才 prepare；payload 仅 `bundle` + `content_hash`；201 回显 `id` / `version` / `latest` 且无 signature；控制面 Guest 头或 Bearer 代签入库；无鉴权 401；多余键 / 坏 hash / 坏 bundle 400；持钥 `POST /content/publish` 仍不绑账号；
- 账号认领：`POST /accounts/guest` 签发 Guest；`PUT /drafts` 写入最新文档；注册认领后账号 GET 到同一份；重复用户名 409；错密码 401；自由文本用户名 400；Godot `AccountCatalog` 同动词；大厅账号窗可注册并关回大厅；入场票据仍不绑账号；
- 发布中途进程退出；
- 被下架内容不能创建新房；
- 已开始对局仍能按锁定版本完成。
...
| 受影响单元测试 | 已启用（**仅本地循环，不是门禁**） | `tools/test-selector/`（`npm run test:gut:affected`）：扫 `preload` / `load` 的 `res://` 字面量建反向依赖图，按 `git diff` 选出受影响的 fast 层脚本。`game/` 下非 `.gd` 改动（**例外**：`content/locale/craft_arena.csv` 按 `ui_copy.gd` 进图）、`tests/support/`、`addons/`、图里没有的路径一律退回 fast 层全量；实参非字面量的 `load()` 把依赖它的脚本标成「永远跑」。CI **不**用它，仍跑 fast 层全量——它算错的上限是「某个失败晚几分钟被发现」，不是「某个失败进了 main」 |
...
| Schema 验证 | 已启用（L0 信封 + Component Schema v1 + AuthoringDocument + SimulationBundle + 音频 cue bank v1） | `tools/content-validator/` 对 `backend/contracts/schemas/` 做正反例（含 `component_record`、`authoring_document`、`simulation_bundle` 与 `audio_cue_bank`），并由根目录 `npm test` 收集。生产 bank JSON 还查 catalog 完整性、缺文件、残留 WAV，以及 [CD-11 §8.3](../10-product/11-scope-and-platforms.md) 的格式 / 采样率 / 声道 / 时长 / 单文件 / 总体积；0 个 `.ogg` 明确输出「什么都没查」。未覆盖 Rule VM 图。未引入 Ajv（新依赖属宪法第十八条人类门禁）。字段名单见 [CD-42 §1.2](../40-technical/42-contracts-and-rulevm.md#12-字段标识符v1)、[CD-42 §1.4](../40-technical/42-contracts-and-rulevm.md#14-音频-cue-bank-v1)、[CD-32 §1.4](../30-ugc/32-editor-and-preview.md#14-共同数据模型) 与 [CD-42 §3.4](../40-technical/42-contracts-and-rulevm.md#34-实现落点) |
| 禁止 API 和依赖检查 | 已启用（宪法红线子集） | `tools/redline-scanner/` + CI step `npm run redline-scan`：`simulation/` 禁 SceneTree/`float`、共享核心禁 `.gdextension`、`game/src` 禁 Godot 3 高信号符号、`game/` 禁 `.cs`/`.csproj`/`.sln`（GUT 仍保留同一条）。`game/src/audio/**` 另禁玩法词汇（连字符串字面量）且仅 `audio_backend.gd` 可碰引擎音频类型。Godot 3 黑名单是[官方更名表](https://docs.godotengine.org/en/stable/tutorials/migrating/upgrading_to_godot_4.html)的高信号子集，不是穷尽。第二十三条仍由 ADR-0001 覆盖 |
| 编辑文件的 linter 诊断 | 未实现 | 依赖开发机 IDE，未进 CI |
| 产品 UI 主题与场景静态校验 | 已启用（2026-09-13） | CI `godot` job 跑 `game/tools/ui/validate_theme.gd`（15 项 stylebox + 10 项 font_size 必须存在）与 `validate_scene.gd`（S1 / S2 / S3 能实例化、`theme_type_variation` 全部可解析、挂 `press_feedback.gd` 的节点必须是 `BaseButton`）。查的是 GUT 查不到的东西：变种名写错在运行时不报错，只静默套默认样式。**接进 CI 当天修掉两个假绿**：`get_stylebox() == null` / `get_font_size() <= 0` 恒为假（引擎返回回退值而非 null/0），已改 `has_*`；`--script` 在脚本自身解析失败时 **exit 0**，故该步另外断言输出含 `RESULT: PASS`，且 `game/tools` 已并入 check-only 范围。五种失败模式的故障注入实测见 [ui-wiring.md §0.2](../../docs/runbooks/ui-wiring.md) |
...
| 内容发布和回滚演练 | 部分 | M4b 第 1–5 章已交哈希/HMAC、`latest` 原子切换、新房/旧房并存、P0/P1 回滚、广场列表与账号认领；持钥 `POST /content/publish` 仍不绑账号。玩家代签 / 广场 GET 已交（M5 C3）；匹配 UGC 与可分享邀请已交（M5 C4），结算 HTTP 仍不改 |
...
| 回滚演练通过 | 未实现 | 同 §4.3 |
| 玩法清单签署 | 未实现 | E6 首次结论已签（好玩）；**M5 退出清单已签**（2026-09-13，八项「通过」，总结论「基本通过」）。两者都**不是**本表——本表是**发布候选**清单，仍未签，且两次签署都不是外部真人测试 |
...
8. 文档或 Schema 已同步；
9. AI 能解释关键实现和失败模式；
10. 人类完成必要审查；若本章有开发机可见行为，按 [开发机窗口验收](../../docs/runbooks/dev-window-check.md) 本刀的编号步骤执行，不得用「AI 说看起来对」代替。未勾选真机不等于自动门禁已覆盖（宪法第二十四条）；义务口径见 [CD-52 §3.2](52-ai-workflow.md)；
