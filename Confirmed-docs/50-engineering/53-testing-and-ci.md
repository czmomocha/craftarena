...
- 官方赛道选择：HTTP JSON 只用 `course_01` / `course_02` / `course_03`；空 body 默认 `course_01`；快速游戏只进同一赛道未满房；队列记住 `course` 且不占邻课余席；按码加入拒绝 body 并回该房课程；未知课 / `res://` 路径 / 多余字段 400；MatchHost 按 id 映射 `--course=` 并登记；大厅跟从响应编译同一赛道，Solo 复用选择器；不锁人数按场下发、账号绑定、插值/预测；
