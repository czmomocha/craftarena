...
| 导出 | 三套预设已在 `game/export_presets.cfg`；测试期可用 `CRAFTARENA_WEB_ROOT` 挂 `/play/` |
...
- 日志：开发构建输出结构化日志；
- 导出：三套预设已写入 `game/export_presets.cfg`（Windows desktop、Linux Headless、Web）。测试期可用 `CRAFTARENA_WEB_ROOT` 让控制面在 `/play/` 托管该 Web 包。Android / iOS 仍占位；开工时机见 [CD-61](../60-plan/61-milestones.md)（一期收尾）。
