...
| 导出 | 三套预设已在 `game/export_presets.cfg`；`npm run export:web` 一条命令出 Web 包；测试期可用 Nginx `:80` 或 `CRAFTARENA_WEB_ROOT` 挂 `/play/` |
...
- 日志：开发构建输出结构化日志；
- 导出：三套预设已写入 `game/export_presets.cfg`（Windows desktop、Linux Headless、Web）。一条命令是 `npm run export:web`。测试期 Nginx `:80` 或 `CRAFTARENA_WEB_ROOT` `/play/` 都能托管该包，见 [CD-61 §2 M-Export](../60-plan/61-milestones.md#m-export平台导出与-web)。Android / iOS 仍占位；开工时机见 [CD-61](../60-plan/61-milestones.md)（一期收尾）。
