...
当前（2026-08-21）**A1、A4 已成立，A2、A3 未成立。** A1：`game/src/shared/` 已有 v1 信封 / 定点与 GUT（[PR #1](https://github.com/czmomocha/craftarena/pull/1)）。A4：该 PR 走通「Agent 产出 → PR → CI 全绿 → 人类 review → 人类合并」。A3 的 L0 JSON Schema 正反例已进 `npm test`；红线扫描与 worktree 基建仍缺。A2（`CODEOWNERS`）仍缺。**A1–A4 不得报全绿**，不得开启多域并行。中间不并行。
