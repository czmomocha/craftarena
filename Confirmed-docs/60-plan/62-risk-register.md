...
| 传送迷路或跳关 | **已治理**（可玩性深化：跳变镜头过渡 + 多层寻路读数 + 头顶方向箭头） |
| 测试期远端明文 | **已接受**（D11） |
| 单人审查带宽超载 | **已缓解**（D9 粒度 5× + 审查分级；E10 不设周上限，深审 = PR 失败模式 + 人类批准） |
...
| P0/P1 对所有运行中房间立即全量发布 | 高，已缓解 | 玩法安全边界、不可变 PatchHash、进程内技术自动回滚（ContentHash / HMAC / `latest` 新房生效已交；P0/P1 运行房补丁与进程内 `note_fault` 反向 Patch 已交；广场与账号认领已交；发布 HTTP / 入场票据仍不绑账号） |
...
| 传送迷路或跳关 | 高，已治理 | 检查点序列与发布前通路/循环验证已有。可玩性深化补上三件：**镜头过渡**——跳变（传送 / 复位）时跟随相机滑行约 0.35 s 而不是同帧换机位；HUD 与本席头顶给出下一个目标的**屏幕**方向、楼层差与距离；传送门从色块换成会转的占位模型。常态移动**没有**引入跟随延迟 |
| BASTION 互设障碍不公平 | 高，已治理 | 同预算、盲设、路径硬验证、槽位与补偿预设（机制在文档；玩法未开工） |
| 网页/微信包体超限 | 中，未开始 | Compatibility 基线与 Web 导出预设已有；**没有**资源变体、分包或按需模块。2026-08-26 无美术时 Web 合计 38.46 MB、wasm brotli 仍约 7 MB。2026-09-02 第一批 `.glb` 后 Web 合计 46.37 MB、pck 0.46→8.36 MB、wasm brotli 仍 6.77 MB，首屏约 15.1 MB，已超过微信主包 4 MB。2026-09-13 字体入包再 +0.80 MB（常用 3500 字子集，含字表与许可全文）。数字见 [desktop-export-check.md §6](../../docs/runbooks/desktop-export-check.md) |
