...
| `_mcp_game_helper` 写入 `project.godot` 后进入 Headless | 高，已缓解 | 上游导出会剥 autoload，但 MatchServer 跑源码。已提交工程只保留 GUT 与 `authoring_editor`，不含 `godot_ai`；打开编辑器时 Authoring Editor 可自动启用本机插件。插件每次运行都会重新写回，改不了它，因此改为「一条命令还原 + 提交前拦住」：`npm run godot-settings:scrub` / `godot-settings:check`（`tools/godot-project-settings/`），Agent 的 `git commit` / `git add` 由 `tools/shell-guard/` fail closed 拦下。口径见 [CD-51 §7.3](../50-engineering/51-dev-environment.md)。仍不得把 helper 当基础 Autoload 或写入 Git |
