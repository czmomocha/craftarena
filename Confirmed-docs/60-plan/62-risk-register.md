...
| 零延迟锁定网络参数 | **已缓解**（C1 列出；代码常量未改；锁定等协议层 RTT） |
...
| 无网络自动门禁且人工测试无固定频率 | 高，已接受 | 如实标记未保证；协议层 RTT 样本仍空，不得用 ICMP 立项「可玩性」门禁 |
...
| 在零延迟条件下锁定网络参数 | 高，已缓解 | C1 已列出：近端 ICMP 未证伪快照/心跳/插值桩；7 局空转证实 CPU 先于内存。**代码常量未改**。锁定等 [server-deploy.md §13](../../docs/runbooks/server-deploy.md) 协议层 RTT，不得用 ICMP |
