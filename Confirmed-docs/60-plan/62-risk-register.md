...
| `_mcp_game_helper` 写入 `project.godot` 后进入 Headless | 高，已缓解 | 上游导出会剥 autoload，但 MatchServer 跑源码。已提交工程只保留 GUT 与 `authoring_editor`，不含 `godot_ai`；本机启用 MCP 后提交前必须还原 helper。生产级启用已成立，仍不得把 helper 当基础 Autoload 或写入 Git |
