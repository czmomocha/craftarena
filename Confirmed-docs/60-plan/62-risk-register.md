...
| 零延迟锁定网络参数 | **已缓解**（C1 列出；§13 已有一场近端协议层样本；代码常量未改；锁定仍待人类拍板） |
| 扫掠取样无上限 | **已治理**（C5 第 15 章：单次 256，超限拒绝；空间划分仍欠） |
...
| 在零延迟条件下锁定网络参数 | 高，已缓解 | C1 已列出：近端 ICMP 未证伪快照/心跳/插值桩；7 局空转证实 CPU 先于内存。§13 已有一场近端协议层样本（P50=16ms），同样未证伪这些桩。**代码常量未改**。锁定仍待人类拍板，不得用 ICMP 或该场改占位桩 |
| 扫掠取样无上限 | 中，已治理 | C5 第 15 章：`MAX_SWEEP_STEPS = 256`，超限拒绝整段，不粗化密度。`radius=1` 下落一整格不再能把 CI 跑死。残余：`is_pose_blocked` 仍全量扫描；超预算位移会失败而不是变粗 |
