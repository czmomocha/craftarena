...
| 进行中 | M3（表现层增强不阻塞 M3 退出，归 M-Art；深化段已收口的表现不再挡本号）；**M5**（A1–C6 **十一章全交**；退出**待人类签署两份清单**，见本节下一行） |
| M5 退出门禁 | 两份人工清单已落库、**待人类执行与签署**：[网络故障人工检查](../../docs/runbooks/network-fault-check.md)（九项记录表）与 [可玩性签署：TRAPRUSH](../../docs/runbooks/playability-signoff-traprush.md)（八项 + 总结论）。签署前 M5 **不得**被表述为已退出（宪法第十八、二十四条）；签署后由人类授权把本表改成「已退出」并在 [CD-91](../90-reference/91-decision-log.md) 追加一行 |
| 部分交付 | M-Export（C1 三预设；**Web 游玩分发第一刀已交**；Android / iOS 排到一期收尾）；M-Art（C4 契约与第一批 `.glb`；与 F 线重叠项已交） |
| 未开工（顺序，不是冻结） | **字体入包**（M5 退出签署后的下一动；2026-09-12 从一期收尾提前）；**产品 UI 接线**（分三批，见 [§2 M-Art](#m-art表现与美术)）；M6、M7；Android / iOS 烟测（一期收尾）；角色选择界面（紧跟主大厅壳，不提前开工） |
...
已完成的底座（M0–M2）
  ...
  → 内容平台 + 账号/草稿云（M4b）
  → TRAPRUSH 纵向验收（M5）
  → 字体入包（M-Art 剩余项；2026-09-12 从一期收尾提前，因为它是 UI 接线的前置）
...
- 字体入包（2026-09-12 **从一期收尾提前**，排在 M5 退出之后、UI 接线第一批之前，因为它是产品 UI 的前置；口径与选型见 [CD-11 §8.2](../10-product/11-scope-and-platforms.md#82-表现规格d42026-08-26-拍板当前生效值)）；
...
状态：**部分交付**。纠偏冻结令已解除。纠偏 E6（方案 §6，**不是** M5）已于 2026-09-02 签署：好玩。未拍板的皮肤 / 镜头产品值不在深化段自选。字体入包与 UI 接线已于 2026-09-12 排定顺序，不再是无限期推迟项。
...
状态：**章节全交、退出待人类签署**（2026-09-10 A1 开工；**A1 / A2 / A3 / B1 / B2 / C1 / C2 / C3 / C4 / C5 / C6 十一章已交**）。M4b 第 1–5 章收口后本号轮到。章节拆分见 [M5 章节计划](../../docs/plans/m5-traprush-vertical-slice.md)（11 章，实现级计划，不是本文件的替代）：人类当日已拍板章节顺序、音频模块基建、素材格式 OGG、总线与默认音量、音频资产预算（该计划 §5.1；预算数值落 [CD-11 §8.3](../10-product/11-scope-and-platforms.md)）。人类 2026-09-11 拍板 04 = 垂直塔、05 = 双路线竞速（该计划 §5.2 问题 2），并确认本地音频授权可用于正式 OGG 入库（该计划 §5.2 问题 1）。同日拍板 C3 控制面代签与 C4 匹配 `content` 对象（该计划 §5.2 问题 3 / 4；宪法第十八条）。C5 把 `tools/bot-runner/` 建成薄壳（该计划 §5.2 问题 5 的 bot-runner 半边）。C6 落库两份人工清单：[网络故障人工检查](../../docs/runbooks/network-fault-check.md)（工具选型为 2026-09-12 拍板的 clumsy / `tc netem` / `dnctl`，外部工具不入库、不进 CI）与 [可玩性签署：TRAPRUSH](../../docs/runbooks/playability-signoff-traprush.md)。**退出条件差的不是代码，是这两份清单的人类执行与签署**：未签署前不得宣布本号退出（宪法第二十四条）。签署期间**不得**接任何一屏产品 UI——清单描述的是现有自绘大厅。不发明 M8。
