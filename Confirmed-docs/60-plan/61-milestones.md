...
| 已退出 | M0、M1、M2 |
| 进行中 | M3（表现层增强仍冻结，不阻塞 M3 退出） |
| 部分交付 | M-Export（C1 三预设）；M-Art（C4 契约与第一批 `.glb`） |
| 冻结未开工 | M4a、M4b、M6、M7；第 4 张及以后官方课 |
| 纠偏闸门 | C0–C5 **批次已结束**，仍**不是**里程碑号；冻结令解除前优先于本文件进度压力 |
| 解冻后顺序 | M-Export 收尾 → M-Art 剩余 → M4a → M4b → M5 → M6 / M7 |

## 1. 开发顺序

本路线只规定顺序，**不承诺固定周数**。每阶段满足退出条件后才能进入下一阶段；TRAPRUSH 先完成，再复用底座开发 BASTION。纠偏冻结令解除前，可开工范围仍是 C0–C5，见 [纠偏方案](../../docs/plans/course-correction-2026-08.md)。

```text
已完成的底座（M0–M2）
  → 权威联机收尾（M3，不含冻结的表现增强）
  → 纠偏闸门 C0–C5（临时，不是里程碑号）
...
诚实边界：一期最小道具 = **爆破球 + 冲刺**，不是完整道具表；无控保护与产品数值仍属 [CD-63](63-open-decisions.md)。表现层增强（镜头、HUD 字段、皮肤）**不是 M3 退出条件**，解冻后归 M-Art，不阻塞 M3。
...
状态：**部分交付**。纠偏期内不新开导出章；剩余项解冻后开工。

### M-Art：表现与美术

不解冻编号。解冻后、M5 可玩性签署前。C4 已交契约、占位规格与第一批 `.glb`。
...
- 传送门模型；
- clip / 绑定动画；
- 纠偏冻结的镜头、HUD 字段、皮肤、动效（**不阻塞 M3**）。

验收：官方课占用不再依赖无资产回退盒（传送门若仍无模型须诚实标注）；可玩性签署能看见规格内的美术，而不是纯色块。

状态：**部分交付**。纠偏冻结令挡住新的表现增强章。纠偏 E6（方案 §6，**不是** M5）已于 2026-09-02 签署：好玩。不解冻。

### M4a：Rule VM

原 M4 的 L2 半边。纠偏冻结期内**不得开工**。
...
原 M4 的 L6 半边，并挂上 CD-11 §4 无主的账号 / 草稿云 / 内容广场。**不另开里程碑号，不发明 M8。** 纠偏冻结期内**不得开工**。
...
- 3～5 张官方赛道（第 4 张及以后仍受纠偏冻结，解冻后本号收）；
- BotRunner 可达性测试；
- 按需人工网络故障检查；
- 项目负责人可玩性清单；
...
Rule VM 属 **M4a**（从原 M4 拆出的 L2 半边；不在 CD-11 §4 十五条里单独成行）。纠偏冻结期内不得开工。
