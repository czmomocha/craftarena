...
| 进行中 | M3（表现层增强不阻塞 M3 退出，归 M-Art；深化段已收口的表现不再挡本号）；**M4a**（第 1–2 章已交：信封 / 解释器 / gas / `OnMatchStarted` 图编译） |
...
状态：**进行中**（2026-09-09 开工）。第 1 章：v1 信封 + 白名单解释器 + gas。第 2 章：`OnMatchStarted` 图编成同一套字节码。未挂 Preview / 对局 / Event 分发。P3 仍拒绝。不发明 M8。

后续章（顺序，不是新里程碑号）：

3. Event 分发（`OnEveryTicks` 等；在开局 / tick 调已有解释器）
