...
| 部分交付 | M-Export（C1 三预设；**Web 游玩分发第一刀已交**；Android / iOS 排到一期收尾）；M-Art（C4 契约与第一批 `.glb`；与 F 线重叠项已交）；**M4b 第 1–3 章**（ContentHash + HMAC + 发布切 `latest` + 新房吃新版本 + P0/P1 运行房补丁 + 进程内回滚 + latest 指针回滚） |
| 未开工（顺序，不是冻结） | M4b 其余（广场 / 账号）、M6、M7；第 4 张及以后官方课属 M5；Android / iOS 烟测（一期收尾）；角色选择界面（紧跟主大厅壳，不提前开工） |
...
状态：**已收口**（2026-09-09）。第 1–5 章已交：信封 / 解释器 / gas / 图编译 / Event 分发 / §2.1 Query·Logic·Action 最小子集 / Preview P3 安全点重编译、公开对局禁止。其它 §2.1 事件仍编译拒绝。规则图不写入 AuthoringDocument / SimulationBundle。不发明 M8。下一动是 M4b（第 1–3 章已交；其余是广场 / 账号）。
