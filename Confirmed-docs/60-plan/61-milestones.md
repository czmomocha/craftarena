...
| 下一动 | **UI 接线第一批：S3 公共内容广场**（前置已交：字体入包、两个校验脚本进 CI、**S3 + 卡片文案迁 `UiCopy`**，均 2026-09-13。**还剩**：S1 / S2 文案迁移（分属第二 / 三批，不挡第一批）、`ui-wiring.md` 按仓库实际落点重写。「85 处」口径已修正，见 [ui-wiring.md §0.3](../../docs/runbooks/ui-wiring.md)） |
...
- **产品 UI 接线（三批，2026-09-12 拍板）**。UI 基础包已于 2026-09-11 落库（`game/content/ui/theme/`、`game/src/client/ui/`、`docs/runbooks/ui-wiring.md`），当前是纯静态资产，运行时无任何引用，因此接线时机可自由择时。分批如下，**一批一刀，不合并**：
  ...
  | 批次 | 接什么 | 时机 | 前置 |
  |---|---|---|---|
  | 第一批 | S3 公共内容广场，替换 `content_plaza_entry.gd` | M5 退出之后 | ~~字体入包~~、~~两个校验脚本进 CI~~、~~S3 + 卡片文案改走 `UiCopy`~~（均 2026-09-13 交）；`ui-wiring.md` 按仓库实际落点重写。S1 / S2 文案迁移随各自批次，不挡本批 |
