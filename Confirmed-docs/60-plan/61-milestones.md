...
| 解冻后顺序 | 创作者可测（已收口）→ F 线（已收口）→ Web 游玩分发（第一刀已收口）→ 可玩性深化（已收口）→ M-Art 剩余已拍板项 → **M4a** → M4b → M5 → **字体入包 → UI 接线第一批（S3 广场）** → **测试期 VPS Web 分发（M-Export 剩余）** → M6 D1–F1（含 **UI 接线第二批 S1 主大厅壳**）→ **天空盒与多层官方课 A1 → A2 → A3**（插入段，2026-09-17 拍板）→ **角色选择**（已交，2026-09-19）→ **开局倒计时 R1**（已交，2026-09-19）→ **对局记录 / 回放 R2**（已交，2026-09-19）→ **M6 F2** → M7 → **公开 TLS / 每个 PR 的 Web 沙盒** → **UI 接线第三批（S2 / S4 / S5 / S6）** → **一期收尾**（Android / iOS 烟测、触控 UI） |
...
已完成的底座（M0–M2）
  ...
  → BASTION（M6 / M7；主大厅壳双玩法入口随 BASTION 落地，UI 接线第二批 S1 在此章内，不提前）
      ...
      · 插入：TRAPRUSH 开局倒计时（R1，已交，2026-09-19）与对局记录 / 回放（R2，已交，2026-09-19）；排在角色选择之后、F2 之前，不发明新里程碑号
