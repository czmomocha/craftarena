...
> 文档 ID：CD-61
> 单一事实源：开发顺序、M0–M3 / M-Export / M-Art / M4a / M4b / M5–M7 产出与验收、CD-11 §4 所有者、阶段退出条件、首个可运行验收场景
...
| 解冻后顺序 | 创作者可测（已收口）→ **F 线**（插入；含与 M-Art 重叠的 HUD / 动效 / asset 视觉 / 传送门）→ **M-Export 收尾**（安卓 / iOS 烟测）→ M4a → M4b → M5 → 公开 TLS / PR Web 沙盒 → M6 / M7 |
...
已完成的底座（M0–M2）
  ...
  → F 线（插入：趣味性增强 F1–F4；含与 M-Art 重叠的 HUD / 动效 / 按 asset_id 解析 / 传送门；示范课 `course_f_playable` 是 F 线验收对象，**不计入** M5 官方课 3～5 张；不是新里程碑号，不发明 M8）
  → M-Export 收尾（Android / iOS 烟测；公开 TLS 与 PR Web 沙盒推迟到 M5 之后）
  → 表现/美术剩余项（M-Art：字体入包仍推迟到一期收尾；与 F 线重叠项在 F 线做，不重复）
  → Rule VM（M4a）
  → 内容平台 + 账号/草稿云（M4b）
  → TRAPRUSH 纵向验收（M5）
...
诚实边界：一期最小道具 = **爆破球 + 冲刺**，不是完整道具表；无控保护与产品数值仍属 [CD-63](63-open-decisions.md)。表现层增强（镜头、HUD 字段、皮肤）**不是 M3 退出条件**，解冻后归 M-Art / F 线，不阻塞 M3。

### F 线：趣味性增强（插入，不是里程碑号）

人类 2026-09-07 拍板（D-F1）：开 F 线；F1–F4 在 **M4a 之前**；**M-Export 安卓 / iOS 烟测排在 F 线之后**。不发明 M8。决策来源见 [CD-91 D.10](../90-reference/91-decision-log.md) `f_line = playability_insert_before_m4a`。

八章：FA 时间闭环 → FB 音频与预警 → FC 可见世界 → FD 跳跃 → FF 参数面板 → FE 移动平台 → FG 编辑效率 → FH 示范课 `course_f_playable`（**不计入** M5 官方课计数）。FH 之后补可读性：拉开示范课、HUD / 世界标签缩字、斜 45° 滚轮调距。

本刀（FA）已交：大厅 / Solo / Preview 大字局时、过垫分段（客户端记忆，不进 `hash_state`）、完赛冻钟、结算表（Solo 本地 `try_build`，在线仍只 GET）。推击 / 爆破 / 道具重生数值不改（D-F4 / D-F5 / D-F6）。跳跃高度与重力决策已锁、接线属 FD。
...
状态：**部分交付**。人类 2026-09-07 拍板：安卓 / iOS 烟测排到 F 线之后。公开 TLS 与 PR Web 沙盒仍在 M5 之后。不发明 M8。

### M-Art：表现与美术

不解冻编号。解冻后、M5 可玩性签署前。C4 已交契约、占位规格与第一批 `.glb`。与 F 线重叠的 HUD / 动效 / 按 `asset_id` 解析 / 传送门在 **F 线做，本号不重复**。

产出（剩余）：

- 字体入包（**推迟到一期收尾**，不挡本号其它项、不挡 M-Export / M4a）；
- 按 `asset_id` 解析视觉（F 线 FC）；
- 传送门模型（F 线 FC）；
...
- 镜头、HUD 字段、皮肤、动效（**不阻塞 M3**；HUD 计时 / 结算表已由 F 线 FA 接线；皮肤池等 D-F9）。

验收：官方课占用不再依赖无资产回退盒（传送门若仍无模型须诚实标注）；可玩性签署能看见规格内的美术，而不是纯色块。

状态：**部分交付**。纠偏冻结令已解除。纠偏 E6（方案 §6，**不是** M5）已于 2026-09-02 签署：好玩。字体入包除外，与 F 线重叠项跟 F 线走。

### M4a：Rule VM

原 M4 的 L2 半边。解冻后按顺序，**当前不得跳过 F 线 / M-Export 安卓 iOS 烟测抢先开工**。
...
原 M4 的 L6 半边，并挂上 CD-11 §4 无主的账号 / 草稿云 / 内容广场。**不另开里程碑号，不发明 M8。** 解冻后按顺序，当前不得抢先开工。
...
- 3～5 张官方赛道（第 4 张及以后本号收，当前未轮到）；
- BotRunner 可达性测试；
- 按需人工网络故障检查；
- 项目负责人可玩性清单；
- 编辑—预览—邀请—发布—游玩—单局结算闭环（发布依赖 M4b）。

导出归 **M-Export**，不再作为本号退出条件。
...
Rule VM 属 **M4a**（从原 M4 拆出的 L2 半边；不在 CD-11 §4 十五条里单独成行）。解冻后按顺序，当前不得抢先开工。
