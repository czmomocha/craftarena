...
| 部分交付 | M-Export（C1 三预设；**Web 游玩分发第一刀已交**；Android / iOS 排到一期收尾）；M-Art（C4 契约与第一批 `.glb`；与 F 线重叠项已交）；**M4b 第 1–2 章**（ContentHash + HMAC + 发布切 `latest` + 新房吃新版本） |
| 未开工（顺序，不是冻结） | M4b 其余（P0P1 运行房 / 回滚 / 广场 / 账号）、M6、M7；第 4 张及以后官方课属 M5；Android / iOS 烟测（一期收尾）；角色选择界面（紧跟主大厅壳，不提前开工） |
...
状态：**已收口**（2026-09-09）。第 1–5 章已交：信封 / 解释器 / gas / 图编译 / Event 分发 / §2.1 Query·Logic·Action 最小子集 / Preview P3 安全点重编译、公开对局禁止。其它 §2.1 事件仍编译拒绝。规则图不写入 AuthoringDocument / SimulationBundle。不发明 M8。下一动是 M4b（第 1–2 章已交）。
...
状态：**进行中**（2026-09-10）。第 1–2 章已交：`ContentSign` sidecar（ContentHash = StateHasher 规范编码 `SimulationBundle.to_dictionary()` 的 SHA-256；HMAC-SHA256 平台签名；恰好五键信封）+ 控制面发布 HTTP（校验 HMAC，不重算哈希；一事务写入 `content_versions` 并切 `content_latest`）+ Godot `ContentCatalog`（按 `latest` 开新房）。已开对局锁开局哈希；版本不可覆盖、只递增。官方课仍不要求信封。无 P0/P1 运行房补丁、无进程内回滚、无内容广场、无账号认领。不改 Bundle Schema / 匹配 HTTP 课表。不发明 M8。
