...
| 部分交付 | M-Export（C1 三预设；**Web 游玩分发第一刀已交**；**第二刀未开工**；Android / iOS 排到一期收尾）；M-Art（C4 契约与第一批 `.glb`；字体入包与 S3 已交；与 F 线重叠项已交） |
| 下一动 | **M-Export 剩余：测试期 VPS Web 分发**（2026-09-14 拍板）。交付与窗口验收见 [§2 M-Export](#m-export平台导出与-web)。测试期继续明文 `http`/`ws` + 自备 VPS 链接和端口。**S3 已交**（2026-09-13 接线；2026-09-14 列表改走非阻塞 HTTP）。S1 仍在 **M6 主大厅壳章之内**，不提前。S3 三项设计缺口仍待产品输入，见 [ui-wiring.md §2.2](../../docs/runbooks/ui-wiring.md) |
...
已完成的底座（M0–M2）
  ...
  → 测试期 VPS Web 分发（M-Export 剩余 / 第二刀；**下一动**。明文 `http`/`ws` + 自备 VPS；不是公开 TLS，不是每个 PR 的沙盒）
...
- **测试期 VPS Web 分发**（**第二刀，下一动**；2026-09-14 拍板，为验收和测试，不是公开运营）。交付：
  1. **构建脚本**：一条命令把 Godot 导出成 Web 包（`.wasm` + `.js` + `.html`）；
  2. **部署脚本**（可选但推荐）：一条命令用 `scp` 或 `rsync` 传到 VPS 指定目录；
  3. **运行环境**：VPS 上 Nginx 指向该目录（**本刀窗口验收对象**）。第一刀的控制面 `CRAFTARENA_WEB_ROOT` + `/play/` 仍可提，不替代本刀；
  4. **连接逻辑**：Web 游戏启动后**默认**连接这台 VPS 上的控制面与 WebSocket 网关。传输锁的是 WebSocket（[CD-43](../40-technical/43-networking-and-replay.md)），**不引入 WebRTC**。大厅仍可改 `主机[:端口]`。手册只写占位符，不入库真实 IP / 域名 / 端口。
...
状态：**部分交付**。人类 2026-09-08：Web 游玩分发第一刀已交（浏览器 Solo、大厅 `主机[:控制面端口]`、查询串、控制面 CORS 与可选 `/play/`）。人类 2026-09-14：第二刀标准如上，**尚未开工**。安卓 / iOS 烟测排到一期收尾。公开 TLS 与 PR Web 沙盒排到 **M7 之后**。测试期仍明文 `http`/`ws` + 自备 VPS 链接和端口。不发明 M8。
