...
| 已退出 | M0、M1、M2、**M5**（2026-09-13 人类签署，结论「基本通过」，见下「M5 退出记录」） |
...
| 解冻后顺序 | 创作者可测（已收口）→ F 线（已收口）→ Web 游玩分发（第一刀已收口）→ 可玩性深化（已收口）→ M-Art 剩余已拍板项 → **M4a** → M4b → M5 → **字体入包 → UI 接线第一批（S3 广场）** → **测试期 VPS Web 分发（M-Export 剩余）** → M6 D1–F1（含 **UI 接线第二批 S1 主大厅壳**）→ **天空盒与多层官方课 A1 → A2 → A3**（插入段，2026-09-17 拍板）→ **角色选择**（已交，2026-09-19）→ **开局倒计时 R1**（已交，2026-09-19）→ **对局记录 / 回放 R2**（已交，2026-09-19）→ **Solo 本机幽灵 R3**（已交，2026-09-19）→ **M6 F2** → M7 → **公开 TLS / 每个 PR 的 Web 沙盒** → **UI 接线第三批（S2 / S4 / S5 / S6）** → **一期收尾**（Android / iOS 烟测、触控 UI） |
...
已完成的底座（M0–M2）
  ...
  → 表现/美术剩余项（M-Art：与深化段重叠的表现在深化段做；字体入包见下）
  → Rule VM（M4a）
  → 内容平台 + 账号/草稿云（M4b）
  → TRAPRUSH 纵向验收（M5）
  → 字体入包（M-Art 剩余项；2026-09-12 从一期收尾提前，因为它是 UI 接线的前置；**2026-09-13 已交**）
  ...
  → BASTION（M6 / M7；主大厅壳双玩法入口随 BASTION 落地，UI 接线第二批 S1 在此章内，不提前）
      ...
      · 插入：TRAPRUSH 开局倒计时（R1，已交，2026-09-19）与对局记录 / 回放（R2，已交，2026-09-19）与 Solo 本机幽灵（R3，已交，2026-09-19）；排在角色选择之后、F2 之前，不发明新里程碑号
...
- **测试期 VPS Web 分发**（**第二刀，已交**；2026-09-14 拍板并落地，为验收和测试，不是公开运营）。交付：
  1. **构建脚本**：`npm run export:web` 把 Godot 导出成 Web 包（`.wasm` + `.js` + `.html`，另有 `.pck`）；
  2. **部署脚本**：`npm run deploy:web` 用 `scp` 或 `rsync` 传到 VPS 指定目录（`--dry-run` 只打印命令）。手册与环境变量只写占位符，不入库真实 IP / 域名；
  3. **运行环境**：VPS 上 Nginx 指向该目录（**本刀窗口验收对象**，配置在 `infra/nginx/craftarena-web.conf`，步骤在 [server-deploy.md §14](../../docs/runbooks/server-deploy.md#14-测试期-vps-web-分发m-export-第二刀)）。第一刀的控制面 `CRAFTARENA_WEB_ROOT` + `/play/` 仍可提，不替代本刀；
  4. **连接逻辑**：Web 游戏启动后**默认**连接这台 VPS 上的控制面与 WebSocket 网关（页主机填 `--server=`，含 Nginx 的 `/`；页端口不是控制面端口）。传输锁的是 WebSocket（[CD-43](../40-technical/43-networking-and-replay.md)），**不引入 WebRTC**。大厅仍可改 `主机[:端口]`。
- Web 轻量 Edit UI（可玩性深化的编辑轨已交入口、surface 分级与**浏览器真机闭环**；规则模板等 M4a）；
- 公开运营前 TLS / 域名 / 受信证书（**推迟到 M7 之后**，见 [CD-11 §9](../10-product/11-scope-and-platforms.md)）；
- 每个 PR 的公开临时 Web 沙盒（**推迟到 M7 之后**；[CD-53 §4.1](../50-engineering/53-testing-and-ci.md) 仍未实现）；
- Android / iOS 导出烟测（**一期收尾**；不是风险项，不挡可玩性深化 / M4a / M4b / M5 / M6 / M7）；
- Steam 预留（核心切片稳定后接入）。

验收：三套已有预设仍可用。**第二刀窗口**：测试员用浏览器打开 `http://VPS`（测试期验收对象是明文 `http`；`https` 随 M7 之后 TLS 章，不得对外说已有 TLS），能加载**这次导出的**游戏，点现有大厅入口之一（**快速游戏 / 创建房间 / 单人试玩**，不新做「开始」按钮），连上 VPS 上的控制面与网关，并能看到这次包里的最新开发内容。公开 Web 联机走 TLS（**M7 之后**补）；Android / iOS 在一期收尾能装上并进一局。触控 UI 仍属 D7，排在移动端烟测之后、同属一期收尾。

状态：**部分交付**。人类 2026-09-08：Web 游玩分发第一刀已交（浏览器 Solo、大厅 `主机[:控制面端口]`、查询串、控制面 CORS 与可选 `/play/`）。人类 2026-09-14：第二刀已交（`npm run export:web` / `npm run deploy:web`、Nginx 明文 `:80`、Web 页主机默认连本 VPS 的 8080/8090）。安卓 / iOS 烟测排到一期收尾。公开 TLS 与 PR Web 沙盒排到 **M7 之后**。测试期仍明文 `http`/`ws` + 自备 VPS 链接和端口。不发明 M8。
...
- **TRAPRUSH 开局倒计时（R1，已交，2026-09-19）**。Solo 与双人对局开局权威 3.0 s（180 tick）；倒计时拒意图、冻重力 / 周期机关；局时从 `racing_tick` 起算；视口中央 3-2-1-GO。默认 `go_tick = 0` 保旧磁带。Preview / Editor / 地图巡览不做。排在角色选择之后、F2 之前，不发明新里程碑号。
- **TRAPRUSH 对局记录 / 回放（R2，已交，2026-09-19）**。冲线才存。本机环 50 / 每账号服务端环 100（Guest 按 guest id）。命令磁带重仿真，只读，倒计时后重放。不替代 `match_settlements`。不做暂停 / 倍速 / 分享码 / BASTION 回放。排在 R1 之后、F2 之前，不发明新里程碑号。
- **TRAPRUSH Solo 本机幽灵（R3，已交，2026-09-19）**。单人试玩默认追赶本课最快命令磁带；设置可关；破纪录后该次成为下次幽灵。并行会话、无碰撞、不占席。不进协议帧。排在 R2 之后、F2 之前，不发明新里程碑号。F2 重签须一并覆盖幽灵。
...
状态：**部分交付**。纠偏冻结令已解除。纠偏 E6（方案 §6，**不是** M5）已于 2026-09-02 签署：好玩。未拍板的皮肤 / 镜头产品值不在深化段自选。字体入包已于 2026-09-13 交；UI 接线三批已排定顺序，不再是无限期推迟项。剩下的缺口是**只有 Regular 一个字重**与**emoji 不在子集内**，两项都需人类拍板才动。
...
状态：**已退出**（2026-09-13 人类签署，结论「基本通过」；2026-09-10 A1 开工，**A1 / A2 / A3 / B1 / B2 / C1 / C2 / C3 / C4 / C5 / C6 十一章已交**）。M4b 第 1–5 章收口后本号轮到。章节拆分见 [M5 章节计划](../../docs/plans/m5-traprush-vertical-slice.md)（11 章，实现级计划，不是本文件的替代）：人类当日已拍板章节顺序、音频模块基建、素材格式 OGG、总线与默认音量、音频资产预算（该计划 §5.1；预算数值落 [CD-11 §8.3](../10-product/11-scope-and-platforms.md)）。人类 2026-09-11 拍板 04 = 垂直塔、05 = 双路线竞速（该计划 §5.2 问题 2），并确认本地音频授权可用于正式 OGG 入库（该计划 §5.2 问题 1）。同日拍板 C3 控制面代签与 C4 匹配 `content` 对象（该计划 §5.2 问题 3 / 4；宪法第十八条）。C5 把 `tools/bot-runner/` 建成薄壳（该计划 §5.2 问题 5 的 bot-runner 半边）。C6 落库两份人工清单：[网络故障人工检查](../../docs/runbooks/network-fault-check.md)（工具选型为 2026-09-12 拍板的 clumsy / `tc netem` / `dnctl`，外部工具不入库、不进 CI）与 [可玩性签署：TRAPRUSH](../../docs/runbooks/playability-signoff-traprush.md)。**退出依据（2026-09-13）**：五张官方课 `npm run bot-run` 全 `completable`；GUT 1645/1645；可玩性八项均「通过」，总结论 **「基本通过」**；网络故障清单走过第一轮。**退出不等于无缺口**——下列两项随本号带走，**不得**在任何材料中被当作已解决（宪法第二十四条）：

1. 网络故障九项里的**乱序与重复包未严格注入**（macOS `dnctl` 无 reorder / duplicate 开关）。TCP 会重组去重是协议推断，不是本项目测出来的事实；要证据须在 Linux 测试机跑 `tc netem`。
2. 可玩性签署的对象是**当前自绘大厅 + 占位美术 + 占位数值**那一版。UI 接线三批、定稿美术、D-F4 / D-F5 / D-F6 数值一旦变动，该结论不自动延续，须重签。

不发明 M8。退出当时的下一动是字体入包（**已交**）与 UI 接线第一批（**已交**）。现行下一动见文首。
...
公开 TLS 与每个 PR 的 Web 沙盒排在本号之后（2026-09-14），**不是**本号产出。测试期仍明文 `http`/`ws`。
