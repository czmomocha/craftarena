...
| 部分交付 | M-Export（C1 三预设；**Web 游玩分发是下一刀**；Android / iOS 排到一期收尾）；M-Art（C4 契约与第一批 `.glb`；与 F 线重叠项已交） |
| 未开工（顺序，不是冻结） | **Web 游玩分发**；**可玩性深化**（插入）；M4a、M4b、M6、M7；第 4 张及以后官方课属 M5；Android / iOS 烟测（一期收尾） |
| 纠偏闸门 | **已解除**（2026-09-03）。C0–C5 不是里程碑号。此后本文件进度顺序生效 |
| 解冻后顺序 | 创作者可测（已收口）→ F 线（已收口）→ **Web 游玩分发**（第一刀：Web 运行与实现验证）→ **可玩性深化** → M-Art 剩余已拍板项 → M4a → M4b → M5 → 公开 TLS / PR Web 沙盒 → M6 / M7 → **一期收尾**（字体入包、Android / iOS 烟测、触控 UI） |
...
已完成的底座（M0–M2）
  ...
  → Web 游玩分发（M-Export 第一刀：导出 Web 包、浏览器能跑、测试期 Solo、大厅填主机+端口打自备 VPS；不是公开 TLS，不是每个 PR 的沙盒）
  → 可玩性深化（插入：游戏性、第一张 TRAPRUSH 玩法与表现、UGC、编辑调试；不是新里程碑号，不发明 M8；人类宣布收口后才进 M4a）
...
人类 2026-09-07 拍板：F 线之后**首要任务是完善游戏功能**，不是移动端导出。Android / iOS 烟测排到 **一期收尾**；移动端导出不是风险项。下一刀是 Web 游玩分发（见本节 M-Export）。不发明 M8。决策来源见 [CD-91 D.10](../90-reference/91-decision-log.md) `cd61_order = web_first_mobile_phase1_end`。
...
4. **UGC**：验证、草稿、Preview 调试。签名发布属 M4b；Rule VM 属 M4a，本段不抢。
...
- **Web 游玩分发**（**第一刀**）。验证 Web 端能运行、实现与桌面一致的可玩路径：用已有 Web 预设导出；浏览器打开能进一局；**测试期允许 Solo**；联机用大厅填写的**主机 + 端口**打自备 VPS（链接发给外人即可）。不是公开运营 TLS，不是每个 PR 的自动沙盒。明文口径见 [CD-11 §9](../10-product/11-scope-and-platforms.md)；Solo 口径见 [CD-13 §3](../10-product/13-account-and-session.md)。
- Web 轻量 Edit UI（可玩性深化的编辑轨；规则模板等 M4a）；
...
状态：**部分交付**。人类 2026-09-07 拍板：Web 游玩分发为第一刀；安卓 / iOS 烟测排到一期收尾。公开 TLS 与 PR Web 沙盒仍在 M5 之后。不发明 M8。
...
原 M4 的 L2 半边。解冻后按顺序，**当前不得跳过可玩性深化（及 Web 游玩分发）抢先开工**。不再以 Android / iOS 烟测为前置。
...
Rule VM 属 **M4a**（从原 M4 拆出的 L2 半边；不在 CD-11 §4 十五条里单独成行）。解冻后按顺序，可玩性深化收口前不得抢先开工。Android / iOS 不是 M4a 前置。
