...
状态：**进行中**。**D1–D4 已交**（2026-09-15），**D5 起未开工**。章节拆分见 [M6 章节计划](../../docs/plans/m6-bastion-1v1.md)（11 章：D1 契约与蓝图编译 → D2 寻路与封路守卫 → D3 阶段机与波次 → D4 炮塔与经济 → D5 互设障碍 → E1 实时帧 → E2 官方蓝图 → E3 对局进程与匹配 → E4 客户端对局壳 → F1 UI 接线第二批 S1 → F2 退出验收）。那是实现级计划，本节仍是产出与退出条件的所有者。
...
**D1–D4 交了什么、没交什么**：交的是**全离线的玩法底座**——BASTION 蓝图 bundle v1 与编译器、九个原型的白名单、占位数值单一配置源、与玩法无关的确定性边图搜索、「不得完全封路」守卫、五阶段会话与镜像波次与核心伤害与胜负、炮塔与经济（含四类伪造建造的反例）。**没交**：协议帧与 intent 线上 id（E1）、官方蓝图（E2）、对局进程与匹配与结算写库（E3）、客户端对局壳（E4）、互设障碍的点数预算与盲设揭示退点（D5）。所以**现在还开不出一局给人看，第一次有画面是 E4**。数值全部是 `game/src/games/bastion/play_stubs.gd` 的占位桩（[CD-63 §1.2 / §1.3](63-open-decisions.md) 仍延期）。
