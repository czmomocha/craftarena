...
| 下一动 | **UI 接线第一批：S3 公共内容广场——四项前置已全部交付**（2026-09-13：字体入包、两个校验脚本进 CI、S3 + 卡片文案迁 `UiCopy`、`ui-wiring.md` 重写）。本批要做的是用 `s3_workshop.tscn` 替换 `content_plaza_entry.gd`（338 行真实 HTTP / tab / 选中态要搬进新壳，不是套皮），落点与现状见 [ui-wiring.md §2](../../docs/runbooks/ui-wiring.md)。S1 / S2 文案迁移随第二 / 三批，不挡本批 |
...
已完成的底座（M0–M2）
  ...
  → UI 接线第一批：S3 公共内容广场（分批表见 §2 M-Art）
  → 公开 TLS / 每个 PR 的 Web 沙盒（仍属 M-Export 产出，2026-09-03 拍板推迟）
  → BASTION（M6 / M7；主大厅壳双玩法入口随 BASTION 落地，UI 接线第二批 S1 在此章内）
...
- **产品 UI 接线（三批，2026-09-12 拍板）**。UI 基础包已于 2026-09-11 落库（`game/content/ui/theme/`、`game/src/client/ui/`、`docs/runbooks/ui-wiring.md`），当前是纯静态资产，运行时无任何引用，因此接线时机可自由择时。分批如下，**一批一刀，不合并**：
  ...
  | 批次 | 接什么 | 时机 | 前置 |
  |---|---|---|---|
  | 第一批 | S3 公共内容广场，替换 `content_plaza_entry.gd` | M5 退出之后 | **四项前置全部已交（2026-09-13）**：字体入包、两个校验脚本进 CI、S3 + 卡片文案改走 `UiCopy`、`ui-wiring.md` 重写。S1 / S2 文案迁移随各自批次，不挡本批 |
