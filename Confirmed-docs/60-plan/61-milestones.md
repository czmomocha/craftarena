...
| 进行中 | M3（表现层增强不阻塞 M3 退出，归 M-Art；深化段已收口的表现不再挡本号）；**M4a**（首章已交：v1 信封 + 白名单 + gas） |
...
状态：**进行中**（2026-09-09 开工）。首章已交：`ruleset_version=1` 版本化信封 + 白名单 `HALT` / `NOP` / `LOAD_I64` / `GET_VAR` / `COMPARE` / `SET_VAR` + 单图 / tick / chain gas。超 gas 中止该次解释、不提交槽位。未挂 Preview / 对局 / 规则图编译。P3 仍拒绝。不发明 M8。
