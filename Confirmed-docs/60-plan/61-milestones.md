...
| 下一动 | **UI 接线第二批：S1 主大厅**（在 M6 主大厅壳章之内，随 BASTION 双玩法入口一起落，不提前）。**第一批已交**（2026-09-13：四项前置 + S3 接入运行时）。第一批留下三项需要**产品 / 设计输入**才能推进的缺口，见 [ui-wiring.md §2.2](../../docs/runbooks/ui-wiring.md)：Solo / 建房在设计里没有位置（当前是临时动作行）、排序 / 标签 / 搜索无后端（当前禁用）、卡片的作者与缩略图在契约里不存在（当前隐藏）|
