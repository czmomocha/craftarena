...
- `gateway_proxy = ticket_verdict_upstream_passthrough`（2026-08-24）：实时网关从 M0 握手骨架升级为真代理。票据裁决（`TicketVerifier.verify`）必须携带上游对局地址——网关不记账、不查库（宪法第二十一条），票据→对局的解析权在控制面。升级后连接与上游一对一绑定，双向原样转发（二进制/文本标志保留），任一侧关闭/出错即关闭另一侧；裁决无上游或上游不可达一律 502。开发期 `DevTicketVerifier` 以 `GATEWAY_DEV_UPSTREAM` 为固定上游（所有非空票据同一场），真票据签发/校验的控制面接口仍待。M0 占位的 `gateway_hello`/`gateway_not_implemented` 退役：通道即上游协议本身。
