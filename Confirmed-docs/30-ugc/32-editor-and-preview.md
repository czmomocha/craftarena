...
- **Preview 窗口**：`AuthoringPreviewShell` 用代码创建独立 Godot `Window`（非 exclusive、非 transient，不挡住编辑会话）。关闭只隐藏，Preview 会话仍连接。状态标签反映 `connected` / `preview_revision` / `entity_count` / `needs_restart` / `playing` / `pads` / `floor` / `finish` / `crates` / `hazards` / `reach_ok` / 问题条数。Play / Stop / Reset / Use item / Jump / Advance tick 与 WASD MoveIntent、R 重置、`use_item` 打箱、检查点占用、传送占用、冲线占用、周期机关固体切换见上一条「Preview 试玩」。浏览器 `tab` 名已保留，本刀 `open_from` 拒绝。不改 `main.tscn`。多人试玩仍待。落点见 [CD-42 §3.4](../40-technical/42-contracts-and-rulevm.md#34-实现落点)。
