...
- **第一张官方赛道**：`game/content/official/traprush/course_01.json` 是一份发布检查通过的 AuthoringDocument：同层检查点加跨层 `two_way` 传送对（纯竖直，z=0），上层另有 `zone.tags` 含 `finish` 的终点占用实体，起点 +Z 侧向另有 `destructible` 箱，起点 −Z 两格有周期机关、−X 一格有始终固体。由 `AuthoringDocument.load_from_path` 读入，编辑外壳 `import_document` 后验证器空列表。不是新 `op`，不是写入门禁。F6 沙箱 `res://src/creator/course_sandbox.tscn` 只供目视。`editor_sandbox.tscn` 仍种悬空传送，供验证器目视。
...
- **第二张官方赛道**：`game/content/official/traprush/course_02.json` 是一份发布检查通过的 AuthoringDocument：同层检查点加跨层且侧向（+Z）的 `two_way` 传送对，上层另有 `zone.tags` 含 `finish` 的终点占用实体，起点 +Z 侧向另有 `destructible` 箱，起点 −Z 两格有周期机关、−X 一格有始终固体。与第一张布局不同。由 `AuthoringDocument.load_from_path` 读入，编辑外壳 `import_document` 后验证器空列表。不是新 `op`，不是写入门禁。F6 沙箱 `res://src/creator/course_02_sandbox.tscn` 只供目视。
- **第三张官方赛道**：`game/content/official/traprush/course_03.json` 是一份发布检查通过的 AuthoringDocument：地面必经走道上摆 `destructible` 箱（耐久 1，不破不过），`one_way` 传送单向上楼，上层另有一对 `two_way` 传送与四个有序检查点，终点占用在上层 +Z，起点 −Z 两格有周期机关、−X 一格有始终固体。与前两张布局不同，是首张带 `one_way` 连线的官方赛道。由 `AuthoringDocument.load_from_path` 读入，编辑外壳 `import_document` 后验证器空列表。不是新 `op`，不是写入门禁。F6 沙箱 `res://src/creator/course_03_sandbox.tscn` 只供目视。
...
- **Preview 窗口**：`AuthoringPreviewShell` 用代码创建独立 Godot `Window`（非 exclusive、非 transient，不挡住编辑会话）。关闭只隐藏，Preview 会话仍连接。再点 Editor 的 Preview：若仍在跟随则把已有窗 `visible=true` 并 `grab_focus` 抬到前台（内嵌最大化时点到 Editor 其它区域会把 Preview 压到后面，只设 visible 抬不起来）；若原生窗实例已释放则重建，会话仍连接。跟随已掉时仍走 `open_from` 重连。默认尺寸 1280×720 并最大化（开发机运行尺寸，不是产品镜头/FOV）；Play 等按钮 `FOCUS_NONE`，空格走 `jump` 而不是点 Play。状态标签反映 `connected` / `preview_revision` / `entity_count` / `needs_restart` / `playing` / `pads` / `floor` / `finish` / `crates` / `hazards` / `solids` / `reach_ok` / 问题条数。Play / Stop / Reset / Use item / Jump / Advance tick 与 WASD MoveIntent、R 重置、`use_item` 打箱、检查点占用、传送占用、冲线占用、周期机关固体切换见上一条「Preview 试玩」。浏览器 `tab` 名已保留，本刀 `open_from` 拒绝。不改 `main.tscn`。多人试玩仍待。落点见 [CD-42 §3.4](../40-technical/42-contracts-and-rulevm.md#34-实现落点)。
...
当前数据落点是 `AuthoringPreview`；窗口落点是 `AuthoringPreviewShell`；3D 表现、传送连线、检查点顺序、可达性叠加与试玩玩家标记的 gizmos 落点是 `AuthoringPreviewMap`（[CD-42 §3.4](../40-technical/42-contracts-and-rulevm.md#34-实现落点)）；把编辑写入转发进来的是 `AuthoringEditorShell`（见 [§3](#3-从编辑到预览)「编辑写入自动进 Preview」）；把拓扑编译接到试玩的是 `AuthoringPreview.try_start_play`，把 WASD / MoveIntent 接到胶囊的是 `try_apply_play_intent`，把检查点占用接到进度的是 `try_accept_play_checkpoint` / 重叠扫垫，把传送占用接到单跳落地的是 `try_land_exit` / 重叠扫门，把冲线占用接到 `finish_tick` 的是 `try_cross_play_finish` / 重叠扫终点，把重置接到最近检查点落点的是 `ResetToCheckpointIntent` / `TraprushCheckpointSpawn`，把 UseItem 接到可破坏占用的是 `TraprushDestructibleBreak`，把 Jump 接到接地跳跃的是 `JumpIntent` / `TraprushIntentStepper`，把 Advance tick 接到周期机关固体切换的是 `try_advance_play` / `TraprushHazardCycle`（见 [§3](#3-从编辑到预览)「Preview 试玩」）。独立会话、安全点增量 Patch、失败整份回滚、无结算写。桌面打开独立 `Window`；关闭只隐藏。窗口内按 Preview 世界重建 1 米占位盒、`portal_link` gizmos、检查点 `order` 标记和发布前问题码叠加；Play 时另画玩家表现桩，开玩且窗口可见时 WASD 移动该桩，重叠检查点垫推进 `pads=n/m`，重叠传送落地后 `floor=n`，重叠终点盒后 `finish=n`，Use item / `use_item` 打开侧向箱后 `crates=n/m`；开玩后 `solids=n/m` 为编译始终固体袋数；Reset / R 把表现桩送回最近已验收检查点落点；出界同样把表现桩送回最近检查点落点；Jump / `jump` 在接地时把表现桩上移 `play_jump_dy`；Advance tick 按 `cooldown_ticks` 切换 hazard 固体（意图不切换）。浏览器标签与多人试玩房仍待。v1 袋到补丁等级：
