...
| 官方课 | 6 张。04 = 垂直塔（电梯 + 弹射 + 压板，考验上下层）。05 = 双路线竞速（安全长路 vs 需打碎能量墙的短路；站墙 −Z 侧 Q）。06 = 七层多层（层距 2 格，底层 13×13 满铺；爬升三对双向传送门 + 三座电梯补被跳过的层；顶层两层无弹射垫——`LAUNCH_DY` 峰值约 4 格会冲出 ±8）。弹射垫在层距 2 格时无法落在同 xz 的上层地板（峰值约 4.25 格，目的盒在上升途中被从下方顶到）。F 线示范课 `course_f_playable` **不计入**该计数。revision 12：危险捷径 +X 只挡能量墙（站其 −Z 侧 Q 打碎），周期机关在 −Z 侧廊；安全路 +Z。官方 01–03 侧廊已接深化段新机关，不挡既有捷径 / 安全路脚本 |
| 描边 | 不做（D8） |
...
| HUD 计时 | F 线 FA：局时 = `racing_tick/60`（世界 tick 减去开局 `go_tick`）；过垫分段为客户端记忆，不进权威；冲线冻钟。结算表画已有 `rows[]` / `mvp_slot`，贴在窗口右上。字号从 `placeholder_spec` 读。Solo 本地 `try_build`，在线仍只 GET |
| 开局倒计时 | **3.0 s**（180 tick @ 60 Hz，`TraprushPlayStubs.COUNTDOWN_MS`）。权威 `go_tick`；倒计时拒全部意图、冻重力与周期机关。双人未满员不推进 tick。Preview / Editor 无倒计时。地图巡览未实现 |
| 对局记录 / 回放 | **已交**（2026-09-19，R2）。全员冲线才存。本机环 50 / 每账号 100。命令磁带重仿真，不是录像。角色网格不进磁带。条数环见 [CD-14](../10-product/14-data-and-telemetry.md) |
...
| 地图巡览 | 创作者白名单参数 | 展示主路线、终点或自定义目标、关键传送节点（**未实现**） |
| 开局倒计时 | 系统控制，**3.0 s** | 不可移动；重力与周期机关冻结；双人未满员停在出生点。视口中央 3-2-1-GO |
| 对局记录 / 回放 | 冲线后才存 | 本机 50 / 每账号 100。点开后同一套倒计时，只读重放，醒目「回放中」，只能退出 |
| Solo 本机幽灵 | 有本课最快磁带且设置开 | 并行会话重仿真，无碰撞、不占席。破纪录后替换。回放局不套 |
