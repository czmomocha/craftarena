...
| 官方课 | 5 张。04 = 垂直塔（电梯 + 弹射 + 压板，考验上下层）。05 = 双路线竞速（安全长路 vs 需打碎能量墙的短路；站墙 −Z 侧 Q）。F 线示范课 `course_f_playable` **不计入**该计数。revision 12：危险捷径 +X 只挡能量墙（站其 −Z 侧 Q 打碎），周期机关在 −Z 侧廊；安全路 +Z。官方 01–03 侧廊已接深化段新机关，不挡既有捷径 / 安全路脚本 |
| 描边 | 不做（D8） |
