...
| 描边 | 不做（D8） |
| 传送带 | 可玩性深化已接：固体 + `zone.tags` 含 `conveyor` + `transform.yaw_bam`（四向）。**不新增组件**。占位步长是走路占位步长的两倍 ⇒ 逆行走不回去。数值仍属 [CD-63 §1.3](../60-plan/63-open-decisions.md) |
...
| 跳跃 / 重力 | **已接线**（D-F2 / D-F3 / F 线 FD）：`JUMP_DY = SCALE*5/4`，`FALL_DY = -JUMP_DY`，Preview Advance 与对局同一加速度 |
...
当前实现：`hazards` 袋按 `cooldown_ticks` 半周期切固体；`solids` 袋始终固体；官方三张课各 1 个机关、出生点 −X 一格固体、出生点正下一格立足固体。编辑器 Place solid / hazard / crate / finish / mover / conveyor 走已有 `place`。形状见 [CD-32 §3](../30-ugc/32-editor-and-preview.md) 与 [CD-42 §3.4](../40-technical/42-contracts-and-rulevm.md)。口径见文首。

「触发型障碍」这一行**只交付了移动平台与传送带**两件：
...
**门 / 开关链仍未做。** `interactable` 组件与 `InteractIntent` 早已在 Schema 与意图白名单里，但把它们接成可开合的门需要两处协议不兼容变更——命令帧要新增 intent id，快照帧要新增门的开合状态（否则线上会画出与权威相反的门）。两者都属宪法第十八条人类门禁，另起一刀。
