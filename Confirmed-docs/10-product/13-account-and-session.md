...
实现落点（2026-09-02）：离线横幅走键 `craft_arena.ui.offline_banner`。`zh_CN` 仍是「离线试玩，成绩不上传」（本节原文）；`en` 为 `Offline play, scores are not uploaded`。大厅 HUD 写解析后的句子，不再把中文写进 `MatchOfflineSession`。字体未入包。落点见 [CD-11 §8](11-scope-and-platforms.md#8-产品表现与设备基线) 与 `game/src/shared/ui_copy.gd`。
