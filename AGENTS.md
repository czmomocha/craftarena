...
→ 运行自动化测试
→ 运行场景 / Headless 验证
→ 将本章开发机窗口步骤写入 docs/runbooks/dev-window-check.md 本刀（见 CD-52 §3.2）
→ 检查日志与性能
→ 人类审查 / 真机验证
...
不得用"代码看起来正确"代替运行证据。完整分工、任务单模板、人类[开发机窗口验收](docs/runbooks/dev-window-check.md)步骤义务与提交边界见 [CD-52](Confirmed-docs/50-engineering/52-ai-workflow.md)。
...
提交落地（默认 `main`、PR 为例外）见 [CD-52 §1.1](Confirmed-docs/50-engineering/52-ai-workflow.md)，本条不复述。
...
- 默认在 `main` 上提交并推送；人类说「提交」即包含 push，只有明确说不要 push 才不推。只有人类主动要求时才开 PR。提交须本回合人类授权（宪法第十八条）。走 PR 时合并必须 CI 全绿并至少获得一次人类批准，AI 审查不能替代人类。
