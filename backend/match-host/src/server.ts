...
export function buildMatchHost(options: BuildMatchHostOptions): FastifyInstance {
	...
	// POST /matches 接受可选 `{ course, seats }` 或 `{ content, seats }` JSON；空 body 视为默认官方赛道与默认人数。
	...
	app.post("/matches", async (request, reply) => {
		...
		const matchResult = readMatchBody(request.body);
		if (!matchResult.ok) {
			reply.code(400);
...
function toWire(record: MatchRecord): Record<string, unknown> {
	...
	if (record.content !== undefined && record.contentHash !== undefined) {
		...
		body.content_hash = record.contentHash;
	}
	return body;
}
