...
/**
 * 与 `tools/test-selector/src/godot.ts` / MatchHost 同一套策略：Windows 上优先
 * `GODOT4_CONSOLE`（GUI 子系统的 `godot.exe` 不向父进程管道写 stdout，JSON 行
 * 会整片消失），其余平台用 `GODOT4`；**没有 PATH 回退**。
 */
export function resolveGodotExecutable(
	...
	if (executable === undefined) {
		...
		throw new Error(
			`${variables} must point at the Godot engine executable; bot-runner does not fall ` +
...
export function botRunGodotArgs(userArgs: readonly string[]): string[] {
	const forwarded = userArgs.filter((arg) => arg !== "--bot-run");
	return ["--headless", "--path", "game", "--", "--bot-run", ...forwarded];
}
