...
func test_try_apply_fall_zero_keeps_pose_tick_and_snapshots() -> void:
	var course: GrayboxCourse = GrayboxCourse.assemble(_valid_layout())
	...
	assert_eq(course.world.tick_index, 0)
	assert_eq(course.tape.size(), 1)
	_assert_system_command(course.tape.command_at(0), SystemOps.APPLY_FALL)
...
func test_try_apply_fall_open_space_drops_y_without_tick_or_snapshot() -> void:
	var course: GrayboxCourse = GrayboxCourse.assemble(_valid_layout())
...
func test_two_courses_same_fall_match_hash_state() -> void:
	...
	_assert_system_command(left.tape.command_at(0), SystemOps.APPLY_FALL)
	_assert_system_command(right.tape.command_at(0), SystemOps.APPLY_FALL)
	assert_eq(left.tape.hash_tape().hex_encode(), right.tape.hash_tape().hex_encode())
...
func test_tape_replay_apply_fall_matches_source_hashes() -> void:
	var source: GrayboxCourse = GrayboxCourse.assemble(_valid_layout())
	var fall_dy: int = -_whole(2)
	assert_true(source.try_apply_fall(fall_dy))
	assert_eq(source.tape.size(), 1)
	_assert_system_command(source.tape.command_at(0), SystemOps.APPLY_FALL)
	var logged_fall: int = source.tape.command_at(0).payload.get("fall_dy", 0)
	assert_eq(logged_fall, fall_dy)
	assert_eq(source.world.tick_index, 0)
	var dest: GrayboxCourse = GrayboxCourse.assemble(_valid_layout())
