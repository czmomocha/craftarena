...
func test_shell_use_item_opens_crate_and_hidden_window_does_not_sample() -> void:
	...
	_preview_shell.view.rebuild(_preview_shell)
	var crate_vis: MeshInstance3D = _preview_shell.view.map.placeholder_node(CRATE_ID)
	assert_not_null(crate_vis)
	assert_false(crate_vis.visible)
