...
func test_open_window_quick_play_ready_begins_play() -> void:
	...
	assert_eq(_shell.solids.solid_total(), 41)
	...
	assert_true(_shell.status_label_text().contains("solids_mapped=41"))
...
func test_solo_own_progress_tints_pads_and_cancel_restores() -> void:
	...
	assert_true(_shell.status_label_text().contains("solids=41/41"))
