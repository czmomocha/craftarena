...
func test_play_blocks_patches_until_stop() -> void:
	...
	assert_true(preview.try_start_play(1))
	assert_false(preview.try_start_play(1))
	assert_false(preview.try_apply_patch(Levels.P2, _edit(1, preview.world.revision, _place_portal(50, 51, CELL))))
	...
	assert_true(preview.try_apply_patch(Levels.P2, _edit(1, preview.world.revision, _place_portal(50, 51, CELL))))
