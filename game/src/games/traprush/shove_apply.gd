...
## dx/dz 只来自参数，不从 payload 读取 impulse/dx/dz，不发明默认位移。
## 位移走 try_move_xz_until_blocked：停在最后未阻挡 XZ 样本，剩余位移丢弃，非贴墙滑行。
## 门闩通过后仍 {ok: true, shoved: true}，不因接触或整段扫掠失败改为 false。不调用 try_move_xz。
## 不调用 world.tick()。
...
static func apply(
	...
	world.try_move_xz_until_blocked(target_id, dx, dz)
