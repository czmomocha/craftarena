...
**扫掠代价**：`SimulationWorldMove.sweep_step_count(length, radius)` 用 `radius` 当采样步长。半径越小，单次移动的采样次数越多。2026-08-29 拍板时**没有上限**（宪法第十七条缺口）。2026-09-02 C5 第 15 章补上 `MAX_SWEEP_STEPS = 256`：超限拒绝该次位移，不粗化取样。创作者自填小半径仍会把合法位移打成预算失败，所以 Q5「不能自填」仍然成立。
...
- 扫掠采样代价上限本身：本 ADR 只通过"创作者不能自填尺寸"避免内容放大它。上限已由 C5 第 15 章落地（`MAX_SWEEP_STEPS = 256`，超限拒绝），见 [CD-42 §1.1](../../Confirmed-docs/40-technical/42-contracts-and-rulevm.md)。
...
| 13 | 换一次地块视觉，同一条 `latest` 语义再走一遍（2026-09-01） | 已做：`floor_tile.glb`（扁板 1.842 × 0.338 × 1.843，厚宽比 0.184）→ `block_static.glb`（正方块 0.768 × 0.769 × 0.768，厚宽比 1.001）。**贴合规则一个字没改**——换它修的是资产自己的比例，不是规则：等比缩放 + 顶面对齐占位盒顶面这套规则下，扁板贴合后是一片 0.184 格厚的板**悬在格子顶部**（底下 0.816 格是空的），而占位盒本体已 `layers = 0` 退出渲染，所以那片板就是玩家看到的全部；正方块贴合后悬空 0.000 格。`--bot-run` 三张课步数与动作序列逐项不变，证明换视觉没碰裁决。**顺带补上一个此前无人覆盖的测试缺口**：原有用例的前提写的是"这块砖本来就不是一格宽"，只验过**缩小**方向；新块 0.768 m 走的是**放大**方向，才发现那半边从没被测过。已改为按资产自适应，两个方向都覆盖 |
| 14 | 按袋类型接占用视觉（2026-09-02） | 已做：5 个已烘焙 `.glb` 入库并接到大厅 / Preview。垫走顶面对齐，门 / 终点 / 箱 / 滚柱走脚底对齐。箱与滚柱保留 D4 橙 / 洋红 overlay（人类拍板）。传送门仍是色块。按 `asset_id` 解析仍未做。官方课 JSON 与占用半长未改 |
...
- **扫掠采样代价上限已于 2026-09-02 落地**（C5 第 15 章，`MAX_SWEEP_STEPS = 256`，超限拒绝）。本 ADR 当时只堵了「创作者自填尺寸」这一面。**静态盒均匀格阔相已于 2026-09-02 落地**（C5 第 16 章）；胶囊对胶囊仍线性；
- **`asset_id → 视觉` 的按资产解析仍未实装**：一期唯一内置资产被 7 类袋共用，按 `asset_id` 接视觉会让终点、检查点垫与箱子共用同一个模型。C4 第 5 章起接的是**袋类型**（玩法语义，不是资产身份）。2026-09-02 已把垫 / 门 / 终点 / 箱 / 滚柱接到袋类型上；传送门仍无模型。等内容能区分资产后另开一章，届时加的是一张表，不改调用方。理由写在 `game/src/shared/visual_asset_catalog.gd` 文件头。
