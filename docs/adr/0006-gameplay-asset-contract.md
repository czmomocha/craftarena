...
- **`asset_id → 视觉` 的按资产解析仍未实装**：一期唯一内置资产被 7 类袋共用（含官方赛道的路面地板），按 `asset_id` 接视觉会让地板、检查点垫与箱子共用同一个模型。等内容能区分资产后另开一章，届时加的是一张表，不改调用方。理由写在 `game/src/shared/visual_asset_catalog.gd` 文件头。

### 2026-08-30 更新（C4 第 5 章）：视觉与碰撞的数值差异已经成立

本 ADR 原先只能声明"结构已分离、数值今天仍相等，不要对外表述为已经用上了不同的视觉与碰撞"。第一个 `.glb` 入库后这条边界**已经解除**，但只在角色这一条线上：

| | 值 | 谁决定 |
|---|---|---|
| 视觉网格 | 0.74 × 1.03 × 0.61 m | `SharedVisualAssetCatalog`，`latest`，改了不产生内容版本 |
| 权威胶囊 | 半径 / 高各 0.125 格 | `PlaceholderSpec`（D4 占位常量，仍不是资产） |
| 占位盒 | 1 米立方 | `PlaceholderSpec.BOX_SIZE`，视觉解析失败时的回退 |

三者互不相等，且有 GUT 用例（`game/tests/unit/test_character_visual_asset.gd`）断言这个不等关系，以及断言视觉**没有**进 `assets` 袋（键数仍为 4）。赛道实体的视觉仍是占位盒，原因见上一条。
