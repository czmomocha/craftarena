...
> **后续更正（2026-08-28）**：本文件 §1 末尾与 §3 末尾的「合入章的自动门禁通过率是 100%」在写下时（基准 `ce95ff6`）成立，但从 PR #176 起不再成立——CI 的 Godot job 连续被 15 分钟超时终止，五个 PR 带着未通过的门禁合入。见 [门禁失守记录](2026-08-28-ci-gate-timeout.md)。本文件的历史数字按原样保留，不追改。
