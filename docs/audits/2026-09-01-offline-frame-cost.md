...
- **扫掠取样代价无上限**（本章写下时的宪法第十七条缺口）：步数正比于 `|dy| / radius`。本刀只把每步的常数压下去了，没有给步数封顶。**2026-09-02 C5 第 15 章已封顶**：单次最多 256 样本，超限拒绝，不粗化密度。长时间自由下落仍可能在单 tick 位移涨过预算后停住（对局出界桩会先复位）；空间划分仍未做。
