...
- **扫掠取样代价无上限**（本章写下时的宪法第十七条缺口）：步数正比于 `|dy| / radius`。本刀只把每步的常数压下去了，没有给步数封顶。**2026-09-02 C5 第 15 章已封顶**：单次最多 256 样本，超限拒绝，不粗化密度。长时间自由下落仍可能在单 tick 位移涨过预算后停住（对局出界桩会先复位）。**2026-09-02 C5 第 16 章已给静态盒加均匀格阔相**；胶囊对胶囊仍线性。
- **无空间划分**：本章写下时 `is_pose_blocked` 仍是 O(静态盒数) 全量扫描。**2026-09-02 C5 第 16 章已给静态盒加均匀格阔相**；胶囊对胶囊仍全量。49 个盒子当时只要 0.97 ms。
