...
## 本刀：M4b 第 3 章（P0/P1 运行房补丁 + 回滚）

本章无开发机可见行为。补丁信封、全量下发、安全边界应用、进程内反向 Patch 与 `latest` 指针回滚是库级 / 控制面 API；官方课 / 大厅 / Solo / Preview 窗口没有新控件，匹配 HTTP 课表不变。HUD 不显示 PatchHash。
...
- 官方课必须带签名才能开局（仍不要求信封）；
- 匹配 HTTP 课表（仍只 01–03）；
- 控制面结算 HTTP 增加哈希字段（Godot payload 已带，MatchHost 只转发已有键）；
- 规则图编辑器 UI。

---

## 上一刀（已合入）：M4b 第 2 章（发布管线）

本章无开发机可见行为。`latest` 与发布 HTTP 是库级 / 控制面 API；官方课 / 大厅 / Solo / Preview 窗口没有新控件，匹配 HTTP 课表不变。新房吃新版本，已开对局锁开局哈希，HUD 不显示。
...
## 上一刀（已合入）：M4a 第 4–5 章（Query/Action 子集 + Preview P3）
...
- Preview P3 / 规则图编辑器 UI / 规则模板；
- 其余 §2.1 事件与 Query / Action 节点；
- 对局或 Solo 里出现可见规则效果（官方课仍无图）；
- 匹配 HTTP 课表（仍只 01–03）。

---

## 上一刀（已合入）：M4a 第 2 章（规则图编译器）

本章无开发机可见行为。编译器把 `OnMatchStarted` 图编成已有 v1 字节码，未挂大厅、Preview 或对局，窗口里看不到新控件。

### 本刀不测

- Preview P3 / 规则图编辑器 UI / 规则模板；
- Event 分发（`OnEveryTicks` 等）；
- 对局或 Solo 里跑规则；
- 匹配 HTTP 课表（仍只 01–03）。

---

## 上一刀（已合入）：M4a 首章（Rule VM 字节码 + gas）

本章无开发机可见行为。首章是纯库（编码 / 解码 / 解释），未挂大厅、Preview 或对局，窗口里看不到新控件。

### 本刀不测

- Preview P3 / 规则图编辑 / 规则模板；
- 对局或 Solo 里跑规则；
- Event 分发与空间查询；
- 匹配 HTTP 课表（仍只 01–03）。
...
## 上一刀（已合入）：可玩性深化收口（进度回写）

本章无开发机可见行为。收口只改 CD-61 / CD-91 / README / 窗口清单的阶段口径，不改 GUI、不改仿真。

### 本刀不测

- 第十八批窗口步骤（人类已验证、已合入）；
- Rule VM / M4a（下一动，该刀未开工）。

---

## 上一刀（已合入）：可玩性深化（示范课可读收口 + 官方侧廊机关）

**不需要三后端**（全部走 Solo）。

这刀验两件事：`course_f_playable` revision 12 捷径只挡能量墙，站其 −Z 侧 Q 后沿 +X 走完，路上不再空等喷火 / 滚柱；官方 01–03 侧廊能看见新机关，但既有捷径 / 破箱上楼仍在。
...
2. 从出生点朝 **+X**（D）走一格。
   - 预期：第一格是半透能量墙，过不去。墙在主路上。
   - 失败：能一路 +X 冲到终点、路上没有墙；或墙在侧廊、主路空着。
3. 按 **S** 走到墙南侧（墙的 −Z 一格），面向 +Z 按 **Q** 打碎，再按 **W** 回到主路。
   - 预期：墙碎。Q 必须站在墙南侧才打得到（reach 只探眼前 +Z）。站在出生点按 Q 打不碎。
   - 失败：站在墙西侧（出生点）按 Q 也能打碎；或碎不了。
4. 打碎后沿 +X 走完。喷火 / 滚柱 / 摆锤 / 压板在 **−Z 侧廊**，主路上不必空等半周期。
   - 预期：能冲线；这条比安全路短。
   - 失败：打碎后主路仍被周期机关永远挡死；或必须站着空等才能过。
5. 点 **取消**。

### B. 示范课安全路（+Z）

6. 再点 **单人试玩**。从出生点按 **W** 走 **+Z 两格**，再沿 +X 绕。
   - 预期：更长的安全路：开关门、冰面。电梯在更外侧 +Z 三格。必须在 x=3 下到 z=0 收检查点 1，再回到 z=+2，最后从终点北侧走进终点。
   - 失败：+Z 仍是不挡冲线的展览廊；或不经过检查点 1 也能冲线；或安全路比主路还短。
7. 点 **取消**。

### C. 官方 01–03 侧廊机关不挡旧脚本

8. 课程 id 改回 `course_01`，点 **单人试玩**。按 **D** 连走五格。
   - 预期：五步冲线仍在。安全路外侧 +Z（大约 z=+6）能看见冰 / 传送带 / 开关门，不挡这条捷径。
   - 失败：+X 被新机关挡住；或走 +Z 一格（x=1）不再掉进坑。
9. 点 **取消**。课程 id 改成 `course_02`，再 **单人试玩**。按 **D** 走 +X。
   - 预期：+X 捷径仍在。出生点 −Z 一格侧廊有喷火 / 滚柱 / 地刺。
   - 失败：+X 被新机关挡住；或出生点 Q 清不掉原来那只箱。
10. 点 **取消**。课程 id 改成 `course_03`，再 **单人试玩**。按 **Q** 破箱，再按 **D** 上楼冲线。
    - 预期：破箱上楼仍在。侧向能看见弹射垫 / 电梯 / 压板 / 摆锤 / 开关传送，不挡这条脚本。
    - 失败：破箱后过不去；或侧廊机关挡在必经格上。
11. 点 **取消**。
...
- `InteractIntent` / 协议帧；
- `--bot-run` 的完整 A*（示范课走脚本）；
- 规则模板 / M4a；能量墙 / 碎石 / 核心接到官方课（它们留在示范课，避免出生点 Q 清箱语义变）。

### 诚实边界

- UseItem reach 仍是硬编码 +Z，不是朝向。站在能量墙西侧按 Q 打不碎，这是现状不是回归。
- 官方 01/02 不加额外可破坏，所以大厅 `crates=1/1`、出生点 Q 清箱仍成立。

---

## 上一刀（已合入）：可玩性深化（Web 轻量 Edit 真机闭环）
