...
## 本刀：角色选择（大厅入口 → 系统目录 → 带入机关狂奔）

**不需要 0.1。** 走共用启动 0.2。验收对象是当前工作树（本刀直接主线）。要验「带入对局」才需要点单人试玩，仍不需要三后端。

S1 右侧「角色选择」从禁用改成可点。点进去是系统角色列表（当前三只：立方猫 / 奔跑者 / 机器人），点选即保存，关界面回 S1。之后进机关狂奔单人试玩，本席换成所选模型；别人仍是默认猫。双塔要塞场上没有玩家网格，但这次选择会被记住。

### 1. 大厅能进角色选择

启动后停在 S1。点右侧 **角色选择**（不要点「我的内容」）。

预期：主视口换成角色选择屏（顶栏返回 + 标题「角色选择」），S1 与频道窗都看不见。失败：按钮仍是灰的、点了没反应；或者弹出一扇小 Window 而不是铺满主视口。

### 2. 点一只角色，预览跟着换

点 **奔跑者**（或机器人）。看右侧 3D 预览和「已选择」那行字。

预期：卡片高亮切到刚点的那只；右侧预览换成对应模型（奔跑者 / 机器人没有动画，会静止旋转）；已选择那行变成新名字。失败：点了没高亮；预览仍是猫；崩了。

### 3. 返回大厅，入口英文副标题跟着变

点左上 **返回**。

预期：回到 S1；角色选择那一行下面的英文副标题不再是 `CHARACTER`，而是 `Runner` 或 `Robot`（看你第 2 步选的）。失败：回不到 S1；副标题仍是 CHARACTER。

### 4. 带进机关狂奔单人试玩

点左卡进机关狂奔，点 **单人试玩**。看本席（青色薄膜那只）。

预期：本席是第 2 步选的那只，不是猫。失败：本席仍是猫；或整局角色都换成了所选（远端也该是猫，Solo 只有自己所以看不出来远端）。

### 5. 关游戏再开，选择还在

完全退出客户端，再走 0.2 打开。先看 S1 角色选择副标题，再点进去看高亮。

预期：仍是第 2 步那只，不必再选。失败：回到默认猫。

### 本刀不测

- F2 可玩性重签；
- Kenney 其余 23 只动物入库（D-F9 未拍）；
- 联机时别人看见你的角色（选择不进协议帧）；
