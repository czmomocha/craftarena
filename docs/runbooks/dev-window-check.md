...
## 本刀：M5 A1 音频内核（PlaySfx 退场）

不需要三后端。按「共用启动」0.2 开窗口化主场景即可。

这刀把 F 线 FB 的一次性 `PlaySfx` 换成可复用音频模块：五条总线、声部池、冷却节流、`user://` 音量设置。Autoload 不加。现有 8 个内部临时 WAV 仍在 `game/content/audio/f_line_temp/`，经 `ClientAudio` 注册进内核。temp 目录里的正式素材**本刀不入库**（A1 不做新素材；B1 / B2 再拷）。

**诚实边界：** F 线 FB 真正接到玩法上的只有机关预警音。跳 / 落地 / 打箱 / 冲线 / 脚步那 7 个 WAV 在磁盘上，但调用点本来就没有。本刀不增删玩法接线（B1 才做路由表）。窗口因此只验「预警仍有声」和「连续跑不会因为本刀突然爆音」。
...
预期：预警盒闪的那一下**仍有一声短音**（和 F 线 FB 同一条 telegraph）。失败：预警盒在、完全无声；或踩实心半周期才响（那是击退，不是预警）。Headless 没有这条——必须窗口化。

### 3. 按住 W 连续跑

操作：按住 **W** 连续跑几格。

预期：不会因为本刀突然每步叠一串脚步声。失败：按住 W 机关枪爆音（说明误把脚步接到了无节流路径）。本刀本来就不接线脚步，所以「没有脚步声」是预期，不是漏做。
