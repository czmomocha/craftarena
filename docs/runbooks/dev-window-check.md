...
启动后先看到产品 UI 的 S1 主大厅（主视口 `Control`），**不是**机关狂奔那扇自绘 `Window`。点一张玩法卡才弹出频道窗（仍是代码创建的 1280×720 `Window`，`own_world_3d`）。窗题与按钮走本地化键（`UiCopy`）：本机 Godot locale 为 `zh*` 时 TRAPRUSH 窗题是 **机关狂奔**；BASTION 频道同样这套按钮，但课 id 钉 `blueprint_01`、人数钉 2，且没有单人试玩 / 创作入口 / 冲刺。节点名仍是英文。

S1 上的中文副标题、卡描述、导航、状态来自 `craft_arena.s1.*`。英文品牌字（CRAFT ARENA）与导航英译是设计稿原样。占位昵称 `Maker_042`、草稿 / 已发布计数、版本号是 `DEMO_*`，不是账号契约。

### 0.1 后端（本刀不需要）

本刀验的是落地壳与双玩法入口。不点快速游戏 / 建房就不需要 `npm run dev`。若你想从频道窗再打一局，再起三后端：
...
预期：主视口是 S1（副标题 **工坊竞技场** 或 **Workshop Arena**，随 locale）。左列两张卡都是 **开放中 OPEN**，CTA **进入频道 ENTER**。BASTION 卡没有锁层、没有 🔒。右侧 **角色选择** / **我的内容** 看得到但是灰的。**公共内容广场**与**设置**可点。失败：一打开就是机关狂奔自绘窗；BASTION 仍写即将推出；文案变成键名 `craft_arena.s1.*`。

---

## 本刀：M6 F1（S1 主大厅壳 · 双玩法入口）
...
预期：S1 隐藏，弹出机关狂奔自绘窗；课 id 是 `course_01`；看得到 **单人试玩**、创作入口、冲刺。失败：仍停在 S1；或课 id 变成 `blueprint_01`。

### 2. 关频道窗回到 S1

点频道窗的关闭（窗右上）。

预期：自绘窗消失，S1 回到主视口。失败：整个客户端退出；或窗藏了主视口是空的 / 黑的。

### 3. 进 BASTION 频道

点右卡（或卡上 ENTER）。

预期：弹出频道窗；课 id 是 `blueprint_01`；人数是 `2`；**没有**单人试玩 / 创作入口 / 冲刺；仍有快速游戏 / 创建房间 / 加入房间。失败：Solo 还在；人数不是 2；课 id 仍是 `course_01`。
...
预期：S3 窗打开，S1 隐藏。关广场（返回）：回到 S1，**不**弹出频道窗。失败：关广场之后直接出现机关狂奔窗。

### 5. 角色选择与我的内容保持禁用

在 S1 点 **角色选择** 和 **我的内容**。

预期：点了没反应（控件灰）。失败：弹出未做的界面，或看起来能点却什么都不发生（启用但空操作）。

### 本刀不测

- S2 匹配屏 / S4 账号页换肤 / S5 我的内容 / S6 局内 HUD（第三批）；
- 角色选择功能（立项后，不塞进 F2）；
- BASTION 单人对 AI / 蓝图 Edit（M7）；
- 把 `craft_arena.tres` 挂到 `project.godot` 全局 theme；
- 新产品向 BASTION OGG；
...
- **这一章让 M5 的 TRAPRUSH 可玩性签署当场作废**，须在 F2 重签；AI 不代签；
- 频道仍是自绘 Window，不是 S2；
- emoji 仍不在字体子集内；本刀只带走 S1 设计稿那一处 🔒，别处再用仍是豆腐块；
- 占位昵称 / 草稿数 / 版本号不是账号或内容契约；
- 九个原型数值仍是占位桩；
- M5 带走的乱序 / 重复包未严格注入，不因本刀消失。
