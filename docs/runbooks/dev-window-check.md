...
启动后先看到产品 UI 的 S1 主大厅（主视口 `Control`），**不是**机关狂奔那扇自绘 `Window`。点一张玩法卡才弹出频道窗（代码创建的窗口化 `Window`，默认与 UI 基准同尺寸，`own_world_3d`）。主窗口与频道窗右下角都有客户端像素读数 `宽 × 高`。窗题与按钮走本地化键（`UiCopy`）：本机 Godot locale 为 `zh*` 时 TRAPRUSH 窗题是 **机关狂奔**，频道窗第一行是 **返回大厅**；BASTION 频道同样这套按钮，但课下拉钉 `blueprint_01` 且锁定、人数钉 2，且没有单人试玩 / 创作入口 / 冲刺。机关狂奔的课选择是下拉（`course_01`…`course_06` 与 `course_f_playable`），选中后 3D 预览立刻换课，不必先点单人试玩。节点名仍是英文。

S1 上的中文副标题、卡描述、导航、状态来自 `craft_arena.s1.*`。英文品牌字（CRAFT ARENA）与导航英译是设计稿原样。占位昵称 `Maker_042`、草稿 / 已发布计数、版本号是 `DEMO_*`，不是账号契约。

### 0.1 后端（本刀不需要）

本刀验 Solo 幽灵，不点快速游戏 / 建房就不需要 `npm run dev`。若你想从频道窗再打一局，再起三后端：
...
预期：客户端**窗口化**打开，默认大约 **1920×1080**，**不是**一上来就最大化 / 全屏。主视口是 S1。右下角常驻 `1920 × 1080`（或你拖过之后的实际客户端像素）。失败：一打开就铺满显示器；右下角没有尺寸读数；一打开就是机关狂奔自绘窗。

---

## 本刀：Solo 追赶本机最高纪录（幽灵）

验收对象是当前工作树。**不需要 0.1 三后端。** 走共用启动 0.2 开窗口化主场景。默认设置开。第一次单人试玩没有幽灵；冲线后再开才出现半透明分身。

### 1. 第一次单人试玩没有幽灵

S1 进机关狂奔，课选 `course_01`，点 **单人试玩**。不要先改设置。

预期：开局 3-2-1-GO；三维里只有本席一只角色；离线横幅仍在（「离线试玩，成绩不上传」）。失败：一开局就有第二个半透明人；没有离线横幅。

### 2. 跑完一次后再开，出现半透明幽灵

冲线（`course_01` 沿出生 +X 五步即可），点 **返回大厅**（不必先点取消），再进机关狂奔，点 **单人试玩**。开局后先不要自己跑。

预期：回大厅后再进频道是开局前预览（没有还停在终点的本席）；点单人试玩后，出生点附近有一个半透明的本席同款网格（大约一半不透明），会按上次磁带自己往前跑；本席仍可 WASD；HUD 可有 `ghost=on`；直播人数仍是 1 人（没有第二席名次）。失败：回大厅再进仍停在刚冲线的那局、必须先点取消；没有幽灵；幽灵是实心第二席并能把你推开；结算多出一个名次。

### 3. 设置关掉后不再出

退出单人，大厅点 **设置**，取消勾选 **追赶本机最高纪录**（英文 Race my best time），关窗。再进单人试玩。

预期：三维里只有本席，没有半透明分身。失败：勾没了仍出幽灵；文案是 emoji 或写死在场景里的中文。

### 4. 破纪录后下次幽灵变快

把设置勾回去并关窗。再跑一局，故意比上次更快冲线（或至少再冲一次）。退出后再开单人，看幽灵。

预期：幽灵仍半透明，轨迹是刚破的那次（若你第二次明显更快，幽灵应更早到终点）。失败：关设置后勾不回去；更快的那次没有成为下次幽灵；平局或更慢的一次把旧纪录盖掉。

### 本刀不测

- 在线 1 人房 / 双人 / BASTION 幽灵；
- 从服务端拉他人磁带；
- 回放局再套幽灵（点对局记录进去不应出现追赶幽灵）；
- 暂停 / 倍速；
- F2 可玩性重签。

### 诚实边界

- 幽灵是命令磁带重仿真，不是录像；与本席同 tick 推进，互不碰撞；
- 最佳磁带每课一条，独立于对局记录环 50；
- 磁带不存角色网格，幽灵用当前本席选择；
- 缺设置文件 = 开；离线不回写在线。
...
验收对象是当时工作树。Solo 记录不需要 0.1；双人列表需要共用启动 0.1 三后端（且本机已有 Guest，否则双人记录不会落库）。走 0.2 开窗口化主场景。

S1 右侧「对局记录」打开全屏列表。Solo 冲线后本机留下一条；点开后仍是 3-2-1-GO，顶栏常驻「回放中」，WASD 不能动本席，只能退出回到列表。

### 1. 入口与空态

S1 点右侧 **对局记录**（英译 RECORDS）。

预期：全屏 overlay 盖住主视口；标题是 **对局记录**（英文 Match records）；副标题写冲线后才保存、回放只读。若本机还没有 Solo 冲线记录，主区是 **还没有对局记录。** 失败：进了频道窗；标题是硬编码中文写在场景文件里；没有空态说明。

### 2. Solo 冲线后出现本机记录

返回大厅，进机关狂奔，点 **单人试玩**，跑完 `course_01` 冲线，再退出。再打开对局记录。

预期：列表至少有一条，前缀 **本机**，带 `course_01`。失败：冲线了仍是空态；Cancel / 中途退出也会多出一条。

### 3. 点开后倒计时 + 「回放中」+ WASD 无效

在列表里激活那一条（双击）。不要先按键。

预期：进对局窗；视口中央 3-2-1-GO；顶栏常驻 **回放中**；倒计时内按 WASD / 空格本席不动。失败：没有倒计时；没有「回放中」；能自己跑。

### 4. 只能退出，回到列表

回放过程中或冲线停住后点 **取消 / 离开**（频道窗第一行那套离开）。

预期：回到对局记录列表，不是 S1 首页。失败：卡在对局窗；退到主大厅把列表丢了。

### 本刀不测（当时）

- 双人服务器列表（需要 0.1 且匹配时已带 Guest；排队入座可能没有身份戳，属诚实缺口）；
- 暂停 / 倍速 / 拖进度 / 分享码；
- BASTION 回放；
- Preview / 编辑器；
- F2 可玩性重签。
...
- 回放是命令磁带重仿真，不是录像；
- 本机环 50，每账号服务端环 100；一期不做按天清扫；
- 角色网格不进磁带；
- 未登录无双人记录；Solo 仍在本机。

---

## 上一刀（已合入）：机器人 / 奔跑者对局朝向（角色选择带入后的落点与轨迹）

**不需要 0.1。** 走共用启动 0.2。验收对象是当时工作树。不需要三后端。

选机器人或奔跑者进机关狂奔单人试玩时，本席应站在出生格上、脸朝前进方向（局部 -Z / WASD 的 W）。这是表现朝向，不改胶囊。

### 1. 选机器人并进单人试玩

S1 点 **角色选择**，点 **机器人**，返回，进机关狂奔，点 **单人试玩**。不要先走、不要跳。

预期：青色本席站在起始地砖上（检查点 0 那一格），不是浮在 L 形内侧的天空里。失败：人在赛道空隙里、镜头对着空洞。

### 2. 按 W 沿出生朝向走

站稳后按住 **W** 约 1 秒。

预期：机器人正面向前走（鼻子朝位移方向），走在地砖上。失败：侧着滑 / 倒着走；一按就掉进内侧空洞。

### 3. 换回立方猫，默认画面仍在

返回大厅，角色选择点 **立方猫**，再进单人试玩。

预期：本席仍是猫，站位与朝向和修这个 bug 之前一样。失败：猫也转了 180°，或选不回去。

### 本刀不测

- F2 可玩性重签；
- 给猫加朝向补偿（默认角色画面保持不动）；
- 奔跑者 / 机器人的 clip 动画（它们仍是静态网格）；
- 协议帧 / 快照带角色 id；
- 联机时别人看见你的角色（选择不进协议帧）；
- 双塔要塞场上出现指挥网格（场上没有玩家胶囊）。

### 诚实边界

- 系统池当前只有仓库里已有许可证的三份网格：猫（有 clip）、奔跑者与机器人（静态，对局里静止）。加角色 = 目录追加 id + locale 一行，不必改界面；
- 本机选择写在 `user://character_select.json`，不进快照、不进回放、不改 ContentHash；
- 双塔要塞只记住这次选择，场上画面不变；
- 选择界面转盘仍按模型原朝向展示正面，对局里才转到 Godot -Z。

---

## 上上刀（已合入）：创作课编辑器窗口可缩放 + 3D 操作

**不需要 0.1。** 走共用启动 0.2。验收对象是当前工作树（本刀直接主线）。

编辑器和预览默认仍各占一半，但会跟着游戏窗口变大变小，也可以拖边沿。工具条去掉「放置」、参数有说明和范围。传送带方向用箭头按钮切换。3D 视口：滚轮拉远近、右键转角度、Ctrl+左键平移；点中物体出现 XYZ 轴可拖。

### 1. 拉大游戏窗口，编辑器 / 预览跟着长

点左卡进入机关狂奔，点 **创作课程**。记下编辑窗和预览窗现在大概占主窗口的一半。把**游戏主窗口**拉大（不要只拖那两块嵌入窗）。若从 Godot 编辑器运行，拉的是编辑器窗口：Game 视图里两块应跟着铺满可用区域，不要停在原来的 1920×1080 小框里留一圈黑边。

预期：两块编辑/预览区域一起变大，仍大约左右各半，3D 场能看见更多。失败：主窗口大了，两块还停在原来的像素大小，周围一圈空黑。

### 2. 拖边沿能改大小

拖编辑窗右边沿变宽，或拖预览窗左边沿。

预期：能拖动；变宽的那一侧吃掉另一侧的宽度，两块仍并排不叠成一团。失败：鼠标在边沿变成禁止符、完全拖不动。

### 3. 工具条更矮，按钮没有「放置」

看编辑窗顶部工具条。

预期：按钮是「检查点 / 传送门 / 固体 / 传送带…」，**没有**「放置」两字；同一流式区域里能看到大部分按钮，不再堆成七八行把 3D 挤没。传送带按钮旁边有一个 **左右**（或 `<->`）小按钮。参数区每一项左边是说明（带 `0-600` 这类范围），右边是数字框，大约两列。天空下拉只占一小段，不是整行拉满。失败：仍写「放置检查点」；参数只有数字框没有说明；天空下拉撑满整行。

### 4. 传送带方向按钮

点传送带旁的方向按钮，再点 **传送带** 摆一块；再点方向按钮切回去，再摆一块。

预期：默认那块箭头沿左右；点一下按钮变成 **上下**（或 `^v`），再摆的那块朝向跟着变，而不是每摆一块自动转 90°。失败：仍是第一次左右、第二次上下自动交替；或按钮点了没反应。

### 5. 3D：滚轮 / 右键 / Ctrl+左键

鼠标放在编辑器 3D 场上（不要放在按钮上）。滚轮。按住右键拖。按住 Ctrl 再按住左键拖。

预期：滚轮拉近拉远；右键拖绕场转角度；Ctrl+左键平移机位，能看到更远处的格子。失败：滚轮没反应；右键拖没转；Ctrl+左键仍在选格子而不是移镜头。

### 6. 选中后拖 XYZ 轴

点一块已放的固体（或先点 **固体** 摆一块再点它）。

预期：物体上出现红 / 绿 / 蓝三根轴。拖红轴只沿 X 动，松开后它停在新格子上。失败：没有轴；拖轴没动或三个方向一起飞。
...
- 嵌入子窗仍不能拖成系统窗口（`gui_embed_subwindows`）；
- 参数范围是编辑器 SpinBox 门闩，不是新的 Schema 字段；
- 传送带只有左右 / 上下两档，没有 180° 反向档。

---

## 上一刀（已合入）：对局窗 HUD 紧凑 + 预览右键转视线

**不需要 0.1。** 走共用启动 0.2。验收对象是当前工作树（本刀直接主线）。

对局频道窗腾出 3D 画面：状态行上移到顶行且默认黑字，`FPS` 贴左下，原来占多行的服务器 / 房间码 / 课 / 人数 / 邀请收到第二行，开局后局时那些字贴右下。开局前右键拖可转视线，开局后还原并锁定，只留滚轮拉远近。设置里可以用调色板改这些字色和按钮字色。

### 1. 顶行是状态，FPS 在左下

点左卡进入机关狂奔。不要点单人试玩。

预期：最上面一行是 `join=idle` 这类调试状态，**黑字**。左下角有 `FPS --` 或数字。失败：顶行仍是 `FPS`；状态行还在第二行；字是白的。

### 2. 服务器 / 房间 / 课 / 人数 / 邀请在同一行

看顶行状态下面的按钮。第一行仍是「返回大厅 / 快速游戏 / …」。再下一行。

预期：服务器地址、应用、房间码、课下拉、人数、邀请、复制邀请挤在**同一行**。3D 预览比改之前更靠上、更高。失败：房间码、课、人数仍各占一整行。

### 3. 开局前右键拖转视线，滚轮仍能拉远近

保持未开局。按住**右键**拖鼠标，再滚轮。

预期：镜头绕赛道转，能看到侧面 / 更俯或更平；滚轮仍能拉近拉远。失败：右键拖没反应，或拖的是平移而不是转角度。

### 4. 开局后还原并锁定，只能滚轮

在转过视线、拉过远近之后，点 **单人试玩**。

预期：镜头立刻回到开局前那种斜 45° 默认机位；再按住右键拖**不能**转角度；中键拖也不能平移；滚轮仍能拉远近。右下角出现局时（`0:00.xx` 这类），默认黑字。失败：开局后仍停在预览时转过的角度；或开局后还能右键转；或局时还在窗口顶部占一行。

### 5. 设置里的调色板能改字色

点 **设置**（大厅齿轮或频道窗按钮）。找到「界面字色」和「按钮字色」。点色块呼出调色板，改成明显的颜色（例如红字），关掉设置窗回到频道。

预期：状态行 / FPS / 开局后右下局时跟你选的界面字色走；频道窗按钮上的字跟按钮字色走。再开设置，颜色还在。失败：调色板呼不出；关窗后颜色丢了；只改了设置窗自己、频道窗没变。

### 本刀不测

- F2 可玩性重签（仍作废，须覆盖 S1 新 UI + 天空 + `course_06`）；
- 改默认相机距离 / FOV 产品值；
- locale 热切换 / 画质 / 键位；
- 对局进行中自由旋转镜头（本刀明确锁住）。
...
- 默认黑字在深色天空上可能偏暗，所以才把调色板放进设置；
- 中键平移仍只在开局前可用，开局后与右键转视线一起锁；
- `FPS` 仍是观察工具，不进裁决、不进快照、不发网络。

---

## 上一刀（已合入）：天空盒 A3（多层官方课 `course_06`）

**不需要 0.1。** 走共用启动 0.2。验收对象是当前 `main`（A3 直接主线）。想顺手验匹配 HTTP 再起三后端。

`course_06` 是七层多层：起点在最底层 `y=-6`，终点在最顶层 `y=+6`，层距 2 格，底层 13×13 满铺。爬升靠三对双向传送门（沿 +X / −X 走进去），被跳过的层有电梯。顶层两层没有弹射垫。

### 1. 单人试玩能开这张课

点左卡进入机关狂奔。课程 id 填 `course_06`，点 **单人试玩**。

预期：能开玩，3D 里看见上下叠了好几层石色地板，背景仍是 A2 那张粉彩群山天空。失败：`offline_error=`、赛道盒为空、或仍加载 `course_01`。

### 2. 从底层传到顶层并冲线

出生点在最底层中央。沿 +X 走进第一道传送门，落地后再 +X 进第二道，再 −X 进第三道，最后 −X 走进金色终点。

预期：`pads=` 从 1/1 走到冲线（`finish=` / `result=`）。楼层读数从约 `-6` 升到约 `6`。失败：卡在某一层走不上去；掉出 ±8 格被重生且爬不回来；或冲线没有结算读数。

### 3. 从上层掉下去落在底盘，不是出界

在某一层（不要最底层）走到地板边缘走下去，或走进电梯井口的空洞。

预期：落到更低一层或最底层满铺上，**不是**立刻 `out_of_range` 弹回出生点。失败：一掉出当前层就被出界复位。

### 4. 顶层两层看不见弹射垫

镜头拉到最上面两层转一圈。

预期：没有弹射垫占位（本课弹射垫袋是空的）。失败：顶层有会把人弹出去界盒的垫。

### 本刀不测

- F2 可玩性重签（仍作废，须覆盖 S1 新 UI + 天空 + 本课）；
- `--route=safe`（仍只对 `course_01`）；
- 改 `STUB_HALF`、爆破半径、推击力度；
- 把 `course_f_playable` 填进创建房间（匹配 HTTP 仍拒）。

### 诚实边界

- 爬升用的是**传送门**，不是弹射垫：层距 2 格时弹射峰值会从下方顶到上层地板，落不上去；
- 三座电梯补被跳过的层，不在最短冲线脚本上；
- A* 探针两次中转不够三段，`--bot-run` 重放的是手写脚本，不是搜索出来的路；
- 独立贴图**不在** `npm run asset-budget` 的 CI 门禁里；
- M5 那份 TRAPRUSH 可玩性签署仍然作废，须在 **M6 F2** 重签。

---

## 上一刀（已合入）：天空盒 A2（天空渲染与创作者选择）

**不需要 0.1。** 走共用启动 0.2。验收对象当时是 worktree（`F:/study/craftarena-wt-sky-a1`，分支 `feat/sky-a1`）。
...
看刚打开的系统窗口：能看到标题栏，能拖边改大小。

预期：窗口小于或等于显示器工作区，可以拖动缩放；右下角数字跟着变。失败：一打开就是最大化 / 独占全屏，拖不出系统边框。

### 2. 进频道后用「返回大厅」回 S1

点左卡进入 TRAPRUSH。

预期：S1 隐藏，弹出频道窗；第一行最左是 **返回大厅**（locale 非 zh 时是 **Back to lobby**）。点它：频道窗消失，S1 回到主视口。右下角尺寸读数仍在。失败：频道窗没有这颗按钮；点了整个客户端退出；或窗藏了主视口是空的 / 黑的。

### 3. 关窗仍回 S1

再进任一频道，点频道窗关闭（若嵌入子窗看得到关闭）。

预期：与第 2 步相同，回到 S1。失败：只有关窗能回、按钮无效，或反过来。
...
- 频道仍是自绘 Window，不是 S2；
- 广场 overlay 仍是 1600×900，本刀没改它；
- M5 可玩性签署仍作废，须在 F2 重签。

---

## 上一刀（已合入）：M6 F1（S1 主大厅壳 · 双玩法入口）

走当时的「共用启动」。不需要三后端。点左卡进 TRAPRUSH（`course_01`，有单人试玩）；关窗回 S1；点右卡进 BASTION（`blueprint_01` / 2 席，无单人试玩）。当时频道窗还没有第一行「返回大厅」，开发机窗口仍是 1600×900 最大化。

历史步骤以 git 为准。

---

## 上一刀（已合入）：M6 E4（客户端 BASTION 对局壳）

本刀**第一次有画面**。占位剪影、自绘大厅、复用已登记 cue；当时还不是 S1。需要两个窗口化客户端与 `npm run dev`。课 id 填 `blueprint_01`，从当时的自绘大厅进房。

历史步骤以 git 为准。F1 之后进 BASTION 的窗口路径改成：S1 点右卡 → 频道窗已钉 `blueprint_01`。
...
## 上一刀（已合入）：M6 D1–D4（BASTION 离线玩法底座）

**窗口验收：无。** 不是省事，是这四章的性质——它们全部在 `game/src/ugc/`、`game/src/simulation/` 与 `game/src/games/bastion/` 里，**没有接任何场景、没有任何入口按钮、没有一行表现代码**。BASTION 第一次有画面是 **E4 客户端对局壳**。

人类确认这四章成立走命令行：`npm run typecheck` / `npm test` / `npm run redline-scan` / `npm run test:gut:full`。GUT 里当时新增四份脚本：`test_bastion_blueprint_contract.gd`、`test_bastion_path_guard.gd`、`test_bastion_match_session.gd`、`test_bastion_towers_and_economy.gd`。
...
## 更早（已合入）：测试期 VPS Web 分发（M-Export 剩余）
...
这刀把公共内容广场的界面从手搓的 `ItemList` 换成产品 UI 的 S3 屏幕。**只换了视图**：tab、HTTP 拉取、选中、Solo、建房全部照旧，`match_lobby_director.gd` 一行没改。

> 上一刀（四项前置）的窗口检查已并入这里：S3 当时没有入口只能截图看，现在它就在窗口里，**按真窗口验**。

### 1. 从大厅进广场，看到的是新界面

操作：按「共用启动」0.2 开窗口化主场景，点大厅的 **内容广场**。

预期：弹出一个 **1600×900** 的窗口（旧版是 960×540），里面是深色的产品 UI：左上 logo 与 **公共内容广场**、一行四个标签页 **最新 LATEST / 评分 RATING / 游玩次数 PLAYS / 已验证 VERIFIED**、右侧三个**灰掉**的控件（排序 / 标签 / 搜索）、下面一行小字说明。大厅窗口同时隐藏。**点了之后广场应马上出现**，不能卡住数秒才弹出来（无后端时也一样）。

失败：还是一个窄窗口加一条条文字列表 ⇒ 视图没换上；窗口空白 ⇒ 场景加载失败，看控制台；**点了内容广场后画面卡住 2–10 秒才弹出** ⇒ 列表 GET 又在主线程上阻塞了（Windows 上连不上控制面会等到 TCP 超时）。

### 2. 空态说人话

预期：没有后端时列表是空的，网格区域显示 **广场还没有内容**（`craft_arena.ui.plaza_empty`），不是一片空白。

失败：什么都没有 ⇒ 空态没接上。

### 3. 四个标签页真的会切

操作：依次点 **评分 RATING**、**已验证 VERIFIED**、**最新 LATEST**。

预期：**被选中的那个是粉色高亮**（设计的 `TabChipActive`），其余是灰底。切换时列表重建。**粉色应马上跟上点击**，不能每点一次卡住数秒。

失败：点了没有任何视觉变化 ⇒ 变种没换；**选中的变成灰白而不是粉色** ⇒ 有人把它改回 `toggle_mode` 了，那会画成 Button 自己的按下态而不是设计的强调色；**每点一个标签页都卡住数秒** ⇒ 列表 GET 又在主线程上阻塞了。

### 4. 三个没有后端的控件必须是灰的

操作：试着点 **排序：综合 ▼** 和 **标签：全部 ▼**，试着在搜索框里打字。

预期：**都点不动、打不进字**。`ContentPlaza` 没有 tab 内排序、没有标签过滤、没有搜索（[ui-wiring.md §2.2](ui-wiring.md)）。

失败：能点、能输入但什么都不发生 ⇒ 比灰掉更糟，必须修。

### 5. 返回有两条路

操作：先点左上角的 **<**；再进一次，点底部动作行的 **返回大厅**。

预期：两条都回到大厅，且大厅窗口重新可见。

> 底部那行 **单人试玩 / 创建房间 / 返回大厅** 是 **STOPGAP**：设计稿里没有这三个按钮的位置，而它们是已上线的产品出口，删掉属于产品回退。所以先原样保留、附在屏幕下方。**样子不好看是已知的**，要把它们放进设计里需要设计稿（[ui-wiring.md §2.2](ui-wiring.md) 第 1 条）。

### 6. 有真实内容时的卡片（需要后端 + 已发布内容）

如果手头有一份已发布的 UGC：起 `npm run dev`，在广场里应能看到卡片。逐项核对：

- 标题是**词库名**（如 `warm_forge`），不是自由文本；
- 标签是白名单结构化值；
- 星级 = `rating_sum / rating_count`，**未评分显示 0.0 与五个空心星**——那是"还没人评分"的事实，不是缺值；
- 次数是**原始整数**（`7 次游玩`），不是 `1.2k`；
- **没有作者名，也没有作者色点**（列表行没有作者字段，不编造署名）；
- **缩略图区域是空的**（玩家不能上传贴图，UGC 一期不可能有预览图）；
- 未验证的内容有橙色 **未验证可完成 ?** 角标，且它的 **编辑/复用** 按钮是灰的。

没有后端时这一步用离屏渲染代替（三行假数据，含未评分与未验证各一条）：
...
**注意不要用 `screenshot.gd` 验这一步**：那个渲染的是独立模式，八张有缩略图有作者的占位卡，与玩家实际看到的完全不同。

### 7. 选中一张卡再单人试玩（需要后端 + 已发布内容）

操作：点一张**已验证**的卡，再点底部 **单人试玩**。

预期：广场关闭，回到大厅并直接开局，HUD 显示离线横幅。这条走的是签名 bundle，不是课程路径。

失败：点卡片没反应 ⇒ 选中信号没接上；点单人试玩无事发生 ⇒ 选中态没传回 entry。
...
预期：GUT **1675/1675**（`test_content_plaza.gd` 从 9 条涨到 17 条，含开广场 / 切 tab 在死控制面下不得阻塞）；两个校验各一行 `RESULT: PASS`；`--package-check` `ok=true`。

### 本刀不测

- **S1 / S2 接线**：分属第二（M6）/ 第三批，仍是自绘；
- **排序 / 标签 / 搜索的功能**：后端就没有，本刀只保证它们不骗人；
- **Solo / 建房在设计里的位置**：需要设计稿，见上面第 5 步；
- **卡片缩略图**：一期不可能有；
- 公开 TLS、PR Web 沙盒、Android / iOS。

### 诚实边界

- **广场窗口从 960×540 放大到 1600×900**，因为四列网格在千把像素以下会塌。小屏开发机上这个窗口会比以前挤，尚未在小屏上看过；
- **第 6、7 步需要真实已发布内容才能验**，没有后端时只能靠离屏渲染看形状，那不能证明 HTTP 路径；
- **底部动作行是临时的**，它不属于设计稿。看着突兀是预期，不是待修 bug——修它需要产品/设计输入；
- **`@onready` 那类 bug 测试抓不到**（[ui-wiring.md §2.3](ui-wiring.md)）：GUT 里 `add_child` 发生在已运行的树上，`_ready` 先于 setter。这一刀是靠离屏渲染发现一张卡都没出来才暴露的，现在补了断言，但同类问题在别处仍可能存在；
- **无后端时列表 GET 不得卡住窗口**。窗口化运行 `live_io` 恒为 true，开广场和切标签都会打控制面。这条路径走 `HTTPRequest`，先画出空态再等结果。若退回 `ControlPlaneHttp` 的 `OS.delay_msec` 轮询，Windows 会卡到 TCP 超时（最长 10 s），macOS 通常立刻 connection refused，所以只在 Windows 上像「点一下停几秒」。

---

## 上一刀（已合入）：UI 接线第一批的四项前置（字体 / CI 门禁 / 文案迁移 / 文档）
...
- 清单一：[网络故障人工检查](network-fault-check.md)（CD-53 §2.5 九项的执行步骤 + §5 记录表）；
- 清单二：[可玩性签署：TRAPRUSH](playability-signoff-traprush.md)（八项 + 总结论）。

**诚实边界：** AI 只落库步骤与空表，**不代签、不代填**。签完之前 M5 **不是**已退出（宪法第二十四条）。本刀期间**不要**接任何一屏产品 UI——清单描述的是现有自绘大厅，换 UI 会让刚签的结论作废。`dnctl` 那组 macOS 整形命令未在本机实测，第一次执行的人把可用写法直接改回 runbook。

按「共用启动」0.1 起 `npm run dev`，再 0.2 开窗口化主场景（客户端 A）。

### 1. 官方五张都还能开局

操作：课程 id 依次填 `course_01` … `course_05`，每张点 **单人试玩**，走两步后 **取消**。

预期：五张都能开玩，3D 里能看见各自的赛道（01 捷径/安全路、02 侧向跨层、03 破箱上楼、04 垂直塔、05 双路线）。失败：某张 `offline_error=` 或赛道盒为空 ⇒ 先修这张，别继续走闭环。

### 2. 创作 → 验证 → 发布

操作：点 **创作课程**，摆到验证面板 **可以发布**，点 **发布**，再点 **返回大厅**。

预期：状态行 `publish=ugc_<32hex>@1`；回显里**没有** signature。失败：`publish=validator_issues`（世界还没合法）；或状态行出现密钥。

### 3. 广场能看见并能 Solo

操作：点 **内容广场** → **最新** → 选刚发布那行 → **单人试玩**。

预期：能进 Solo，跑得动。失败：广场空（控制面没起 / 没走 `GET /content/plaza`）。

### 4. 广场建房 + 邀请 + 第二客户端加入

操作：回广场选同一行，点 **创建房间**；点 **复制邀请**；另开一个窗口化主场景（客户端 B），把房间码贴进 **房间码** 框，点 **加入房间**（不要填课程）。

预期：A 状态行 `join=ready`、`content=ugc_…`、有 `room=`；邀请串是 `主机[:端口] 房间码`，**不含**内容 id；B 拿到同一 `room=` 与同一 `content=`，两人互相看得见。失败：B 掉回官方 `course_01`；或邀请串里带了课 id。

### 5. 两人跑完并写库

操作：两人冲线。

预期：状态行先 `result=`，随后 `settled=`（MatchHost POST 成功后大厅 GET 到）；结算表有名次行与 MVP。失败：只有 `result=` 一直没有 `settled=`（看 `npm run dev` 终端的控制面日志）；或两台看到不同课。

### 6. 音频三条路径都有声

操作：本局里听脚步 / 跳 / 落地 / 打箱 / 过垫 / 冲线；听大厅→对局→结算的 BGM 切换；让 B 在你旁边动作，听远端 3D 定位；打开 **设置** 拉低音乐再关窗。

预期：关键事件都有声且能区分；BGM 三态会换；远端动作听得出方向；音乐滑条即时生效并落 `user://`。失败：完全无声（先排除系统静音）；或按住 W 爆音。

### 7. 走一遍网络故障清单（至少延迟 + 短时断线 + 帧注入）

操作：照 [网络故障人工检查](network-fault-check.md) §2.4 第 2、6 步与 §3 第 8、9 步做，把结论填进它的 §5。

预期：延迟下本席仍跟手、远端可见延迟；断网 5 秒后**自动补票重连**且 `seat=` 不变；`flood` 下 `x` 每 tick 至多前进一格；`tamper` 的九种畸形帧一条都不生效，最后那条合法帧仍能动。失败：位姿一拍多格（位置伪造门禁失效）、对局进程退出、或重连换了席位。

### 8. 签可玩性清单

操作：照 [可玩性签署：TRAPRUSH](playability-signoff-traprush.md) 从 §1 走到 §8，填第 9 节八行与总结论。

预期：八项都有结论（通过 / 带条件通过 / 不通过），"带条件通过"写清条件落到哪个里程碑。失败：留空却宣布 M5 退出。
...
预期：`typecheck` 无输出；`npm test` 全绿（本刀新增 `tools/dev-launcher/tests/m5_exit.test.ts` 的文档断言）；`redline-scan` `no findings`；GUT 全绿且**数量与上一刀一致**——本章没碰仿真。失败：GUT 或 bot-run 数字变了 ⇒ 有人在文档章里动了代码，停下来查。

### 本刀不测

- 由 AI 代签任何一份清单（明确不做）；
- 把外部整形工具（clumsy / netem / dnctl）写进仓库、CI 或 npm script；
- 接 S1 / S2 / S3 产品 UI 任何一屏（M5 退出之后才排第一批）；
- 字体入包、公开 TLS、PR Web 沙盒、Android / iOS；
- 改爆破半径 / 推击力度 / 道具重生、改 [CD-43 §4](../../Confirmed-docs/40-technical/43-networking-and-replay.md#4-已锁定的网络参数) 的锁定值。

---

## 上一刀（已合入）：M5 C4 邀请与已签名 UGC 联机

需要三后端。广场「创建房间」走 `POST /matchmaking/rooms` 的 `content: { id, version }`（与官方 `course` 互斥）。邀请串只有房间码：桌面是 `主机[:控制面端口] 房间码`，Web 是 `?room=`。内容 id **不进** URL。

按「共用启动」0.1 起 `npm run dev`，再 0.2 开窗口化主场景（客户端 A）。

### 1. 发布一张自制课（若广场里已有自己的课可跳过）

操作：点 **创作课程** → 摆到验证器 **可以发布** → 点 **发布**。点 **返回大厅**。

预期：状态行 `publish=ugc_…@1`。失败：没有 id，或密钥出现在状态行。

### 2. 广场创建房间

操作：点 **内容广场**，在 **最新** 里选刚发布的那一行，点 **创建房间**（不是大厅里那颗官方课「创建房间」）。

预期：广场关掉；大厅状态行 `join=ready`、`content=ugc_…`、有 `room=`；邀请只读框出现 `127.0.0.1:8080 ABCD23` 这种串（端口随本机控制面），**没有** content id。失败：400 / `invalid_content`；或邀请框把 `ugc_` 写进 URL；或状态行仍是 `course_id=course_01` 而没有 `content=`。

### 3. 复制邀请

操作：点 **复制邀请**。

预期：剪贴板是第 2 步那串 `主机[:端口] 房间码`，不含 `content=`。失败：空剪贴板，或串里带了课 id。

### 4. 第二客户端按码加入

操作：再开一个窗口化主场景（客户端 B）。把房间码贴进 **房间码** 框（或只贴码；桌面邀请串的最后一段就是码），点 **加入房间**。不要带课表。Web 路径等价：同一控制面的 `/play/?room=码`（`?edit=` 优先，本刀不要两个都带）。

预期：B 的 `join=ready`、同一 `room=`、同一 `content=`；两人能看见对方。失败：B 变成官方 `course_01`；或加入请求带了 body 被 400。

### 5. 跑完并看结算哈希

操作：两人冲线（或按本机习惯跑完）。看大厅状态行 `settled=`。

预期：结算写库成功；Godot 结算 payload 带 `content_hash`（控制面结算 HTTP 字段仍不改）。失败：只有官方课结算、没有 hash，或两台看到不同课。

### 本刀不测

- 把 content id 写进邀请 URL（明确不做）；
- 改结算 HTTP；
- 公开 TLS / PR Web 沙盒；
- 改爆破半径 / 推击力度 / 道具重生。
...
## 上一刀（已合入）：M5 C3 创作者发布闭环

需要三后端。编辑外壳「发布」走控制面代签 `POST /content/submit`（不调持钥 `POST /content/publish`）。广场 `live_io` 时拉 `GET /content/plaza`。匹配 HTTP 扩表是 C4（已交）。

按「共用启动」0.1 起 `npm run dev`，再 0.2 开窗口化主场景。

### 1. 打开创作并放到可发布

操作：点 **创作课程**（**Create course**）。工具条应有 **发布** / **Publish**。
...
预期：不发 HTTP；状态行 `publish=validator_issues`。

### 3. 导入可发布课并发布

操作：摆到验证器 **可以发布**，点 **发布**。

预期：状态行 `publish=ugc_<32hex>@1`。失败：回显带 `signature`，或密钥出现在玩家包。

### 4. 广场能看见并 Solo

操作：返回大厅 → **内容广场** → 选刚发布的行 → **单人试玩**。

预期：能进 Solo。失败：广场仍空。

### 本刀不测

当时不测匹配 UGC / 邀请串（那是 C4，现已交）。
...
## 上一刀（已合入）：M5 C2 第 5 张官方赛道（双路线竞速）

需要三后端。`course_05` 双路线竞速：能量墙挡在 +X 脊 x=2。危险捷径站墙 −Z 侧 Q 打碎再上楼；安全路沿 +Z 绕到 x=4。官方课数量已到 CD-61 的 5 张上限。`--route=safe` 仍只对 `course_01`。`course_f_playable` 仍不是匹配课。

---

## 上一刀（已合入）：M5 C1 第 4 张官方赛道（垂直塔）

需要三后端。按「共用启动」0.1 起 `npm run dev`，再 0.2 开窗口化主场景。

`course_04` 是垂直塔：电梯 + 弹射 + 压板在侧廊，上楼靠 `two_way` 传送（发布可达图不认电梯）。危险捷径 = 沿 +X 走进 entity 10 上楼。安全路 = 走到 z=+1 坐电梯再回上层脊。侧向 `two_way` 是区块传送夹具，不在最短冲线路径上。

**诚实边界：** 电梯跨层不进发布可达图，探针会走传送捷径。联机动作音仍是 B2。`--route=safe` 仍只对 `course_01`。`course_f_playable` 仍不是匹配课。

### 1. 课程改成第四张并建房

操作：课程 id 改成 `course_04`，人数保持 `2`，点 **创建房间**。

预期：状态行 `course=` 跟到 `course_04`；房间码出现；3D 里能看见上层甲板、出生点箱、侧廊电梯/弹射/压板。失败：400 / `invalid_course`、仍加载 `course_01`、或窗口立刻退出。

### 2. 第二席加入同一房

操作：另开一个窗口化主场景（或第二个客户端），填同一房间码，点 **加入房间**。不要在加入请求里带课程字段。

预期：两人进同一局，看到彼此胶囊。失败：加入被拒、各开各的课、或第二席卡在排队。

### 3. 上楼并冲线

操作：本席沿 +X 走进上楼传送门（或走 z=+1 电梯再回到上层脊），踩过上层两块检查点垫，走进金色终点。

预期：`pads=` 从 1/3 走到 3/3，冲线后出现结算（`result=` / `settled=`）。失败：上层垫永远不计、掉出 ±8 格出界桩后无法复位、或冲线没有结算。

### 4. 侧廊机关看一眼（不必用它完赛）

操作：从出生点往 −Z / +Z 看弹射、压板、电梯是否在动。

预期：压板上下砸、电梯走纯 Y、弹射垫是固定固体。失败：三件都看不见，或走 +X 捷径脊被电梯挡住。
...
## 上一刀（已合入）：M5 B1 TRAPRUSH 玩法音效路由（离线路径）

不需要三后端。按「共用启动」0.2 开窗口化主场景即可。

Solo / Preview 从本地会话 diff 派生事件，经 `traprush_audio_router.gd` 唯一映射表变成 cue id。脚步按步长节流。机关循环环境音走 3D（`AudioListener3D` 挂跟随相机）。流文件仍是 F 线短 WAV，同一文件被多个 cue 复用。

**诚实边界：** 正式 OGG 与联机远端 3D 是 B2。短 WAV 循环会有接缝。三种环境失败音目前只是同一批 WAV 的音高变体。v1 快照没有事件字段，本刀不接线联机动作音（预警仍走大厅表现层，A1 已有；Preview 本刀没有预警音）。Preview 拒绝推击，Solo 单人打不到人则无推击音。

### 1. 课程填示范课并 Solo

操作：课程 id 改成 `course_f_playable`，点 **单人试玩**。

预期：切到对局曲；开跑后能听到脚步（按住 W 不再是每帧一响）。失败：完全无声，或按住 W 机关枪爆音。

### 2. 跳与落地

操作：空格跳起再落地。

预期：起跳一声、落地一声，和脚步不是同一个音色。失败：跳没有声、或落地与跳完全同音且无法分辨。

### 3. 打碎箱子

操作：捡到爆破球后对着可破坏箱按 Q（或鼠标左键）。

预期：使用道具与打碎各有一声（当前是同一 WAV 不同音高）。失败：打了没声。

### 4. 检查点、传送、冲线

操作：踩过一块检查点垫；走进传送门；最后踩终点。

预期：过垫、传送、冲线各有反馈；冲线后曲切到结算床垫。失败：过垫/传送完全静音，或冲线没有收束音。

### 5. 环境失败三种

操作：踩周期机关固体半周期 / 走出 ±8 格出界桩 / 被压板或门挤住（示范课里都有对应件）。

预期：三种复位不是完全同一声；复位时 HUD 仍有 `setback=` 读出。失败：三种失败完全无声。

### 6. 走近循环机关

操作：走近传送带、电梯、喷火、滚柱、压板、摆锤、传送门、开关门或冰面（不必每一种都走完，听到远近变化即可）。

预期：靠近能听到循环环境音，走开变小或消失；转镜头时声像会跟着走。失败：贴着机关也完全无环境音，或离开后仍以原音量贴耳。

### 7. Preview 同样有声

操作：点 **关卡编辑**（或已开的编辑窗）→ Preview 试玩，跳一下、走两步。

预期：Preview 里也能听到跳 / 脚步。失败：Solo 有声、Preview 完全静音。

### 本刀不测

- 把 temp 正式 OGG 拷进 `content/audio/`（B2；许可证未拍）；
- 两个客户端听远端玩家（B2）；
- 改爆破半径 / 推击力度 / 道具重生；
- locale 热切换设置页；画质 / 键位；
- 第 4 张官方课。
...
## 上一刀（已合入）：M5 A3 音乐总监与设置窗

不需要三后端。按「共用启动」0.2 开窗口化主场景即可。

大厅开始循环播临时床垫曲（`theme_idle`，仍是 F 线短 WAV 顶替，不是正式 OGG）。「设置」打开独立窗，五条滑条 + 静音，关窗落 `user://audio_settings.json`。Solo 换 `theme_run`，冲线换 `theme_end`，取消回大厅曲。

**诚实边界：** 四首床垫都是现有短 WAV 的拷贝，循环会有接缝。正式 OGG 是 B2。玩法 SFX 路由是 B1。locale 热切换 / 画质 / 键位不做。

### 1. 大厅能听到曲

操作：共用启动 0.2 开窗，等一两秒。

预期：能听到循环短曲（Music 总线）。Headless 没有这条。失败：完全无声（先排除系统静音 / 本机音量）。

### 2. 拉低音乐滑条

操作：点 **设置**（**Settings**），把 **音乐** / **Music** 滑条拉到最左，关窗回大厅。

预期：大厅曲明显变小或听不见；音效预警若再测仍在（Sfx 没动）。失败：滑条无效、关窗丢设置、大厅关不回来。

### 3. Solo 换曲

操作：点 **单人试玩**。

预期：曲切到另一条床垫（`theme_run`）。失败：仍是大厅那首、或完全停了。

### 4. 冲线换结算曲

操作：沿 `course_01` +X 冲线（或走完垫再踩终点）。

预期：冲线后曲再切到第三条（`theme_end`）。失败：仍是 Solo 那首。

### 5. 取消后重启仍记住音量

操作：点 **取消** 回大厅；关游戏再按 0.2 开一次，点 **设置**。

预期：大厅曲回来；音乐滑条仍在步骤 2 的位置。失败：音量回到默认 −6 dB。
...
**诚实边界：** 四首床垫都是现有短 WAV 的拷贝，循环会有接缝。正式 OGG 是 B2。玩法 SFX 路由仍只有预警（B1）。locale 热切换 / 画质 / 键位不做。
...
- 把 temp 正式 OGG 拷进 `content/audio/`（B2；许可证未拍）；
- 跳 / 落地 / 打箱 / 冲线 / 脚步的玩法路由（B1）；
- 联机远端 3D 音（B2）；
- locale 热切换设置页；画质 / 按键重映射；音量上云；
- 第 4 张官方课。

---

## 上一刀（已合入）：M5 A2 Cue Bank 契约

8 个 F 线临时 WAV 从 `ClientAudio` 硬编码登记改成 `game/content/audio/banks/` JSON，经 catalog 白名单校验。`spatial` 只作为数据字段。玩法接线仍只有预警。

---

## 上一刀（已合入）：M5 A1 音频内核（PlaySfx 退场）

F 线 FB 的一次性 `PlaySfx` 换成可复用音频模块：五条总线、声部池、冷却节流、`user://` 音量设置。Autoload 不加。8 个内部临时 WAV 当时仍由 `ClientAudio` 硬编码登记。玩法接线仍只有预警音。

---

## 上一刀（已合入）：M4b 第 5 章（账号认领）

大厅新增 **账号** 按钮。点开后是独立窗：访客 id、用户名、密码、注册 / 登录 / 认领草稿、返回大厅。开发机窗口走进程内目录，不打控制面 HTTP。注册成功后状态变成已登录。关窗回到大厅。发布与匹配仍不绑账号。

不需要三后端。按「共用启动」0.2 开窗口化主场景即可。

### 1. 大厅看见账号按钮

预期：动作行在 **内容广场** 右侧有 **账号**（en：**Account**）。失败：没有这个按钮，或文案变成键名 `craft_arena.ui.account`。

### 2. 打开账号窗看见访客

操作：点 **账号**。

预期：大厅窗隐藏；出现账号窗（窗题 **账号** / **Account**）；状态行以 **访客** / **Guest** 开头并跟一串 `gst_` id；有用户名、密码框和 **注册** / **登录** / **认领草稿**。失败：大厅仍最大化挡在前面，或状态行是键名。

### 3. 注册后变成已登录

操作：用户名填 `warm_forge`（3–24 位字母数字），密码填至少 8 位（例如 `password1`），点 **注册**。

预期：状态行变成 **已登录** / **Signed in** 加 `warm_forge`。失败：点注册没反应、或状态仍是访客、或崩掉。

### 4. 关窗回大厅

操作：点 **返回大厅**，或点账号窗标题栏关闭。

预期：账号窗关闭；大厅窗重新可见，状态行仍是 `join=idle`。失败：关账号后画布空白、或大厅回不来。

### 本刀不测

- 邮箱验证 / CAPTCHA / 密码找回 / 用户名脏话过滤；
- 大厅账号窗打控制面 HTTP（云端路径由 Node 测试覆盖）；
- 发布 HTTP 或入场票据绑账号；
- 完整「我的内容」页（已发布版本 / 内容管理）；
- 把广场课送进快速游戏 / 建房。

---

## 上一刀（已合入）：M4b 第 4 章（内容广场）

大厅新增 **内容广场** 按钮。点开后是独立窗：四个标签（最新 / 评分 / 游玩次数 / 已验证）、列表、单人试玩、返回大厅。开发机默认没有已发布 UGC，所以列表为空。关窗回到大厅。匹配仍只官方课。不测账号。

不需要三后端。按「共用启动」0.2 开窗口化主场景即可。

### 1. 大厅看见内容广场按钮

预期：动作行在 **创作课程** 右侧有 **内容广场**（en：**Content plaza**）。失败：没有这个按钮，或文案变成键名 `craft_arena.ui.plaza`。

### 2. 打开广场窗

操作：点 **内容广场**。

预期：大厅窗隐藏；出现广场窗（窗题 **内容广场** / **Content plaza**）；四个标签从左到右是 **最新**、**评分**、**游玩次数**、**已验证**（en：Newest / Rating / Plays / Verified）；空列表文案 **还没有公开课程** / **No public courses yet**。失败：大厅仍最大化挡在前面、没有四个标签、或空列表文案是键名。

### 3. 切换标签仍是空列表

操作：依次点四个标签。

预期：每个标签都能点；列表仍空（本机没有发布过 UGC）。失败：某个标签点不了，或切到「已验证」崩掉。

### 4. 关窗回大厅

操作：点 **返回大厅**，或点广场窗标题栏关闭。

预期：广场窗关闭；大厅窗重新可见，状态行仍是 `join=idle`。失败：关广场后画布空白、或大厅回不来。

### 本刀不测

- 账号认领 / Guest 登录 / 云端草稿；
- 在开发机窗口里发布 UGC 再看列表（发布仍是控制面 HTTP；大厅不提供发布按钮）；
- 把广场课送进快速游戏 / 建房（匹配 HTTP 仍只 `course_01`/`course_02`/`course_03`）；
- 结算后 1–5 星写回广场（本期 plays/ratings 是独立 HTTP，不接 MatchHost）；
- 官方课必须带签名才能开局（仍不要求信封）。

---

## 上一刀（已合入）：M4b 第 3 章（P0/P1 运行房补丁 + 回滚）
