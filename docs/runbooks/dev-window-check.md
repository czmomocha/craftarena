...
## 本刀：可玩性深化（摆锤 / 冰面 / 失败次数 HUD）

**不需要三后端**（全部走 Solo 与创作入口）。

这刀验三件事：示范课主路旁摆锤会砸人、冰面会滑、HUD 能看见失败次数。

### A. Solo 示范课

默认课是 `course_01`（没有这些）。必须改课程 id。新机关在主路 **+Z 两格**，**不挡 +X 冲线**。

1. 共用启动 0.2 开窗。课程 id 改成 `course_f_playable`，点 **单人试玩**。
   - 预期：开玩后能走。工具栏能看见「爆破球 ×0  冲刺 ×0  失败 ×0」（或等价中英）。
   - 失败：没进 Solo；或仍停在 `course_01`；或开玩后道具栏空白。
2. 从出生点按 **S** 看 +Z 两格：横梁占位是摆锤，旁边一格半透冰面。
   - 预期：摆锤沿 +Z 往返。走进去且不是站在它上面会被砸回检查点，失败次数 +1。站上冰面会被沿箭头方向滑。
   - 失败：那两格仍是空地；或摆锤当电梯只载人；或冰面完全不滑。
3. 点 **取消**。

### B. 编辑器 Place pendulum / ice

4. `& $env:GODOT4 --editor --path game`，菜单 **项目 → 工具 → Authoring Editor**。光标 Y=0 **放置检查点**，Y=0 **放置摆锤**，X 再 +3、Y=−1 **放置冰面**，点 **预览**，点 **开始**（焦点应在右侧 Preview）。
   - 预期：Preview 里摆锤是横梁+锤头，冰面是半透板+箭头。自动推进后摆锤会水平移动；站上冰面会滑。
   - 失败：没有 Place pendulum / Place ice 按钮；或 Preview 仍是石色地块；或编译失败（验证摘要报错）。

### C. 碎裂 / 楼层色 / 预警

5. 仍在 Solo `course_f_playable`：捡出生点爆破球，走到能量墙或碎石前按 **Q**。
   - 预期：箱子 / 碎石消失时弹出几块小碎片，很快没；HUD 爆破球数量减 1。
   - 失败：直接瞬没、没有碎片；或碎片挡住走路（权威仍在）。
6. 看主路石色地板与跳台（−Z 或电梯上去）的地块颜色。
   - 预期：出生层偏石色；往上的楼层偏绿；往下偏品红。传送带 / 冰面 / 摆锤本身不跟着改色。
   - 失败：所有楼层同一石色；或机关占位也被染成绿/品红。
7. 再看压板或摆锤。
   - 预期：横梁 / 锤头会轻微胀缩，像在预警。
   - 失败：完全静止，看不出和普通箱子的差别。

上一刀（滚柱 / 碎石 / 障碍核心 / 道具栏 HUD）已合入。历史步骤以该章 PR 正文为准。

### 本刀不测

- 官方课 01–03 的摆锤 / 冰面（本刀只改示范课）；
- 线上联机失败次数（v1 快照没有该字段，加字段是协议不兼容变更）；
- 滚柱 / 碎石 / 核心（上一刀已验）。
