...
课选择从输入框改成下拉：列出官方课 `course_01`…`course_06` 和示范课 `course_f_playable`，点一项就换 3D 预览，不必再点 **单人试玩** 才切图。单人试玩仍是开局，不是切图按钮。

### 1. 课选择是下拉，不是输入框

点左卡进入机关狂奔。找原来填课程 id 的那一行。

预期：那是下拉菜单，能展开看到 `course_01` … `course_06` 和 `course_f_playable`。不能在框里自由打字。失败：仍是可输入的文本框，必须手打课名。

### 2. 选课立刻切图，不必点单人试玩

默认是 `course_01`（三块检查点垫、一层主路）。打开下拉，选 `course_06`。**不要**点单人试玩。

预期：3D 立刻变成上下叠了好几层的石色地板；状态行 `course_id=course_06`。失败：画面仍是 `course_01` 那张平层，直到点了单人试玩才变。

### 3. 单人试玩仍能在当前课开局

保持 `course_06`，再点 **单人试玩**。

预期：能走，HUD 有离线横幅「离线试玩，成绩不上传」。失败：`offline_error=`，或切回了 `course_01`。

### 4. BASTION 仍钉蓝图

点 **返回大厅**，点右卡。

预期：课下拉只有 `blueprint_01` 且点不开/改不了；没有单人试玩。点返回大厅回到 S1。失败：BASTION 也能选 TRAPRUSH 课，或下拉把战场切成了多层课。

### 本刀不测

- F2 可玩性重签（仍作废，须覆盖 S1 新 UI + 天空 + `course_06`）；
- 匹配 HTTP 是否仍拒 `course_f_playable`（已有自动化；本刀只验下拉里能看见它、Solo 能开）；
- 改 `STUB_HALF`、爆破半径、推击力度。

### 诚实边界

- 下拉项是课程 id，不是本地化课名（UiCopy：课程 id 不翻译）；
- 示范课 `course_f_playable` 在下拉里，只给 Solo / 预览；快速游戏 / 建房仍走匹配白名单（01–06）；
- 对局进行中改下拉不会拆掉当前权威场（等取消后再切预览）。
