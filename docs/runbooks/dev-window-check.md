...
预期：弹出一个 **1600×900** 的窗口（旧版是 960×540），里面是深色的产品 UI：左上 logo 与 **公共内容广场**、一行四个标签页 **最新 LATEST / 评分 RATING / 游玩次数 PLAYS / 已验证 VERIFIED**、右侧三个**灰掉**的控件（排序 / 标签 / 搜索）、下面一行小字说明。大厅窗口同时隐藏。

失败：还是一个窄窗口加一条条文字列表 ⇒ 视图没换上；窗口空白 ⇒ 场景加载失败，看控制台。
...
预期：**被选中的那个是粉色高亮**（设计的 `TabChipActive`），其余是灰底。切换时列表重建。

失败：点了没有任何视觉变化 ⇒ 变种没换；**选中的变成灰白而不是粉色** ⇒ 有人把它改回 `toggle_mode` 了，那会画成 Button 自己的按下态而不是设计的强调色。
...
预期：GUT **1674/1674**（`test_content_plaza.gd` 从 9 条涨到 16 条）；两个校验各一行 `RESULT: PASS`；`--package-check` `ok=true`。
