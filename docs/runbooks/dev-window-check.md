...
启动后先看到产品 UI 的 S1 主大厅（主视口 `Control`），**不是**机关狂奔那扇自绘 `Window`。点一张玩法卡才弹出频道窗（代码创建的窗口化 `Window`，默认与 UI 基准同尺寸，`own_world_3d`）。主窗口与频道窗右下角都有客户端像素读数 `宽 × 高`。窗题与按钮走本地化键（`UiCopy`）：本机 Godot locale 为 `zh*` 时 TRAPRUSH 窗题是 **机关狂奔**，频道窗第一行是 **返回大厅**；BASTION 频道同样这套按钮，但课 id 钉 `blueprint_01`、人数钉 2，且没有单人试玩 / 创作入口 / 冲刺。节点名仍是英文。
