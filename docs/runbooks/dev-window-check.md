...
## 本刀：静态盒均匀格阔相（C5 第 16 章）

静态盒占用改走均匀格阔相。窄相没变，窗口里走/跳/落应与上一刀相同。不需要 `npm run dev`。验的是编辑器 / `--path game` 窗口，不是导出包。
...
3. 沿官方课出生点 −Z 走到地块边缘并走下去（或从出生点向外走下路面）。
   - 预期：先下落，随后出界复位回出生 / 最近检查点（开发桩 ±8 格）。不会穿过脚下固体。
   - 失败：穿地、半空冻结、或出界不复位。
