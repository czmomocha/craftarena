...
## 本刀：M5 C2 第 5 张官方赛道（双路线竞速）

需要三后端。按「共用启动」0.1 起 `npm run dev`，再 0.2 开窗口化主场景。

`course_05` 是双路线竞速：能量墙挡在 +X 脊 x=2。危险捷径 = 走到墙的 −Z 侧按 Q 打碎，再沿脊走进 entity 10 上楼。安全路 = +Z 走到 z=+3，沿 +X 到 x=4，再 −Z 回到脊上的上楼门。侧向 `two_way` 在 z=−2，周期机关在侧廊远端，都不在最短冲线路径上。

**诚实边界：** UseItem reach 只探 +Z 一格，必须站在墙的 −Z 侧才能打碎。探针完整动作集能在重力还没拉下去时对角抄到上楼门，所以 `--bot-run --course=course_05` 重放打碎脚本，而不是搜索。`--route=safe` 仍只对 `course_01`。`course_f_playable` 仍不是匹配课。联机动作音仍是 B2。官方课数量已到 CD-61 的 5 张上限。

### 1. 课程改成第五张并建房

操作：课程 id 改成 `course_05`，人数保持 `2`，点 **创建房间**。

预期：状态行 `course=` 跟到 `course_05`；房间码出现；3D 里能看见出生点脊上的能量墙、+Z 安全长路、上层检查点与金色终点。失败：400 / `invalid_course`、仍加载 `course_01`、或窗口立刻退出。
...
操作：本席向 +X 走一格，再向 −Z 走到墙侧面，按 Q 打碎能量墙，回到脊上走进上楼传送门，踩过上层两块检查点垫，走进金色终点。

预期：墙打碎后能沿 +X 过去；`pads=` 从 1/3 走到 3/3，冲线后出现结算（`result=` / `settled=`）。失败：站在墙 −Z 侧 Q 打不碎、打碎后仍过不去、或冲线没有结算。

### 4. 安全路看一眼（不必用它完赛）

操作：从出生点沿 +Z 走到 z=+3 的长路，确认不用打碎能量墙也能绕到 x=4。

预期：+Z 长路有立足面，走到 x=4 再 −Z 能回到上楼门。失败：安全路走两步就掉出 ±8 格，或长路被能量墙挡住。

### 本刀不测

- 把 `course_f_playable` 填进创建房间（匹配 HTTP 仍拒）；
- `--route=safe` 接到 `course_05`（仍只认 `course_01`）；
- 联机远端 3D 音效（B2）；
- 改爆破半径 / 推击力度 / 道具重生；
