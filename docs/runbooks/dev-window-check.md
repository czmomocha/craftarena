...
## 本刀：M5 A2 Cue Bank 契约

不需要三后端。按「共用启动」0.2 开窗口化主场景即可。

这刀把 8 个 F 线临时 WAV 从 `ClientAudio` 硬编码登记改成 `game/content/audio/banks/` 的 JSON bank，经 catalog 白名单校验。音色、路径、冷却与优先级与 A1 相同。`spatial` 只作为数据字段入库，本刀仍走 2D 播放（3D 是 B1）。

**诚实边界：** 玩法接线仍只有机关预警。其余 7 个 cue 已在 bank 里，调用点仍没有（B1）。窗口只验「预警仍有声」和「没因为改走 bank 而失声 / 爆音」。

### 1. 大厅能开，Solo 能走

操作：共用启动 0.2 开窗，点 **单人试玩**。

预期：开玩后能走。失败：没进 Solo、立刻退出。

### 2. 周期机关预警仍有声

操作：走到出生点 **−Z 两格** 那个洋红滚柱（`course_01` 默认课），等它进入固体半周期之前的预警窗。

预期：预警盒闪的那一下**仍有一声短音**（和 A1 / F 线 FB 同一条 telegraph）。失败：预警盒在、完全无声；或踩实心半周期才响（那是击退，不是预警）。Headless 没有这条——必须窗口化。

### 3. 按住 W 连续跑

操作：按住 **W** 连续跑几格。

预期：不会因为改走 bank 而突然每步叠一串脚步声。失败：按住 W 机关枪爆音。本刀本来就不接线脚步，所以「没有脚步声」是预期。

### 4. 取消回大厅

操作：点 **取消**。

预期：回到 `join=idle`。失败：关不掉 Solo。
