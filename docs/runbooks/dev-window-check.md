...
## 本刀：可玩性深化第二批（传送带入课 / 电梯 / 弹射垫）

**不需要三后端**（全部走 Solo 与创作入口）。

这刀验的是三件连在一起的事：示范课里传送带会推人、弹射垫会弹人、电梯会载人上下；以及编辑器能摆电梯和弹射垫。

### A. 示范课侧廊（Solo `course_f_playable`）

1. 共用启动 0.2 开窗。课程 id 框改成 `course_f_playable`，点 **单人试玩**。
   - 预期：状态行出现示范课（`course=` 不是默认的 `course_01`）；出生点附近能开玩。
   - 失败：仍是官方第一张（没改课程框）。
2. 不要走 +X 主路。从出生点往与主路垂直的那一侧走（示范课的 **+Z**，原先「往返平台」那侧）。走到 +Z 五格的平台后，再往远处多走一格，然后沿 +X 走上石色带子。
   - 预期：三块带子朝 +X 推人；站上去会被带到尽头。
   - 失败：侧廊不存在；或站上去当普通地板。
3. 走到带子尽头那块垫上（比带子更靠 +X）。
   - 预期：被往上弹并沿 +X 送出，落到更高一层的落点；站在落点上**不会连弹**。
   - 失败：站上去没反应；或每次落地又弹。
4. 回到出生点 +Z 七格、+X 一格附近，站上竖直往返的那块平台（电梯）。
   - 预期：站上去会被带着升高，再降回来。
   - 失败：站上去当普通地板。
5. 点 **取消**。

### B. 编辑器 Place lift / Place launch

6. `& $env:GODOT4 --editor --path game`，菜单 **项目 → 工具 → Authoring Editor**。工具条第二行在 **放置传送带** 旁边有 **放置电梯**、**放置弹射垫**。
   - 失败：按钮不存在。
7. 把光标 Y 设到 `-1`，点 **放置电梯**。点 **预览** 开出并排 Preview，点 **开始**（开始后焦点应在右侧 Preview 窗；左边编辑器是摆放世界，电梯不会在那边演），WASD 把人走到电梯上。
   - 预期：一块石色固体落下；站上去会竖直往返（默认沿 +Y 两格）；**右侧 Preview 里那块石色盒自己在动**，不是只有人动、盒子原地。头顶 anim 从 `idle` 变成走路。Godot 输出**没有**刷 `InputMap action 'move_left' doesn't exist`。
   - 失败：点了没反应；或站上去不动；或人动了盒子仍钉在原地；或 WASD 改的是左边 X/Y/Z 格子而不是人；或控制台每帧刷 InputMap 缺动作。
8. 另格连点四次 **放置弹射垫**。Preview 把人走到垫上。
   - 预期：四块朝向顺时针转 90°（与传送带同一规则）；站上去弹一次，走开再踩才再弹。
   - 失败：四块朝向相同；或站着连弹。

### 本刀不测

- 开关门 / `InteractIntent` 接线：需要新增协议 intent id **与**快照字段两处不兼容变更（宪法第十八条），另起一刀；
- 官方课 `course_01` / `02` / `03` 上的传送带 / 电梯 / 弹射垫（本刀只改示范课）；
- 浏览器里的创作路径：本机 `export_templates` 为空；
