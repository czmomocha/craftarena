...
对应：当前完整章节 PR。锁的是 **对局 ShoveIntent**：命令 `intent_id=5`，无线上目标 id。服务端在一格邻域内把最近其它胶囊沿 XZ 推开。大厅 **F** 上升沿采样。不是产品力度，也不接线 Interact。

### 1. Quick play 后 F 推开另一席盒子

前置：共用启动 0.1（`npm run dev`）与 0.2。人数保持默认 **2**。

操作：点 **Quick play**，等到 `play=connecting` 或 `in_match`，点窗口内部一次。应看见青色本席盒和海军蓝另一席盒（空席仍有权威胶囊）。按 **F**。

预期：海军蓝盒子沿远离本席的水平方向挪开一截（占位步长四分之一格）；本席青盒不瞬移。失败：进不去、按 F 崩溃、或两盒都不动且也没有海军蓝第二盒。

### 2. Solo 按 F 不把自己推走

操作：点 **Cancel** 停掉对局（必要时再点一次），点 **Solo play** 开玩，点窗口内部，按 **F**。

预期：只有一枚青盒，按 F 后本席位姿不变、不崩。失败：Solo 把自己弹飞。

### 本刀不测

- 在窗口里指定推击目标 id（协议没有该字段，GUT 覆盖选最近胶囊）；
- 产品推击力度、冷却秒数、Interact、墙钟发送速率、离开对局 HTTP。
