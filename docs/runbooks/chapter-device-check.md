...
## 本刀：无（纠偏 C4 前置 — TRELLIS 2 参考图提示词手册）

对应：当前完整章节 PR。这一章只新增一份作业手册 [`trellis2-reference-prompts.md`](trellis2-reference-prompts.md)，不改 GDScript、不改 TypeScript、不改 `Confirmed-docs/`。

**本章无开发机可见行为**（[CD-52 §3.2](../../Confirmed-docs/50-engineering/52-ai-workflow.md)）：没有窗口、没有按键、没有颜色或数值变化可看。不需要 `npm run dev`，不需要打开 Godot。

人类审查对象是文档本身，两步：

1. 按手册 §9 拍板或驳回 7 条未决事项（BASTION 色板、玩家视觉高度、玩法主题、面数/贴图预算、是否重开描边、生成物授权、出图工具许可）；
2. 抽查 §6 / §7 的资产清单是否覆盖 [CD-21 §7](../../Confirmed-docs/20-gameplay/21-traprush.md) 与 [CD-22 §5](../../Confirmed-docs/20-gameplay/22-bastion.md) 的白名单，并确认 §5.1 的 HEX 与 `game/src/shared/placeholder_spec.gd` 当前值一致。

失败长什么样：色值抄错、把 BASTION 候选色写成已拍板、或资产清单里出现白名单外的对象。

### 本刀不测

- 任何 `.glb` 或真实美术资源（本手册只产出提示词，出图与建模发生在仓库外的工作目录）；
- `GameplayAsset` 契约、导入规范与性能预算（C4 第 2–3 章）；
- 上一章（C4 第 1 章）的占位几何与色板接线，已随 PR #181 合入。
