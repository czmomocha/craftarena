...
## 本刀：周期机关进拓扑（Preview + 对局）

对应：当前完整章节 PR。锁的是 **v1 SimulationBundle 编入 hazard 占用袋，并在 tick 后按已有 `cooldown_ticks` 切换固体**（CD-21 §5.1）。不发明 `period` 字段。不读 damage/knockback。官方三张赛道仍是 0 个机关。意图不推进 tick，故不切换。

### 1. Preview 沙箱：固体时挡住

前置：共用启动 0.2。本刀不需要三后端。不要用 F5 跑主场景。

操作：在 Godot 编辑器打开 `res://src/creator/preview_sandbox.tscn`，**F6 运行当前场景**。Preview 窗口点 **Play**，点窗口内部一次。按住 **D**（世界 +X）。

预期：玩家表现桩走到右侧占位盒（出生点 +X 一格的机关）后停下，穿不过去。失败：一开玩就能穿过该盒、或崩溃。
...
预期：刚才挡住的盒变为可通过，表现桩能继续向 +X 走。失败：点了 Advance tick 仍永远挡住，或还没点就能穿过。

### 3. Solo 官方赛道无暗墙（对照）

前置：共用启动 0.2。本刀对照不需要三后端；若窗口已在对局里，先点 **Cancel**。

操作：点 **Solo play**，在 `course_01` 上沿赛道走几步（含 +X / +Z）。

预期：赛道内走动不被看不见的周期墙弹回或挡住（官方赛道 0 个 hazard）。失败：刚迈出一两步就被一堵不在橙色箱位置的墙挡住。

### 本刀不测

- 伤害、击退、预警动画、产品秒数、墙钟自动 tick、官方赛道塞机关、大厅画机关、编辑器 Place hazard、重力/下落。
