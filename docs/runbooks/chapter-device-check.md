...
对应：当前完整章节 PR。锁的是**本席 SnapshotCamera 跟随表现位姿**（线上为预测 overlay，Solo 为本地权威），偏移与 Preview 相同。不是产品镜头、FOV 或第三人称手感。
...
2. 看 3D。
   ...
   预期：蓝色玩家盒大致在画面中部附近（镜头对着它，而不是钉死在世界原点）。失败：蓝盒已经在画面外，或镜头完全不动地对着空地。

3. 点窗口内部。按住 **D**（或 WASD）走几步。

   预期：镜头跟着蓝色盒走，盒保持在画面里。失败：盒走出画面，镜头仍停在出生点。

4. 点 **Cancel**。

   预期：`offline=` 不再是 `playing`，蓝色盒消失，镜头回到开窗时对着原点的位置；WASD 不再移动任何玩家盒。失败：点了之后还能继续走，或镜头仍停在刚才走过的地方对着空地。
...
3. 点窗口内部。在平地上 **轻点 WASD**（不要冲进传送门）。

   预期：按下的瞬间蓝盒就动，镜头跟着本席盒走。失败：盒动了镜头不动，或按了键要明显顿一拍才动。

4. 点 **Cancel** 停场（不要点窗口关闭，见共用启动说明：Traprush 是嵌入子窗口，关闭只是隐藏）。

   预期：状态行变为 `join=idle` `play=idle`，不再出现 `tick=`；蓝色玩家盒消失；镜头回到开窗时对着原点的位置；WASD 不再让盒移动。失败：点了之后 `tick=` 仍在涨，还能继续走，或镜头留在对局中的位置。
