...
对应：当前完整章节 PR。锁的是 **环境失败后无限复活到最近检查点**（CD-21 §6）。开发桩半宽 ±8 格，不是产品场地。不接重力、不写复活硬直、不计数掉出次数 N。

### 1. Solo 走出桩范围后弹回检查点

前置：共用启动 0.2。本刀不需要三后端；若窗口已在对局里，先点 **Cancel**。

操作：点 **Solo play** 开玩，点窗口内部一次。按住 **W** 约 2 秒（大厅 `play_move_step` 为十六分之一格，8 格约 128 拍）。也可以按 **D** 沿 +X 走出赛道再继续。

预期：青盒走出约 8 格后弹回最近已验收检查点（出生点附近或刚踩过的垫），进度不回退。失败：走几步就弹、或走出很远（远超 8 格）永不复位、或崩溃。

### 2. Quick play 赛道内走动不被弹

前置：共用启动 0.1（`npm run dev`）与 0.2。人数保持默认 **2**。

操作：点 **Quick play**，等到 `play=connecting` 或 `in_match`，点窗口内部。在赛道上走几步（不要刻意冲出 8 格）。

预期：青盒在赛道内正常走动，不被弹回起点。失败：刚迈出一两步就被弹回。

### 3. Preview Play（可选）

操作：F6 跑 Preview 沙箱或编辑器打开 Preview 窗口，点 **Play**，点窗口内部，按住 **W** 走出约 8 格。

预期：玩家表现桩弹回最近检查点。失败：永不复位或一开玩就被弹。

### 本刀不测

- 产品场地尺寸、掉出次数 N、复活硬直、重力/下落、计数淘汰。
