...
操作：WASD 移动，空格跳跃（大厅按钮不抢空格），Q 或鼠标左键使用道具（打碎眼前箱；官方课出生点已拾爆破球），F 基础推击（推开邻座胶囊，无线上目标 id），Left Shift 或 **Sprint** 沿当前朝向冲刺一格（官方课出生点已拾冲刺），R 重置到最近已验收检查点。点窗口内部一次，确保键盘焦点在游戏窗口而不是终端。

---

## 本刀：纠偏 C3 第 3 章 — 爆破球能打碎眼前箱，冲刺沿朝向猛冲且不穿墙

对应：当前完整章节 PR。这一章闭合的是服务端裁决的道具最小集：出生点叠放的爆破球 / 冲刺可拾；Q 打箱要有弹且服务端 reach 命中；Shift 冲刺要有冲刺且沿当前 yaw 走 8 向，固体挡住就停。数值是占位桩，不是产品表。

**不需要三后端。** Solo 足够看见拾取、打箱与冲刺。

### 1. Solo：出生点 Q 打碎 +Z 橙箱，再 Q 无效

按上面共用启动 0.2 开大厅（可以不跑 0.1）。点 **Solo play**。点窗口内部一次。默认课是 `course_01`。

1. 面向默认朝向（不必先走路）。按一下 **Q**（或点 **Use item**）。
2. 预期：出生点 +Z 的橙色箱消失（或变成可穿过）。
3. 失败：箱子还在；或者人没动箱自己碎了。
4. 再按一次 **Q**。
5. 预期：没有第二下有效破坏（已经没有爆破球）。
6. 失败：又能打一次，或场景里另有箱子被误伤。

### 2. Solo：D 面向 +X 后 Shift 冲刺约一格，不穿固体

仍在 `course_01` Solo，点窗口内部一次。

1. 按 **D** 面向 +X（世界 +X）。再按一下 **Left Shift**（或点 **Sprint**）。
2. 预期：青色本席盒沿 +X 猛冲约 1 格，停在路上，不掉进坑，也不穿进石色盒里。
3. 失败：没动；沿 −Z 冲下路面；或者穿墙。
4. 再按一次 **Left Shift**。
5. 预期：没有第二次冲刺位移（已经没有冲刺道具）。
6. 失败：还能再冲一格。
...
1. 预期：停在石色路上不掉；跳一下再落回立足盒。
2. 失败：沿路走几步就下落，或一跳被出界闪回。

### 本刀不测

- 机关伤害/击退、复活硬直、90 秒语义关、锁定快照频率；
- 道具 HUD / 刷新重生 / 产品伤害与冲刺距离；
- 远端双机与 ICMP（C1 已采，C3 不得用那份 ~3ms 锁 CD-43 §4）。

### 仍然欠着（不因本章消失）

机关伤害/击退、D5 复活硬直（候选已写在纠偏方案 D5，待人类选）、24 小时 ICMP、协议层 RTT、C3 网络参数锁定、符合 CD-61 §4.1 语义的赛道。
