...
`dnctl` 的 `plr` 是丢包率，`delay` 是单向毫秒。

抖动没有单独开关，用**两条管道 + `prob` 分流**近似（2026-09-13 实测可用）：

```bash
sudo dnctl pipe 1 config delay 120 plr 0.05
sudo dnctl pipe 2 config delay 150 plr 0.05
cat <<'RULES' | sudo pfctl -f - -E
dummynet out proto tcp from any to any port 8090 prob 0.7 pipe 1
dummynet out proto tcp from any to any port 8090 prob 0.3 pipe 2
RULES
```

70% 的包走 120ms、30% 走 150ms，等效 ±30ms 抖动。想加大抖动就拉开两条管道的 `delay` 差。

**`dnctl` 没有 reorder 与 duplicate**——乱序与重复包在 macOS 上注入不了，只能去 §2.2 的 Linux 测试机。不要因为"跑完没发现异常"就把这两项记成通过：没注入的检查等于没做（见 §5 缺口）。

延迟、丢包与上面这套抖动写法已由人类于 2026-09-13 在 macOS 开发机实测跑通。
...
5. **乱序与重复包**：netem `reorder 25% 50%` + `duplicate 1%`。**macOS 做不了这一步**（`dnctl` 没有这两个开关），必须去 Linux 测试机或用 Windows clumsy；在 macOS 上跳过它就是跳过，别记成通过。
...
一次检查填一行块。**结论只能来自人类执行**；没执行就留"未执行"。AI 只能誊抄人类给出的结论，不得自行判定。

### 第一轮（2026-09-13，人类在 macOS 开发机执行，AI 誊抄）

**7 项已执行并符合预期，2 项未严格注入。**

| 清单项（CD-53 §2.5） | 执行方式 | 结论 | 日期 / 机器 |
|---|---|---|---|
| 延迟 | §2.4 第 2 步 | 符合预期：`rtt` 上升至注入量级，本席预测响应正常，远端延迟可见，快照到达后收敛至权威位姿 | 2026-09-13 / macOS |
| 抖动 | §2.4 第 3 步 | 符合预期：双管道（pipe 1/2，120/150ms）+ `prob` 0.7/0.3 分流实现 ±30ms 抖动；远端动作不匀但未回跳穿墙，`rtt` 波动、`rtt_n` 持续增长 | 2026-09-13 / macOS |
| 丢包 | §2.4 第 4 步 | 符合预期：5% 丢包下卡顿明显但可玩，`tick` 持续前进，未出现伪造位置或崩溃 | 2026-09-13 / macOS |
| 乱序 | §2.4 第 5 步 | **未严格注入**（`dnctl` 无 reorder 开关）。TCP 重组属协议预期、未获链路级反证；待 Linux 测试机 `tc netem reorder 25% 50%` 补充 | 2026-09-13 / macOS |
| 重复包 | §2.4 第 5 步 | **未严格注入**（`dnctl` 无 duplicate 开关）。TCP 去重属协议预期、未获链路级反证；待 Linux 测试机 `tc netem duplicate 1%` 补充 | 2026-09-13 / macOS |
| 短时断线 | §2.4 第 6 步 | 符合预期：5s 断线后自动重连，`seat` 不变，对局仿真未中断 | 2026-09-13 / macOS |
| 基线丢失 | §2.4 第 7 步 | 符合预期：15s 断线恢复后直接贴最新权威位姿，未回放历史过程，`tick` 为当前值 | 2026-09-13 / macOS |
| 恶意高频命令 | §3 第 8 步 | 符合预期：服务端每 `commit_tick` 仅入队一条有效命令，未瞬移，进程稳定，`leaseExpiresAt` 续租正常 | 2026-09-13 / macOS |
| 篡改序号 / Tick / 目标 ID / Payload | §3 第 9 步 | 符合预期：9 类畸形帧全部被拒（`x` 未变），合法帧仍可移动，连接保持存活，无崩溃 | 2026-09-13 / macOS |

备注（整形加在哪一端、用了哪套工具、远端还是本机）：

```text
本机单机执行，未走远端测试机；整形加在本机出方向，走 §2.1 的 dnctl + pfctl。
抖动用两条管道（pipe 1 = 120ms、pipe 2 = 150ms）加 prob 0.7 / 0.3 分流近似 ±30ms。
乱序与重复包在 macOS 上没有对应开关，本轮未注入，见下方缺口。
```

### 本轮缺口：乱序与重复包未获链路级反证

`dnctl` 没有 reorder / duplicate 开关（只有 §2.2 的 `netem` 有）。所以这两项**没有注入过任何异常**，观察到"应用层一切正常"是**同义反复**，不能当成证据——没注入本来就不该有异常。

按 §0 的推理，TCP 会重组与去重，因此应用层收不到乱序或重复帧；这是**协议层面的预期**，不是本轮测出来的事实。要真正证伪，得在 Linux 测试机上按 §2.2 跑：

```bash
sudo tc qdisc add dev eth0 root netem delay 120ms reorder 25% 50% duplicate 1%
```

在补上这一轮之前，**CD-53 §2.5 的九项只能算 7 项已执行**，乱序与重复包是已知缺口（宪法第二十四条：未执行不得表述为已覆盖）。

### 仍然成立的边界

- 本轮是**一次人工检查，不是回归覆盖**。下次改动网络路径（协议帧、网关、重连、插值）**不会**自动被这张表保护，要重跑。
- 本轮全部在**本机单机**，没有真实广域链路。远端样本仍只有 [server-deploy.md §13](server-deploy.md#13-协议层-rttc3) 那一场。
- 第 8 步验的是"刷帧改不了裁决结果"，**不是**"刷帧会被挡住"——一期没有速率限制与封禁（宪法第二十五条）。
