...
- [x] **Clients & Tools → Settings**：Vision Routing 保持关（主 Dock 上看不到此项，属正常；macOS 第 3 步已在 Settings 页确认，EditorSettings 无 `vision_routing/*` 键即默认关）
- [x] 没有同时启用第二套 Godot MCP（macOS：Cursor 用户级 `mcp.json` 与当前会话 MCP 目录仅 `godot-ai`）
