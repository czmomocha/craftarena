...
| 2 | 是否引入烘焙工具与依赖（如 MIT 的 `@gltf-transform/cli`） | **已拍板（2026-08-30）**：引入 `@gltf-transform/cli` 4.4.2（MIT），但**不进 `package.json`**，由 [资产烘焙 runbook](../runbooks/asset-bake.md) 用 `npx @gltf-transform/cli@4.4.2` 按版本拉起；门禁侧只用 `@gltf-transform/core` 4.4.2（无 native、0 漏洞）。理由与 CVE 边界见 [CD-51 §1](../../Confirmed-docs/50-engineering/51-dev-environment.md)。**本文 §2「跨平台不成立」与 §9「不提供可复现的工具链」自此失效**：跨平台命令已存在，硬编码 `sips` 的一次性脚本退役 |
| 3 | 是否把烘焙流水线立为 C4 的一章 | **已拍板（2026-08-30）：不放 C4**。C4 第 4 章只落"一条手动烘焙命令 + 单资产准入门禁"；按用途分档参数（§5.1 / §5.3 的结论）、批量与自动化留到 C5 重排时归入表现/美术纵向线 |

§7 三项均已拍板。**仍未拍板**：场景总量 / Draw call / 材质数 / 骨骼上限（[CD-63 §1.7](../../Confirmed-docs/60-plan/63-open-decisions.md) 延期，人类同日明确 C4 第 4 章不做），以及 KTX2 / Basis 压缩路径。
...
> 以下步骤依赖本机 `test-res/` 下的原始样本与烘焙脚本，两者均**未入库**（见 §2 与 `.gitignore`）。**2026-08-30 起不必再重写脚本**：跨平台烘焙命令见 [资产烘焙 runbook](../runbooks/asset-bake.md)，`sips` 那份一次性脚本已退役。换机器复现仍需自备样本。
...
- 烘焙脚本硬编码 `/usr/bin/sips`，**只在 macOS 成立**（该脚本已于 2026-08-30 退役，见 §7 第 2 项）；
