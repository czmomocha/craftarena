...
> 日期：2026-09-10。状态：章节划分与音频模块基建已由人类拍板（§5.1）；**A1 已无阻断项，可以开工**。
...
| # | 产出 | 现状 | 落在本文件哪几章 |
|---|---|---|---|
| 1 | 3～5 张官方赛道（第 4 张及以后） | 01–03 已入库；匹配 HTTP 白名单只认这三张 | C1、C2 |
...
3. **已签名 UGC → 联机**：匹配 HTTP 只认 `course_01/02/03` 三个 enum，自制课只能 Solo。
...
| 2 | 第 4 / 第 5 张官方课的主题与机关组合 | C1、C2 | 04 = 垂直塔（电梯 + 弹射 + 压板，考验上下层）；05 = 双路线竞速（安全长路 vs 需打碎能量墙的短路） |
| 3 | 发布签名密钥去向（**阻断项**） | C3 | 客户端不持密钥；控制面在账号鉴权后代签。安全边界变化，须明确批准 |
| 4 | 匹配 HTTP 扩到已签名 UGC（**阻断项**） | C4 | 批准后再开工；协议不兼容变更 |
| 5 | 两个小目录：`tools/audio-bank/` 建不建、`tools/bot-runner/` 建还是从 CD-41 删 | A2、C5 | 音频校验并入 `tools/content-validator/`；`tools/bot-runner/` 建薄壳（CD-41 已列它，删了要改架构文档） |
| 6 | 网络故障注入工具 | C6 | Windows 用 clumsy、Linux 用 `tc netem`；外部工具不入库，仍需点头 |

**A1 已无阻断项，可以开工。** 第 1 项在 B2 素材入库前必须有答案，其余在对应章开工前再定。
