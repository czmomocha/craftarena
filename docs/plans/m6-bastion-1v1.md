...
> 日期：2026-09-16。状态：**D1–F1 已交，F2 起未开工。** 八项已拍板或按诚实边界落地（§5.1）；§5.2 无余项。D 段交的是**全离线的玩法底座**；E1 交了协议帧与服务端裁剪；E2 交了第一张官方蓝图；E3 交了对局进程分派、匹配 HTTP 玩法位与结算队伍结果；E4 交了客户端对局壳（第一次有画面是 E4）；**F1 交了 S1 主大厅壳**（双玩法入口；频道仍是自绘 Window）。
...
| CD-61 明写的下一动 | **M6 F2**（退出验收 + TRAPRUSH 可玩性重签） |
| D1 的两项硬阻断 | **已拍板**（2026-09-15，均采纳 AI 推荐），见 [§5.1](#51-已拍板2026-09-15) |
| D1–F1 | **已交**（F1：2026-09-16）。下一刀是 **F2**（M6 退出验收 + TRAPRUSH 可玩性重签） |
...
| 4 | 建塔、升级、出售 | D4 已交 | D4 |
| 5 | 基础兵线与核心 | D3 已交 | D3 |
| 6 | 无预测网络同步 | E1 已交实时帧 v1 扩展（type 5/6）；E3 已交对局进程分派；**E4 已交客户端跟从（无预测）** | E1、E3、E4 |
| + | UI 接线第二批：S1 主大厅壳 | **已交（F1，2026-09-16）**：落地壳 + 双玩法入口；频道仍是自绘 Window；`project.godot` 全局 theme 仍不挂 | F1 |
...
| 面 | 现状 | BASTION 缺什么 |
|---|---|---|
| 快照帧 | TRAPRUSH：每玩家 41 字节 + 每箱 16 字节。**E1 已交** type=6 BASTION 快照。**E3 已交**对局进程按席位单播裁剪。**E4 已交**客户端跟从 | — |
| 命令帧 | TRAPRUSH intent id 1–6。**E1 已交** type=5、id 7–12。**E3 已交**对局进程按玩法分派解码。**E4 已交**建造意图提交（无预测） | — |
| 匹配 HTTP | `course` 与 `content` 互斥。**E3 已交**可选 `gameplay` 与官方 `blueprint`。**E4 已交**自绘大厅课 id `blueprint_01` | S1 主大厅壳仍是 F1 |
| 结算 HTTP | `finishTick` / `acceptedCount` / `padTotal` / `place`。**E3 已交**可选 `teams[]`。**E4 已交**状态行 `result=` token | S1 结算面板仍是 F1 |
...
### E3 对局进程 + 匹配 + 1v1 席位与队伍 + 结算写库（**已交，2026-09-15**）
...
- **阻断**：第 4、5 项已于开工时拍板（见 [§5.1](#51-已拍板2026-09-15)）。
- **窗口**：无。纯 HTTP / Headless 分派，没有可点的控件；第一次有画面仍是 E4。
- **不做**：跨玩法匹配池混排；改 FIFO 排队规则；把入场票据绑账号。

### E4 客户端 BASTION 对局壳（**已交，2026-09-16**）
...
### F1 UI 接线第二批：S1 主大厅壳（双玩法入口）（**已交，2026-09-16**）

- **交付**：`s1_lobby.tscn` 接进运行时，替换 `match_lobby_*` 自绘 `Window` 的**壳**；新建 `s1_lobby.gd`；硬编码文案迁 `UiCopy`（键在 `ui_copy_s1.gd`，查表仍走 `UiCopy.text()`），并把 `s1_lobby.tscn` 从 `test_ui_scene_copy.gd` 的 `PENDING_SCENES` 移到 `MIGRATED_SCENES`；BASTION 卡从「即将推出 COMING SOON 🔒」改成开放态；双玩法频道入口按 CD-12 §1 的树接线；关频道窗回 S1；`project.godot` 的全局 theme **仍不挂**（等第三批接完再统一挂，否则会当场改写还在跑的自绘界面）。公开 API：`open()` 仍显示频道窗（GUT 回归）；玩家启动走 `try_show_home()`。
...
- **不做**：S2 / S4 / S5 / S6（第三批）；**角色选择界面**（见 [§5.3](#53-角色选择排期已关闭2026-09-19)，当时不塞进本章；2026-09-19 已交，不在 M6 章内）。
...
前五项都采纳了 AI 推荐。所有者文档与覆盖链见 [CD-91 D.4](../../Confirmed-docs/90-reference/91-decision-log.md)，本节不复述那里的未覆盖范围。第五项（官方蓝图主题）同日开工 E2 时拍板；第 4、5 项（匹配玩法位、结算队伍）同日开工 E3 时拍板，也都采纳 AI 推荐。
...
| 第一张官方蓝图的主题 | **`blueprint_01` = 对称双线**：每侧一条短主路 + 一条绕行分支，核心前汇合；每侧 3 障碍槽卡在分支口；5 波（快速 → 集群 → 快速 → 重装 → 集群）、5 分钟局时。经济沿用灰盒占位桩，不锁产品数值。**E3 已接线匹配 HTTP** | [CD-22 当前生效值](../../Confirmed-docs/20-gameplay/22-bastion.md)；键 `official_bastion_blueprint_01` |
| 匹配 HTTP 玩法判别位 | **加可选 `gameplay`，缺省 TRAPRUSH**；官方蓝图走独立的 `blueprint` id，与 `course` / `content` 三者互斥。不把 `course` 塞成 `"bastion:blueprint_01"`。BASTION 锁定 2 席。广场按玩法分列 | [CD-42 §3.5](../../Confirmed-docs/40-technical/42-contracts-and-rulevm.md#35-匹配与玩家发布-httpc3-已接线c4-已接线)；键 `match_http_gameplay` |
| 结算 HTTP 队伍结果 | **加可选 `teams[]`，复用已有 `mvpSlot`**。TRAPRUSH 现有必填字段一律不动。`place` 在 BASTION 读作队伍名次。平局两侧都可以是 1。一期仍只有单局名次与 MVP | [CD-42 §3.5](../../Confirmed-docs/40-technical/42-contracts-and-rulevm.md#35-匹配与玩家发布-httpc3-已接线c4-已接线)；键 `settlement_http_teams` |
| E4 音效映射 | **复用已登记 cue**，无新 catalog id、无新 OGG。router 在 `game/src/games/bastion/audio_router.gd`。新素材入库仍须宪法第十八条 | [CD-42 §1.4](../../Confirmed-docs/40-technical/42-contracts-and-rulevm.md#14-音频-cue-bank-v1)；键 `bastion_e4_audio` |

**「3 种障碍」是本次新增的口径，不是文档原有要求。** CD-61 §4.2 明写「3 种塔、3 种兵」，但障碍那一条只写「双方各 3 个障碍**槽**」。这一句留在这里，是为了下次有人回头查「三种障碍是谁定的」时不会误读成夹具本来就要求。

### 5.2 待拍板（阻断，AI 不得自选）

**八项均已关闭并迁到 [§5.1](#51-已拍板2026-09-15)**，行号保留以便先前讨论仍能对上号（覆盖而非删除，照 CD-63 的记法）。CD-22 §4.2 / §5.1 / §5.2 的表格自己写着「不是锁定清单」，不因本表关闭而变成锁定清单。
...
| 4 | ~~匹配 HTTP 玩法判别位~~ | ~~E3~~ | **已拍板**（2026-09-15）：可选 `gameplay` + 独立 `blueprint`。结论见 [§5.1](#51-已拍板2026-09-15) | — |
| 5 | ~~结算 HTTP 队伍结果~~ | ~~E3~~ | **已拍板**（2026-09-15）：可选 `teams[]`，复用 `mvpSlot`。结论见 [§5.1](#51-已拍板2026-09-15) | — |
...
| 8 | ~~BASTION 音效素材与 cue 映射~~ | ~~E4~~ | **已落地**（2026-09-16）：E4 按本章诚实边界复用已登记 cue，无新 OGG。新素材入库仍须宪法第十八条。结论见 [§5.1](#51-已拍板2026-09-15) | — |

第 1、2 项曾是硬阻断——没有它们，D1 连文件名都定不下来；第 3、6 项曾挡 E1；第 7 项曾挡 E2；第 4、5 项曾挡 E3；第 8 项曾挡 E4；**现已全部关闭**。

### 5.3 角色选择排期（已关闭，2026-09-19）

CD-61 §1 的顺序列表曾把「角色选择界面」排在 **UI 接线第三批之后**，而同一文件「未开工」行与该列表的括注都写「**紧跟主大厅壳**」。主大厅壳在 M6（F1），第三批在公开 TLS 之后——两句给出的位置差了一大截。

本计划当时**不自行择一**，只把角色选择排在 M6 之外、不塞进 F1。人类 2026-09-19 点名开工并拍板：排在 **F1 与天空 A3 之后、F2 之前**，不发明新里程碑号，不塞进 F2。落地见 [CD-12](../../Confirmed-docs/10-product/12-product-structure.md) 与 [CD-91](../../Confirmed-docs/90-reference/91-decision-log.md) `character_select_screen = catalog_local_persist`。
...
本文件里有两句话只要被后人顺手删掉，下一个读者就会做出错误判断：「E4 交完 ≠ M6 已退出」与「F1 会让 M5 那份可玩性签署作废」。所以它们有机械门禁：`tools/dev-launcher/tests/m6_plan.test.ts` 钉住本文件的 11 章、§5.2 的八行（现均已关闭）、四处第十八条边界，以及 CD-61 §2 M6 / CD-22 与两份 `.cursor/rules` 的指向。

D1–F1 落地时这条门禁**收窄过**：原先钉的是「BASTION 一行实现都没有」，那句话在 D1 之后就假了；后来钉「E1 起未开工」……「F1 起未开工」，交完那句也假了。现在改钉三件仍然为真、且同样容易被顺手抹掉的事——F2 起未开工、§4 引子里那句「第一次有画面是 E4」、M6 未退出。
...
| 把 CD-61 §2 M6 里「F2 起**未开工**」改成「已全部交付」 | 非零 | `exit=1`，6 项里 1 项红 |
...
`tools/dev-launcher/tests/m5_exit.test.ts` 里那条「两份规则指向哪一刀」的断言同时收窄了：从整号 `M6 / M7` 改成具体章（`M6 D1`，随后 D5、E1、E2、E3、E4，E4 落地后是 `M6 F1`），并加了一条反例禁止再退回整号指向——上表第三行就是它。

里程碑归属：M6
是否绕过 placeholder_spec 散落几何/色板常量：否
审查级别：D1 深审（`shared/` 契约 + `ugc/` 编译 + UGC 安全边界）；D2 深审（进 `simulation/`，权威裁决路径）；D3 常审（`games/`）；D4 深审（CD-61 §2 M6 验收句直接点名「伪造金币和伪造建造均被拒绝」）；D5 深审（受控 P2 拓扑修改 + 安全边界）；E1 深审（网络协议不兼容变更）；E2 常审（官方蓝图）；E3 深审（匹配 HTTP + 结算 HTTP + 数据迁移）；E4 常审（客户端壳）
开发机窗口步骤：E4 起有编号步骤，见 [开发机窗口验收](../runbooks/dev-window-check.md) 本刀。D 段与 E1 / E2 / E3 没有可见窗口行为。
