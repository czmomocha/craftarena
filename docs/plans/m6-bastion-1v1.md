...
> 日期：2026-09-15。状态：**D1–E1 已交，E2 起未开工。** 四项已拍板（§5.1）；其余四项仍待人类裁决（§5.2），卡在各自那一章之前。**E2 被第 7 项挡住**（官方蓝图主题），AI 不得自选、不得开工。D 段交的是**全离线的玩法底座**；E1 交了协议帧与服务端裁剪，但官方蓝图、对局进程、客户端壳仍没有，**现在开不出一局给人看，第一次有画面仍是 E4**。
...
| D1–E1 | **已交**（2026-09-15）。下一刀是 **E2**，被 §5.2 第 7 项挡住（官方蓝图主题），AI 不得自选、不得开工 |
...
四项都采纳了 AI 推荐。所有者文档与覆盖链见 [CD-91 D.4](../../Confirmed-docs/90-reference/91-decision-log.md)，本节不复述那里的未覆盖范围。
...
余下四项全部仍是「AI 给推荐、不给结论」，**没有一项被当成结论写进章节交付**。CD-22 §4.2 / §5.1 / §5.2 的表格自己写着「不是锁定清单」。下表第 1、2、3、6 行已于 2026-09-15 关闭并迁到 [§5.1](#51-已拍板2026-09-15)，**行号保留**以便先前讨论仍能对上号（覆盖而非删除，照 CD-63 的记法）。
...
第 1、2 项曾是硬阻断——没有它们，D1 连文件名都定不下来；第 3、6 项曾挡 E1；**2026-09-15 已关闭**。第 4、5、7、8 项卡在各自那一章开工前，**不挡 E2 之前已交的章**。
...
D1–E1 落地时这条门禁**收窄过**：原先钉的是「BASTION 一行实现都没有」，那句话在 D1 之后就假了；后来钉「E1 起未开工」，E1 交完那句也假了。现在改钉三件仍然为真、且同样容易被顺手抹掉的事——E2 起未开工、§4 引子里那句「要到 E4 才有画面」、M6 未退出。
...
| 把 CD-61 §2 M6 里「E2 起**未开工**」改成「已全部交付」 | 非零 | `exit=1`，6 项里 1 项红 |
...
`tools/dev-launcher/tests/m5_exit.test.ts` 里那条「两份规则指向哪一刀」的断言同时收窄了：从整号 `M6 / M7` 改成具体章（`M6 D1`，随后 D5、E1，E1 落地后是 `M6 E2`），并加了一条反例禁止再退回整号指向——上表第三行就是它。

里程碑归属：M6
是否绕过 placeholder_spec 散落几何/色板常量：否
审查级别：D1 深审（`shared/` 契约 + `ugc/` 编译 + UGC 安全边界）；D2 深审（进 `simulation/`，权威裁决路径）；D3 常审（`games/`）；D4 深审（CD-61 §2 M6 验收句直接点名「伪造金币和伪造建造均被拒绝」）；D5 深审（受控 P2 拓扑修改 + 安全边界）；E1 深审（网络协议不兼容变更）
开发机窗口步骤：无。D 段与 E1 不改任何运行时可见行为，没有可点的控件；M6 第一次有窗口行为是 E4。
