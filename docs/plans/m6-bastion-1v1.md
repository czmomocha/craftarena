...
> 日期：2026-09-15。状态：**D1–E3 已交，E4 起未开工。** 七项已拍板（§5.1）；其余一项仍待人类裁决（§5.2 第 8 项音效素材），卡在 E4 之前。D 段交的是**全离线的玩法底座**；E1 交了协议帧与服务端裁剪；E2 交了第一张官方蓝图；E3 交了对局进程分派、匹配 HTTP 玩法位与结算队伍结果。客户端壳仍没有，**现在开不出一局给人看，第一次有画面仍是 E4**。
...
| D1–E3 | **已交**（2026-09-15）。下一刀是 **E4**，被 §5.2 第 8 项挡住（BASTION 音效素材），AI 不得自选、不得开工 |
...
| 4 | 建塔、升级、出售 | D4 已交 | D4 |
| 5 | 基础兵线与核心 | D3 已交 | D3 |
| 6 | 无预测网络同步 | E1 已交实时帧 v1 扩展（type 5/6）；E3 已交对局进程分派；客户端壳仍是 E4 | E1、E3、E4 |
...
| 面 | 现状 | BASTION 缺什么 |
|---|---|---|
| 快照帧 | TRAPRUSH：每玩家 41 字节 + 每箱 16 字节。**E1 已交** type=6 BASTION 快照。**E3 已交**对局进程按席位单播裁剪 | 客户端壳还没跟从（E4） |
| 命令帧 | TRAPRUSH intent id 1–6。**E1 已交** type=5、id 7–12。**E3 已交**对局进程按玩法分派解码 | 客户端壳还没提交建造意图（E4） |
| 匹配 HTTP | `course` 与 `content` 互斥。**E3 已交**可选 `gameplay` 与官方 `blueprint` | 客户端大厅入口仍是 TRAPRUSH（E4 / F1） |
| 结算 HTTP | `finishTick` / `acceptedCount` / `padTotal` / `place`。**E3 已交**可选 `teams[]` | 客户端结算面板仍是 TRAPRUSH 形状（E4） |
...
余下一项仍是「AI 给推荐、不给结论」，**没有被当成结论写进章节交付**。CD-22 §4.2 / §5.1 / §5.2 的表格自己写着「不是锁定清单」。下表第 1、2、3、4、5、6、7 行已于 2026-09-15 关闭并迁到 [§5.1](#51-已拍板2026-09-15)，**行号保留**以便先前讨论仍能对上号（覆盖而非删除，照 CD-63 的记法）。
...
第 1、2 项曾是硬阻断——没有它们，D1 连文件名都定不下来；第 3、6 项曾挡 E1；第 7 项曾挡 E2；第 4、5 项曾挡 E3；**2026-09-15 已关闭**。第 8 项卡在 E4 开工前，**不挡 E4 之前已交的章**。
...
D1–E3 落地时这条门禁**收窄过**：原先钉的是「BASTION 一行实现都没有」，那句话在 D1 之后就假了；后来钉「E1 起未开工」「E2 起未开工」「E3 起未开工」，交完那句也假了。现在改钉三件仍然为真、且同样容易被顺手抹掉的事——E4 起未开工、§4 引子里那句「要到 E4 才有画面」、M6 未退出。
...
| 把 CD-61 §2 M6 里「E4 起**未开工**」改成「已全部交付」 | 非零 | `exit=1`，6 项里 1 项红 |
...
`tools/dev-launcher/tests/m5_exit.test.ts` 里那条「两份规则指向哪一刀」的断言同时收窄了：从整号 `M6 / M7` 改成具体章（`M6 D1`，随后 D5、E1、E2、E3，E3 落地后是 `M6 E4`），并加了一条反例禁止再退回整号指向——上表第三行就是它。

里程碑归属：M6
是否绕过 placeholder_spec 散落几何/色板常量：否
审查级别：D1 深审（`shared/` 契约 + `ugc/` 编译 + UGC 安全边界）；D2 深审（进 `simulation/`，权威裁决路径）；D3 常审（`games/`）；D4 深审（CD-61 §2 M6 验收句直接点名「伪造金币和伪造建造均被拒绝」）；D5 深审（受控 P2 拓扑修改 + 安全边界）；E1 深审（网络协议不兼容变更）；E2 常审（官方蓝图）；E3 深审（匹配 HTTP + 结算 HTTP + 数据迁移）
开发机窗口步骤：无。D 段与 E1 / E2 / E3 不改任何运行时可见行为，没有可点的控件；M6 第一次有窗口行为是 E4。
