...
> 日期：2026-09-15。状态：**D1–D4 已交，D5 起未开工。** 两项硬阻断已拍板（§5.1）；其余六项仍待人类裁决（§5.2），卡在各自那一章之前。D 段交的是**全离线的玩法底座**：`game/src/games/bastion/` 与 `game/src/ugc/bastion_*` 已落地，但协议帧、官方蓝图、对局进程、客户端壳一样都没有，**现在开不出一局给人看，第一次有画面仍是 E4**。
...
| D1–D4 | **已交**（2026-09-15）。下一刀是 **D5**，它不被 §5.2 的任何一项挡住 |
...
### D5 互设障碍阶段（离线：盲设 → 锁定 → 揭示 → 重验 → 退点）（**下一刀**）

- **D3 已经落了哪一半**：阶段本身、锁定标志、倒计时、以及「放一个障碍要先过 `BastionPathGuard`」。**没落的是 D5 的全部要点**——点数预算、盲设、统一揭示、揭示后重验与退点、`MatchSetupState`、布障命令进回放。不得把 D3 那一半说成盲设已经做了。
...
D1–D4 落地时这条门禁**收窄过一次**：原先钉的是「BASTION 一行实现都没有」，那句话在 D1 之后就假了，直接删掉等于把门禁拆了。现在改钉三件仍然为真、且同样容易被顺手抹掉的事——D5 起未开工、§4 引子里那句「要到 E4 才有画面」、M6 未退出。
...
| 把 CD-61 §2 M6 里「D5 起**未开工**」改成「已全部交付」 | 非零 | `exit=1`，6 项里 1 项红 |
...
`tools/dev-launcher/tests/m5_exit.test.ts` 里那条「两份规则指向哪一刀」的断言同时收窄了：从整号 `M6 / M7` 改成具体章（`M6 D1`，D1–D4 落地后是 `M6 D5`），并加了一条反例禁止再退回整号指向——上表第三行就是它。

里程碑归属：M6
是否绕过 placeholder_spec 散落几何/色板常量：否
审查级别：D1 深审（`shared/` 契约 + `ugc/` 编译 + UGC 安全边界）；D2 深审（进 `simulation/`，权威裁决路径）；D3 常审（`games/`）；D4 深审（CD-61 §2 M6 验收句直接点名「伪造金币和伪造建造均被拒绝」）
开发机窗口步骤：无。D 段四章不改任何运行时可见行为，没有可点的控件；M6 第一次有窗口行为是 E4。
