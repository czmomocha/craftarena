...
- 当前阶段：M1 阶段 B 进行中（两域第三轮已合入；第 3 域未开）。进度与退出条件见 [CD-61](Confirmed-docs/60-plan/61-milestones.md)。
