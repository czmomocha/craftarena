...
- 当前阶段：**纠偏冻结令已解除**（2026-09-03）。F 线已收口。**Web 游玩分发第一刀已交**。**可玩性深化已收口**（2026-09-09；第一至十八批。示范课 `course_f_playable` revision 12 与官方 01–03 侧廊机关仍在，匹配 HTTP 仍只这三张）。**M4a 已收口**（第 1–5 章：v1 信封 / 解释器 / gas / 图编译 / Event 分发 / Query·Logic·Action 子集 / Preview P3 安全点重编译，公开对局禁止）。**M4b 第 1–3 章已交**（ContentHash + HMAC + 发布切 `latest` + 新房吃新版本 + P0/P1 运行房补丁 + 进程内回滚 + latest 指针回滚；无广场 / 账号）。Android / iOS 烟测与字体入包排到 **一期收尾**。公开 TLS 与 PR Web 沙盒仍在 M5 之后。进度见 [CD-61](Confirmed-docs/60-plan/61-milestones.md)。`--bot-run` 默认三张官方课；示范课用 `--course=course_f_playable`（不计入 M5 官方课）。D-F10：跳跃接线后步数 / 回放哈希允许变。
