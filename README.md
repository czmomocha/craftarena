...
- 当前阶段：M1 阶段 B 进行中（章节节奏；XZ 接触推进与调用方下落已合入；第 3 域未开）。进度与退出条件见 [CD-61](Confirmed-docs/60-plan/61-milestones.md)。
