...
- 当前阶段：**纠偏冻结令已解除**（2026-09-03）。下一动：F 线（趣味性增强 FA–FH）之后是 M-Export 的 Android / iOS 烟测。公开 TLS 与 PR Web 沙盒排到 M5 之后。字体入包推迟到一期收尾。进度见 [CD-61](Confirmed-docs/60-plan/61-milestones.md)。`--bot-run` 默认三张官方课；示范课用 `--course=course_f_playable`（不计入 M5 官方课）。D-F10：跳跃接线后步数 / 回放哈希允许变。
