...
- 当前阶段：**纠偏冻结令已解除**（2026-09-03）。F 线已收口。**Web 游玩分发第一刀已交**。**可玩性深化已收口**（2026-09-09；第一至十八批。示范课 `course_f_playable` revision 12 与官方 01–03 侧廊机关仍在）。**M4a 已收口**（第 1–5 章：v1 信封 / 解释器 / gas / 图编译 / Event 分发 / Query·Logic·Action 子集 / Preview P3 安全点重编译，公开对局禁止）。**M4b 第 1–5 章已交**（ContentHash + HMAC + 发布切 `latest` + 新房吃新版本 + P0/P1 运行房补丁 + 进程内回滚 + latest 指针回滚 + 内容广场 + 账号认领；发布 HTTP 与入场票据仍不绑账号）。**M5 C1 已交第 4 张官方课** `course_04`（垂直塔）；**C2 已交第 5 张** `course_05`（双路线竞速）；匹配 HTTP 认 01–05。**C3 玩家代签发布、C4 邀请与已签名 UGC 联机、C5 BotRunner 薄壳、C6 退出验收清单均已交——M5 十一章全交，退出只差人类执行并签署** [网络故障人工检查](docs/runbooks/network-fault-check.md) 与 [可玩性签署：TRAPRUSH](docs/runbooks/playability-signoff-traprush.md)；签完后下一动是**字体入包**，再是 UI 接线第一批。Android / iOS 烟测排到 **一期收尾**。公开 TLS 与 PR Web 沙盒仍在 M5 之后。进度见 [CD-61](Confirmed-docs/60-plan/61-milestones.md)。`--bot-run` 默认匹配白名单课；示范课用 `--course=course_f_playable`（不计入 M5 官方课）。D-F10：跳跃接线后步数 / 回放哈希允许变。
