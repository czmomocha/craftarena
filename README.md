...
- 当前阶段：**纠偏冻结令已解除**（2026-09-03）。F 线已收口。**Web 游玩分发第一刀已交**。**可玩性深化进行中**（第十八批已交：示范课 `course_f_playable` revision 12 捷径不再空等半周期；官方 01–03 侧廊已接新机关，匹配 HTTP 仍只这三张）。Android / iOS 烟测与字体入包排到 **一期收尾**。公开 TLS 与 PR Web 沙盒仍在 M5 之后。进度见 [CD-61](Confirmed-docs/60-plan/61-milestones.md)。`--bot-run` 默认三张官方课；示范课用 `--course=course_f_playable`（不计入 M5 官方课）。D-F10：跳跃接线后步数 / 回放哈希允许变。
