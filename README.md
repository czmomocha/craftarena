...
- 当前阶段：**纠偏冻结令已解除**（2026-09-03）。下一动：创作者可测（XYZ / Place pickup / Preview 并排），然后 M-Export 的 Android / iOS 烟测。公开 TLS 与 PR Web 沙盒排到 M5 之后。字体入包推迟到一期收尾。进度见 [CD-61](Confirmed-docs/60-plan/61-milestones.md)。`--bot-run` 三张课步数须仍为 5 / 5 / 12。
